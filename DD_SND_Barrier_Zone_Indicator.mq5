...
color GetIndicatorTFColor(bool isDemand, string tf)
{
   if(tf == "D1")  return isDemand ? C'0,229,255' : C'255,180,0';      // Cyan / Gold
   if(tf == "H4")  return isDemand ? C'0,230,118' : C'235,50,150';     // Mint Green / Magenta
   if(tf == "H1")  return isDemand ? C'120,255,0' : C'255,60,60';      // Lime / Coral Red
...
void RenderSNDBoxes()
{
   ...
   for(int i = 0; i < InpMaxZonesPerSide; i++)
   {
      ...
      if(i < ArraySize(finalSupply))
      {
         ...
         if(ObjectFind(0, boxName) < 0)
         {
            ...
            ObjectSetInteger(0, boxName, OBJPROP_COLOR, GetIndicatorTFColor(false, item.tf));
         ...
         if(InpShowLabels)
         {
            ...
            if(ObjectFind(0, txtName) < 0)
            {
               ...
               ObjectSetInteger(0, txtName, OBJPROP_COLOR, GetIndicatorTFColor(false, item.tf));
   ...
   for(int i = 0; i < InpMaxZonesPerSide; i++)
   {
      ...
      if(i < ArraySize(finalDemand))
      {
         ...
         {
            ObjectCreate(0, boxName, OBJ_RECTANGLE, 0, t1, item.hi, t2, item.lo);
            ObjectSetInteger(0, boxName, OBJPROP_COLOR, GetIndicatorTFColor(true, item.tf));
         ...
         if(InpShowLabels)
         {
            ...
            if(ObjectFind(0, txtName) < 0)
            {
               ...
               ObjectSetInteger(0, txtName, OBJPROP_COLOR, GetIndicatorTFColor(true, item.tf));
               ObjectSetInteger(0, txtName, OBJPROP_FONTSIZE, 9);
               ObjectSetString(0, txtName, OBJPROP_FONT, "JetBrains Mono");
            }

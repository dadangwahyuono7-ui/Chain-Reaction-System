...
#define EA_VERSION "v52.67-FRIEND-REMOTEBM"
...
// v52.67 - DD_ChainReaction_FRIEND variant only, NOT in the production EA.
// Dadang: "dia berdiri sendiri bro baca mt5 dia sendiri dia independen hanya
// data bookmap ikut data gw" - a friend runs his OWN MT5/broker/account, but
// mirrors Dadang's live Bookmap read over the network instead of needing his
// own Bookmap install+subscription. Source: sultan_dashboard_server.py's
// sanitized /bookmap_share.json endpoint (Bookmap-derived fields ONLY - no
// balance/equity/CMP-signals, see BOOKMAP_SHARE_FIELDS there). No login -
// Dadang: "wkwkwk lupa gw bro buang aja deh gak usah password password an" -
// this ONE path is allowlisted open in tunnel_gate.py (everything else on
// the dashboard, balance/equity included, stays password-gated as before).
// Requires the receiving MT5 to whitelist the URL under Tools > Options >
// Expert Advisors > "Allow WebRequest for listed URL" - WebRequest fails
// silently (falls back to Bookmap OFFLINE, same as no bridge at all)
// otherwise.
input group "=== REMOTE BOOKMAP (FRIEND build only) ==="
input bool                InpUseRemoteBookmap     = true;                                            // Ambil data Bookmap lewat network (bukan file lokal) - matiin kalau PC ini PUNYA Bookmap sendiri
input string              InpRemoteBookmapUrl     = "https://trade.dadangchatai.com/bookmap_share.json"; // Endpoint sanitized publik (Bookmap doang, gak ada balance/equity/sinyal, gak perlu login)
input int                 InpRemoteBookmapPollSec = 2;                                                 // Interval fetch (detik) - jangan terlalu sering, ini lewat internet
...
input bool                InpUseFusionH1H4       = false;  // Aktifkan sinyal Fusion H1/H4 - OFF default, WAJIB tes demo dulu
input double              InpFusionZoneTolUsd    = 4.0;    // Toleransi retest ke ZONA H4 (USD) - 40 pip XAUUSD = $4.00, angka yang tervalidasi
input double              InpFusionEmergencySLUsd= 140.0;  // SL DARURAT (USD, backstop doang bukan exit normal) - worst case historis -$132.1, dibulatin ke atas
...
#define SWEEP_DISPLAY_MAX_SEC (20.0 * 60.0)   // don't show a marker/row for a sweep older than this - stale, not "live"
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         // v52.36: Dadang asked for POC/VAH/VAL explained, then: "jika di
         // area tengah 2 statusnya sideways... jika di atas poc atau di
         // bawah poc statusnya apa ya mungkin potensi atau apa gitu" - see
         // ComputeVaLocation()'s comment for the full BUY/POTENTIAL BUY/
         // SIDEWAYS/POTENTIAL SELL/SELL breakdown.
         string vaLoc = ComputeVaLocation();
         color vaLocClr = (StringFind(vaLoc, "SELL") >= 0) ? PNL_ROSE
                         : (StringFind(vaLoc, "BUY") >= 0)  ? PNL_EMERALD : PNL_LABEL;
         PnlRow(x0, y, contentW, "VA Bias", vaLoc, vaLocClr); y += 16;

         // v52.42: VA Retest trigger state - Dadang: "kita hanya entri
         // ketika BREAK VAL ATAU VAH DAN pullback ke area val atau hal atau
         // poc." Shows: OFF (toggle disabled), ARMED (breakout confirmed,
         // waiting for pullback), IN POS (this trigger has an open position,
         // SL trailing active), or "-" (nothing armed right now).
         if(InpUseVaRetestTrigger)
         {
            string vrTxt; color vrClr;
            if(g_vaRetestTicket != 0)      { vrTxt = "IN POSITION (SL trailing)"; vrClr = PNL_EMERALD; }
            else if(g_vaRetestArmedDir != ""){ vrTxt = "ARMED " + g_vaRetestArmedDir + " - nunggu retest"; vrClr = C'251,191,36'; }
            else                            { vrTxt = "-"; vrClr = PNL_LABEL; }
            PnlRow(x0, y, contentW, "VA Retest", vrTxt, vrClr); y += 16;
         }
         if(InpUseFusionH1H4)   // v52.45
         {
            string fzTxt; color fzClr;
            if(g_fusionTicket != 0)       { fzTxt = "IN POSITION (struktural, tunggu H4 flip)"; fzClr = PNL_EMERALD; }
            else if(g_fusionVrSet)         { fzTxt = "VR ARMED " + g_fusionDir + " - nunggu retest zona H4"; fzClr = C'251,191,36'; }
            else                           { fzTxt = "-"; fzClr = PNL_LABEL; }
            PnlRow(x0, y, contentW, "Fusion H1/H4", fzTxt, fzClr); y += 16;
         }
         if(InpUseBarrierVeto)   // v52.47, compacted to 1 multi-TF row v52.59
         {
            color bvClr; string bvTxt = BarrierWarningText(bvClr);
            PnlRow(x0, y, contentW, "Barrier", bvTxt, bvClr); y += 16;
         }
         ...
         // v52.26: wall SWEEP + reversal - Dadang: "ide gila lagi bro?" ->
         // stop-hunt/liquidity-grab detection (see UpdateSweepMarker()'s
         // comment for the full doctrine). Only shown while fresh
         // (SWEEP_DISPLAY_MAX_SEC) - a resolved event from an hour ago would
         // otherwise sit here forever, since it stays "latest" until the
         // NEXT sweep happens.
         string sweepTxt; color sweepClr;
         if(g_bmSweepStatus == "NONE" || g_bmSweepSinceSec > SWEEP_DISPLAY_MAX_SEC)
         {
         ...
         else
         {
            ...
            // different ones.
            string sweepTag = (g_bmSweepStatus == "REVERSAL_CONFIRMED") ? "REVERSAL"
                             : (g_bmSweepStatus == "CONTINUATION")      ? "LANJUT" : "PENDING";
            sweepTxt = StringFormat("%s %.0fL %s (%.0fs)", g_bmSweepSide, g_bmSweepSize, sweepTag, g_bmSweepSinceSec);
...
{
   datetime dummyT;
   string dir = ReadCMP(cmpHandle, dummyT);
   if(dir != "BUY" && dir != "SELL") { clrOut = PNL_LABEL; return "-"; }
   bool isBuy = (dir == "BUY");

   double brkLevel = ReadLiveLevel(cmpHandle, 4);   // frozen flip-level - the level that broke
   if(brkLevel <= 0) { clrOut = PNL_LABEL; return "-"; }
   ...
   clrOut = DirColor(dir);
   double ratio = distance / avgRange;
...
//======================================================================
// v52.67 REMOTE BOOKMAP (FRIEND build only) - lightweight, schema-specific
// JSON reading (no library - MQL5 has none built in). Deliberately NOT a
// general-purpose parser: it only knows the exact field names
// /bookmap_share.json produces, which WE control on both ends.
//======================================================================
string JsonGetObject(string json, string key)
{
   ...
   if(p < 0) return "";
   int start = p + StringLen(needle) - 1;
   int depth = 0;
...
//--- parses a "[[px,sz],[px,sz],...]" wall-ladder array into 2 parallel
//--- output arrays, returns how many pairs it found (capped at maxSlots).
int JsonGetLadder(string json, string key, double &pxOut[], double &szOut[], int maxSlots)
{
   string arr = JsonGetArray(json, key);
   int count = 0;
   int len = StringLen(arr);
   int i = 0;
   while(i < len && count < maxSlots)
   {
      int lb = StringFind(arr, "[", i);
      if(lb < 0) break;
      int rb = StringFind(arr, "]", lb);
      if(rb < 0) break;
      string pair = StringSubstr(arr, lb + 1, rb - lb - 1);
      int comma = StringFind(pair, ",");
      if(comma > 0)
      {
         pxOut[count] = StringToDouble(StringSubstr(pair, 0, comma));
         szOut[count] = StringToDouble(StringSubstr(pair, comma + 1));
         count++;
      }
      i = rb + 1;
   }
...
datetime g_lastRemoteBookmapFetch = 0;
//--- fetches the sanitized Bookmap-only snapshot from Dadang's PC and
//--- populates the SAME g_bookmap* globals ReadBookmapBridge() normally
//--- fills from the local CSV - every downstream consumer (wall drawing,
//--- POC/absorption warnings, panel display) works unmodified either way.
//--- g_bookmapPrice is deliberately set to THIS terminal's OWN live bid
//--- (not Bookmap's real GCZ6 price) - every existing +bmOffset call site
//--- computes offset = thisBid - g_bookmapPrice, so this makes that offset
//--- resolve to 0 and the already-XAUUSD-scale remote values pass through
//--- unchanged, with ZERO changes needed to any downstream code. Valid
//--- because Dadang confirmed the friend uses the SAME broker - see memory,
//--- otherwise the two brokers' XAUUSD quotes could differ and this would
//--- need re-deriving from a raw (un-offset) price instead.
void FetchRemoteBookmapData()
{
   if((int)(TimeGMT() - g_lastRemoteBookmapFetch) < InpRemoteBookmapPollSec) return;
   g_lastRemoteBookmapFetch = TimeGMT();

   char postData[];
   char result[];
   string resultHeaders;
   ResetLastError();
   int res = WebRequest("GET", InpRemoteBookmapUrl, "", 5000, postData, result, resultHeaders);   // no auth header - endpoint is publicly allowlisted, see input group comment
   if(res != 200)
   {
      g_bookmapOnline = false;
      if(res == -1)
         Print("Remote Bookmap fetch gagal: WebRequest error ", GetLastError(),
               " - cek Tools>Options>Expert Advisors>'Allow WebRequest for listed URL', tambahin: ", InpRemoteBookmapUrl);
      else
         Print("Remote Bookmap fetch gagal: HTTP ", res);
      return;
   }

   string json = CharArrayToString(result, 0, WHOLE_ARRAY, CP_UTF8);
   if(!JsonGetBool(json, "online")) { g_bookmapOnline = false; return; }

   string flow  = JsonGetObject(json, "flow");
   string liq   = JsonGetObject(json, "liquidity");
   string loc   = JsonGetObject(json, "location");
   string sweep = JsonGetObject(json, "wall_sweep");

   g_bookmapOnline = true;
   g_bookmapPrice  = SymbolInfoDouble(_Symbol, SYMBOL_BID);   // see function comment - makes bmOffset a no-op
   g_bookmapAgeMs  = 0;                                       // fresh fetch this cycle

   g_bookmapCvd            = JsonGetNumber(flow, "cvd");
   g_bookmapPulsePct       = JsonGetNumber(flow, "pulse_pct");
   g_bookmapAbsorption     = JsonGetString(flow, "absorption");
   g_bookmapVolRatioBuyPct = JsonGetNumber(flow, "vol_ratio_buy_pct");

   g_bookmapPocPrice = JsonGetNumber(loc, "poc");
   g_bookmapVal      = JsonGetNumber(loc, "val");
   g_bookmapVah      = JsonGetNumber(loc, "vah");

   ArrayInitialize(g_bookmapBidPx, 0.0); ArrayInitialize(g_bookmapBidSz, 0.0);
   ArrayInitialize(g_bookmapAskPx, 0.0); ArrayInitialize(g_bookmapAskSz, 0.0);
   double bidPx[], bidSz[], askPx[], askSz[];
   ArrayResize(bidPx, WALL_SLOTS_PER_SIDE); ArrayResize(bidSz, WALL_SLOTS_PER_SIDE);
   ArrayResize(askPx, WALL_SLOTS_PER_SIDE); ArrayResize(askSz, WALL_SLOTS_PER_SIDE);
   int nb = JsonGetLadder(liq, "bid_ladder", bidPx, bidSz, WALL_SLOTS_PER_SIDE);
   int na = JsonGetLadder(liq, "ask_ladder", askPx, askSz, WALL_SLOTS_PER_SIDE);
   for(int i = 0; i < nb; i++) { g_bookmapBidPx[i] = bidPx[i]; g_bookmapBidSz[i] = bidSz[i]; }
   for(int i = 0; i < na; i++) { g_bookmapAskPx[i] = askPx[i]; g_bookmapAskSz[i] = askSz[i]; }

   string sweepStatus = JsonGetString(sweep, "status");
   if(sweepStatus != "")
   {
      g_bmSweepSide     = JsonGetString(sweep, "side");
      g_bmSweepPrice    = JsonGetNumber(sweep, "price");
      g_bmSweepSize     = JsonGetNumber(sweep, "size");
      g_bmSweepStatus   = sweepStatus;
      g_bmSweepSinceSec = JsonGetNumber(sweep, "since_sec");
   }
}
...
void ReadBookmapBridge()
{
   ...
   if(InpUseRemoteBookmap)   // v52.67: FRIEND build - fetch over network instead of the local CSV, see function comment
   {
      FetchRemoteBookmapData();
      return;
   }
...
void DrawWallLine(string key, double bookmapPrice, double size, double offset, bool isBid, string sideLabel, int barsAhead = 8, bool showLabel = true)
{
   ...
   bool sweptHere = (g_bmSweepStatus != "NONE" && g_bmSweepSinceSec <= SWEEP_DISPLAY_MAX_SEC &&
                     MathAbs(bookmapPrice - g_bmSweepPrice) < 0.05 &&
                     ((isBid && g_bmSweepSide == "BID") || (!isBid && g_bmSweepSide == "ASK")));
   ...
   if(sweptHere)
   {
      ...
      string tag = (g_bmSweepStatus == "REVERSAL_CONFIRMED") ? "REVERSAL" : (g_bmSweepStatus == "CONTINUATION") ? "LANJUT" : "PENDING";
      clr       = clrDeepSkyBlue;   // fixed - never used by a normal wall, so this alone says "sweep event"
      width     = 3;
      lineText  = StringFormat("%s SWEPT %.0f lot @ %.2f - %s (%.0fs ago)", sideLabel, size, mt5Price, tag, g_bmSweepSinceSec);
      labelText = StringFormat(" %s SWEPT: %s (%.0fs)", sideLabel, tag, g_bmSweepSinceSec);
   }
   else
   {
...
void DrawWallZone(string sideName, int zoneIdx, double loPriceBm, double hiPriceBm, double totalLot, int wallCount, double offset, bool isBid)
{
   ...
   bool sweptHere = (g_bmSweepStatus != "NONE" && g_bmSweepSinceSec <= SWEEP_DISPLAY_MAX_SEC &&
                     g_bmSweepPrice >= (loPriceBm - 0.05) && g_bmSweepPrice <= (hiPriceBm + 0.05) &&
                     ((isBid && g_bmSweepSide == "BID") || (!isBid && g_bmSweepSide == "ASK")));
   color  clr;
   string labelText;
   if(sweptHere)
   {
      string tag = (g_bmSweepStatus == "REVERSAL_CONFIRMED") ? "REVERSAL" : (g_bmSweepStatus == "CONTINUATION") ? "LANJUT" : "PENDING";
      clr = clrDeepSkyBlue;
      labelText = StringFormat(" %s ZONE SWEPT: %s (%.0fs) - %d wall (%.0f lot)", sideName, tag, g_bmSweepSinceSec, wallCount, totalLot);
   }
   else
   {
...
//--- Iceberg=magenta) - "blue = sweep event" either way. Auto-hides once
//--- older than SWEEP_DISPLAY_MAX_SEC - a resolved event from an hour ago
//--- shouldn't linger looking "live".
void UpdateSweepMarker()
{
   string name     = WALL_PREFIX + "SWEEP_FALLBACK";
   string textName = WALL_PREFIX + "SWEEP_FALLBACK_TXT";
   bool freshSweep = (g_bookmapOnline && g_bmSweepStatus != "NONE" && g_bmSweepPrice > 0 &&
                      g_bmSweepSinceSec <= SWEEP_DISPLAY_MAX_SEC);
   if(!freshSweep || g_sweepDrawnOnWallLine)   // already shown on the wall's own slot - nothing more to draw
   {
      if(ObjectFind(0, name) >= 0)     ObjectDelete(0, name);
      if(ObjectFind(0, textName) >= 0) ObjectDelete(0, textName);
      return;
   ...
   ObjectSetString(0, name, OBJPROP_TEXT,
      StringFormat("SWEEP %s %.0f lot @ %.2f - %s (%.0fs ago)", g_bmSweepSide, g_bmSweepSize, mt5Price, tag, g_bmSweepSinceSec));
   ...
   ObjectSetString(0, textName, OBJPROP_TEXT,
      StringFormat(" SWEEP %s: %s (%.0fs)", g_bmSweepSide, tag, g_bmSweepSinceSec));
}
...
void WriteSultanStatus()
{
   ...
   double sweepMt5Price = (g_bmSweepPrice > 0) ? g_bmSweepPrice + offset : 0.0;
   json += StringFormat("\"wall_sweep\":{\"side\":\"%s\",\"price\":%.2f,\"size\":%.1f,\"age_at_sweep_sec\":%.0f,\"status\":\"%s\",\"since_sec\":%.0f},",
                         g_bmSweepSide, sweepMt5Price, g_bmSweepSize, g_bmSweepAgeSec, g_bmSweepStatus, g_bmSweepSinceSec);
...
bool TryOpen(string dir, string tag, bool isScalp, datetime parentFlipTime = 0, datetime childFlipTime = 0, double slOverride = 0.0)
{
   ...
   if(g_fusionTicket == 0 && g_fusionDir != "WAIT")
   {
      ...
      else
      {
         ...
         if(g_fusionVrLevel > 0.0 && crossed && m5Ok)
         {
            ...
            {
               double slLevel = (g_fusionDir == "BUY") ? curPrice - InpFusionEmergencySLUsd
                                                         : curPrice + InpFusionEmergencySLUsd;
               if(TryOpen(g_fusionDir, "FUSION_H1H4", false, 0, 0, slLevel))
               {
                  ...
                  Print("FUSION H1/H4 ENTRY: ", g_fusionDir, " @ ", curPrice,

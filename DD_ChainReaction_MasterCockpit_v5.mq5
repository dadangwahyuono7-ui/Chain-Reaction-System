...
#define EA_VERSION "v56.00-DADANG-MASTER-COCKPIT-HD-FONTS"
...
input group "=== ENTRI ==="
input bool                InpAllowScalpEntries = false;  // Blind Chain Entries OFF (Wajib lewat S&D / Kotak Suport & Resisten Asli)     // Safety valve - false = infrastruktur jalan tapi gak ada posisi beneran dibuka
...
enum ENUM_BOOKMAP_INPUT_SOURCE
{
   BM_SOURCE_WEBREQUEST = 0, // Native WebRequest (Mini PC Tailscale 100.71.97.6)
   BM_SOURCE_LOCAL_FILE = 1  // Local File (Common\Files\bookmap_live_signal.csv)
};

input group "=== BOOKMAP NATIVE HTTP STREAM (V4 ZERO-CLIENT) ==="
input ENUM_BOOKMAP_INPUT_SOURCE InpBookmapSource       = BM_SOURCE_WEBREQUEST; // Data Ingestion Source
input string                    InpBookmapURL          = "http://100.71.97.6:8766/bookmap_live_signal.csv"; // Mini PC Endpoint URL (Tailscale)
input int                       InpFetchTimeoutMs      = 1000; // WebRequest Timeout (ms) [Default 1000ms]
...
input group "=== VISUAL S&D PRIORITY DISPLAY (Clean Visual & Data Preserved) ==="
input bool                ShowSDZones              = true;   // Gambar Rectangle & Label S&D di chart
input bool                ShowDetailedZoneLabels   = false; // BERSIH TOTAL v53.30   // Tampilkan detail lot, kekuatan, uji, serap
input bool                ShowBookmapWallMarkers = true;
input double              InpWallMinDrawLot      = 50.0;   // Minimal Akumulasi Lot Wall yang digambar di chart (50-100+ lot)  // v53.42: OFF - Semua data wall sudah tertulis rapi di dalam kotak S&D (chart bersih tanpa garis wall)
input bool                ShowBookmapWallZones   = false;  // v53.39: OFF - S&D zones dari zones_v2.csv sudah menjadi satu-satunya zona di chart (tidak ada kotak dobel)
input bool                ShowCMPLevels            = false; // BERSIH TOTAL v53.30   // Tampilkan CMP levels
input bool                ShowProfileLevels        = true; // BERSIH TOTAL v53.30   // Tampilkan POC, VAH, VAL

input int                 ZoneP1Opacity            = 45;     // Opacity P1 (S1/D1) - paling jelas
input int                 ZoneP2Opacity            = 25;     // Opacity P2 (S2/D2) - sedang
input int                 ZoneP3Opacity            = 12;     // Opacity P3 (S3/D3) - ringan
input int                 ZoneDistantOpacity       = 0;      // Zone > 3 hidden secara visual
...
input int                 InpZoneCFMinScore      = 70;     // v53.30: Dadang - hanya PUCUK-PUCUK LANCIP / BENTENG RAKSASA (Skor >= 70) yang jadi dasar S&D     // Skor zona minimum buat ENTRY/TP (SEDANG/KUAT) - zona LEMAH diabaikan, terlalu noisy buat jadi dasar entry beneran
input int                 InpMinScoreToShowChart = 70;     // v53.30: Dadang - filter murni hanya Pucuk Lancip & Benteng Raksasa (Skor >= 70)     // v53.30: Dadang - hanya BENTENG UTAMA (Skor >= 60) yang jadi patokan jarak S1/D1 agar peta lebar dan tidak kena false kompresi sempit     // Skor zona minimum buat TAMPIL di chart (terpisah dari syarat entry) - v53.8: dinaikin dari 0 - Dadang: "zona zona yang penting aja bro, kalo banyak banget gimana mau trading" - zona LEMAH banget (baru kebentuk, <20) sekarang gak digambar dulu, biar chart gak penuh
//--- v53.47: SIERRA CHART PRO ORDER FLOW SUITE (6 WEAPONS)
input group "--- Sierra Chart Pro Order Flow Suite ---"
input bool                InpSC_StackedImbalance = true;   // 1. Stacked Diagonal Imbalance (3:1 Walls)
input bool                InpSC_NakedPoc         = true;   // 2. Virgin / Naked POC Magnet (VPOC)
input bool                InpSC_DeltaDivergence  = true;   // 3. Delta Divergence (Trapped Traders / Perangkap Bandar)
input bool                InpSC_BigTradesWhale   = false; // Handled by unified Footprint Whale Badges   // 4. Whale Bubbles / Big Trades (>50 Lot)
input double              InpSC_BigTradeLotMin   = 100.0;  // Minimal Lot Paus Sejati (100 Lot) agar tidak spam label
input bool                InpSC_UnfinishedAuction= true;   // 5. Unfinished Auction (Poor High / Low)
input bool                InpSC_ProfileShapeBias = true;   // 6. P-Shape & b-Shape Auction Profile Bias

#define SC_PREFIX "DD_SC_"

// v53.56: Live state Sierra Chart Suite buat export ke sultan_status.json
// (dashboard web). Diisi DI DALAM masing-masing Update*Engine() function di
// bawah, PERSIS titik yang sama tempat dia mutusin gambar/gak gambar - jadi
// JSON export baca hasil keputusan yang SAMA, bukan itung ulang sendiri
// (itu anti-pattern yang barusan dibuang dari custom-chart-engine's
// CMPLogic.js - dua sumber kebenaran bisa beda angka).
string   g_scImbalanceSide   = "NONE";   // BUY / SELL / NONE
string   g_scDivergenceType  = "NONE";   // BEAR / BULL / NONE (trapped buyer/seller)
string   g_scWhaleSide       = "NONE";   // BUY / SELL / NONE
double   g_scWhaleLots       = 0.0;
bool     g_scUnfinishedFound = false;
datetime g_scUnfinishedTime  = 0;
double   g_scUnfinishedPrice = 0.0;

//--- v53.46: CANDLE FOOTPRINT DELTA OVERLAY
input group "--- Candle Footprint Delta Overlay ---"
input bool                InpShowCandleDelta     = true;   // Tampilkan Angka Delta Footprint di Setiap Lilin Candle
input int                 InpCandleDeltaBars     = 1;      // Jumlah Candle yang Diberi Delta (Default 1 = HANYA CANDLE AKTIF, chart super bersih)

#define CDELTA_PREFIX "DD_CDELTA_"
#define MAX_CDELTA_BARS 50
...
// fix: S&D PRICE MAP panel was showing g_zoneTotalLot[0] (raw CSV row 0 -
// whichever zone happened to be first in zones_v2.csv, not necessarily
// S1/D1) next to the S1/D1 range. sLot[]/dLot[] in DrawAllZones() already
// carry the CORRECTLY matched lot through the sort+overlap-filter
// pipeline - just wasn't saved anywhere the panel could read. These two
// arrays close that gap.
double   g_supplyRoadLot[SDPM_MAX_ROADMAP], g_demandRoadLot[SDPM_MAX_ROADMAP];
...
//+------------------------------------------------------------------+
//| v53.30: SMART REFRESH GLOBALS
//+------------------------------------------------------------------+
datetime g_lastPanelDraw   = 0;
datetime g_lastWallDraw    = 0;
datetime g_lastZoneDraw    = 0;
datetime g_lastSierraDraw  = 0;
datetime g_lastProfileDraw = 0;
bool     g_needRedraw      = false;


//+------------------------------------------------------------------+
//| v53.31: STABLE OBJECT RENDERER - Create Once, Update When Changed
//| Eliminates ObjectsDeleteAll + ObjectCreate on every tick = no flicker
//+------------------------------------------------------------------+
#define SD_MAX_ZONES 6
...
ZoneRenderState g_supplyState[SD_MAX_ZONES];
ZoneRenderState g_demandState[SD_MAX_ZONES];

// Wall render state cache
...
WallRenderState g_wallBidState[20];
WallRenderState g_wallAskState[20];

bool g_zoneStateInited = false;

void InitZoneStateCache()
{
   for(int i = 0; i < SD_MAX_ZONES; i++)
   {
      g_supplyState[i].active = false; g_supplyState[i].lo = 0; g_supplyState[i].hi = 0;
      g_demandState[i].active = false; g_demandState[i].lo = 0; g_demandState[i].hi = 0;
   }
   for(int i = 0; i < 20; i++)
   {
      g_wallBidState[i].active = false; g_wallBidState[i].price = 0;
      g_wallAskState[i].active = false; g_wallAskState[i].price = 0;
   }
   g_zoneStateInited = true;
}

// Helper: Create if not exists, always returns true
// Helper: Create if not exists, always returns true
bool EnsureObject(string name, ENUM_OBJECT type, int subwindow=0)
{
   if(ObjectFind(0, name) < 0) { ObjectCreate(0, name, type, subwindow, 0, 0); }
   return true;
}

// Helper: Update RECTANGLE area only (stays in candle area, never crosses left panel!)
void SetRectangle(string name, datetime t1, double p1, datetime t2, double p2, color clr, int width, ENUM_LINE_STYLE style, bool fill=true)
{
   if(!EnsureObject(name, OBJ_RECTANGLE)) return;
   ObjectMove(0, name, 0, t1, p1);
   ObjectMove(0, name, 1, t2, p2);
   ObjectSetInteger(0, name, OBJPROP_COLOR, clr);
   ObjectSetInteger(0, name, OBJPROP_WIDTH, width);
   ObjectSetInteger(0, name, OBJPROP_STYLE, style);
   ObjectSetInteger(0, name, OBJPROP_FILL, fill);   // Shaded soft fill persis gambar
   ObjectSetInteger(0, name, OBJPROP_BACK, true);   // Di belakang lilin candle
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, name, OBJPROP_HIDDEN, true);
}

// Helper: Update HLINE position only if changed
void SetHLine(string name, double price, color clr, int width, ENUM_LINE_STYLE style)
{
   if(!EnsureObject(name, OBJ_HLINE)) return;
   double cur = ObjectGetDouble(0, name, OBJPROP_PRICE, 0);
   if(MathAbs(cur - price) > 0.001) ObjectMove(0, name, 0, 0, price);
   ObjectSetInteger(0, name, OBJPROP_COLOR, clr);
   ObjectSetInteger(0, name, OBJPROP_WIDTH, width);
   ObjectSetInteger(0, name, OBJPROP_STYLE, style);
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
...
// Helper: Update OBJ_TEXT only if changed
void SetText(string name, datetime tPos, double price, string txt, color clr, int fontSize, string font="Segoe UI Semibold", ENUM_ANCHOR_POINT anchor=ANCHOR_LEFT)
{
   if(!EnsureObject(name, OBJ_TEXT)) return;
   ObjectSetString(0, name, OBJPROP_TEXT, txt);
   ObjectMove(0, name, 0, tPos, price);
   ObjectSetInteger(0, name, OBJPROP_COLOR, clr);
...
// Helper: Hide an object (set far off-screen price) instead of delete
void HideLineObject(string name)
{
   if(ObjectFind(0, name) < 0) return;
   ObjectSetString(0, name, OBJPROP_TEXT, "");
   ObjectMove(0, name, 0, 0, -99999.0);
}

int OnInit()
{
   UpdateHudToggleButtons();
   if(!g_togglePocVa) { ObjectDelete(0, WALL_PREFIX + "POC"); ObjectDelete(0, WALL_PREFIX + "POC_TXT"); ObjectDelete(0, WALL_PREFIX + "VAH"); ObjectDelete(0, WALL_PREFIX + "VAH_TXT"); ObjectDelete(0, WALL_PREFIX + "VAL"); ObjectDelete(0, WALL_PREFIX + "VAL_TXT"); }
   // PEMBERSIHAN MUTLAK KOTAK & GARIS OTOMATIS (ULTRA CLEAN CHART)
   ObjectsDeleteAll(0, "ICT_ZONE_");
   ObjectsDeleteAll(0, "ICT_");
   ObjectsDeleteAll(0, "SD_ZONE_");
   ObjectsDeleteAll(0, "DD_ZONE_");
   ObjectsDeleteAll(0, "WALL_ZONE_");

   // v53.30: NUCLEAR CLEANUP - hapus semua objek DD_ dari chart setiap init
   ObjectsDeleteAll(0, "DD_");
   ObjectsDeleteAll(0, "SDZ_");
   ObjectsDeleteAll(0, "SD_");
   ObjectsDeleteAll(0, "WALL_");
   ObjectsDeleteAll(0, CDELTA_PREFIX);
   ObjectsDeleteAll(0, SC_PREFIX);
   ObjectsDeleteAll(0, "DD_HUD_"); ObjectsDeleteAll(0, "DD_RADAR_"); ObjectsDeleteAll(0, "HUD_");
   // ChartRedraw(0); // removed - anti-flicker v53.30

   trade.SetExpertMagicNumber(InpMagic);
   trade.SetDeviationInPoints(InpSlippage);

   // v53.37: Auto-detect symbol filling mode for Strategy Tester & Live Brokers
   ...
   // v53.30: Smart Refresh Timer - 5 detik interval
...
void OnTimer()
{
   ...
   g_ictZoneCount = 0;
   ScanICT_InstitutionalStructures(PERIOD_H4);
   ScanICT_InstitutionalStructures(PERIOD_H1);
   DrawICT_StructuresOnChart();
   ...
   // 1b. Sierra Chart Pro Order Flow Suite - v53.50: dipisah dari gate zona
   // di atas. Sebelumnya nempel di dalam if(InpShowZonesOnChart...), jadi
   // kalau Dadang matiin tampilan zona SND, seluruh suite ini ikut mati
   // diam-diam tanpa alasan yang kelihatan. Sekarang independen, tetap
   // jalan tiap 5 detik terlepas dari toggle zona.
   if(now - g_lastSierraDraw >= 5)
   {
      RunSierraChartSuite();
      g_lastSierraDraw = now;
   }

   // 2. Profile Levels (VAH, VAL, POC) - periksa setiap 5 detik
   if(ShowProfileLevels && now - g_lastProfileDraw >= 5)
   {
      UpdatePocLine();
      UpdateValueAreaLines();
      UpdateIcebergLines();
      UpdateSweepMarker();
      g_lastProfileDraw = now;
   }

   // 3. Main Dashboard Panel (Kiri) - update setiap 2 detik
   if(now - g_lastPanelDraw >= 2)
   {
      UpdatePanel();
      g_lastPanelDraw = now;
   }

   // 4. Bookmap Wall Markers - periksa setiap 15 detik jika diaktifkan
   if(ShowBookmapWallMarkers && now - g_lastWallDraw >= 5)
   {
      UpdateWallZones();
      UpdateWallLines();
      g_lastWallDraw = now;
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "WALL_");
   ObjectsDeleteAll(0, "DADANG_");
   ObjectsDeleteAll(0, "DD_HUD_");
   ObjectsDeleteAll(0, SDPM_PREFIX);
   ObjectsDeleteAll(0, SDRM_PREFIX);
   ObjectsDeleteAll(0, SDBRK_PREFIX);
...
//+------------------------------------------------------------------+
//| 👑 DADANG MASTER COCKPIT V5: LARGE CRISP HD FONTS & 5-TAB SUITE   |
//+------------------------------------------------------------------+
enum ENUM_COCKPIT_TAB
{
   TAB_ORDER_FLOW_INTEL = 0,   // [ 📊 ORDER FLOW ]
   TAB_BOOKMAP_SUITE    = 1,   // [ 🌊 BOOKMAP SUITE ]
   TAB_FOOTPRINT_SIERRA = 2,   // [ 👣 FOOTPRINT & SC ]
   TAB_SD_GENESIS_LADDER= 3,   // [ 🧱 CME LADDER ]
   TAB_SNIPER_SULTAN    = 4    // [ 🎯 SNIPER & SULTAN ]
};

int               g_cockpitX = 20;              // Window X offset
int               g_cockpitY = 25;              // Window Y offset
int               g_cockpitW = 800;             // Window Width (HD Spacious)
int               g_cockpitH = 630;             // Window Height (HD Spacious)
...
// ⚡ News Straddle Settings (Dual Trap Breakout)
...
//+------------------------------------------------------------------+
//| v53.48: CONFLUENCE RADAR SCORE ENGINE (0 - 100% SETUP VALIDATOR) |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.48: CONFLUENCE RADAR SCORE ENGINE (0 - 100% SETUP VALIDATOR) |
//+------------------------------------------------------------------+
double CalculateConfluenceScore(string dir, string &gradeOut, string &reasonsOut)
{
   if(dir != "BUY" && dir != "SELL")
   {
      gradeOut = "STANDBY";
      reasonsOut = "0% [STANDBY]";
      return 0.0;
   }
   ...
   // 1. Multi-TF CMP Alignment (Max 30%)
   maxScore += 30.0;
   if(g_masterDir == dir) score += 10.0;
   if(g_scalpMasterDir == dir) score += 10.0;
   if(g_scalpEntryDir == dir) score += 10.0;

   // 2. Bookmap Order Flow & Footprint Delta (Max 25%)
   if(g_bookmapOnline)
   {
      maxScore += 25.0;
      if((isBuy && g_bookmapCvd > 0) || (!isBuy && g_bookmapCvd < 0)) score += 10.0;

      double m1Delta = g_bookmapFootM1BuyVol - g_bookmapFootM1SellVol;
      if((isBuy && m1Delta > 0) || (!isBuy && m1Delta < 0)) score += 10.0;

      if((isBuy && g_bookmapPulsePct >= 55.0) || (!isBuy && g_bookmapPulsePct <= 45.0)) score += 5.0;
   }

   // 3. Suport & Resisten Location (Max 25% = 15 lokasi + 10 Bookmap Wall)
   // Dadang Rule: JANGAN PERNAH BUY DI PUCUK RESISTEN KECUALI SUDAH JEBOL!
   // DI RESISTEN = PRIORITAS SELL (REJEKSI) / DI SUPORT = PRIORITAS BUY (REBOUND)
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   maxScore += 15.0;
   
   if(isBuy)
   {
      // Rebound di Area Suport Bawah -> Bonus Penuh
      if(g_demandRoadCount > 0 && price >= g_demandRoadLo[0] - 2.0 && price <= g_demandRoadHi[0] + 6.0)
         score += 15.0;
      
      // Jebol Resisten Atas (Breakout Valid) -> Bonus Breakout
      else if(g_supplyRoadCount > 0 && price > g_supplyRoadHi[0] && g_sdBrkState_S == "BREAK_CONFIRMED")
         score += 15.0;
      
      // BAHAYA: Harga persis di dalam Resisten tapi belum jebol -> Penalti keras (jebakan beli di pucuk!)
      else if(g_supplyRoadCount > 0 && price >= g_supplyRoadLo[0] - 1.5 && price <= g_supplyRoadHi[0] + 1.0)
         score = MathMax(0.0, score - 20.0);
   }
   else // SELL
   {
      // Rejeksi di Area Resisten Atas -> Bonus Penuh
      if(g_supplyRoadCount > 0 && price >= g_supplyRoadLo[0] - 6.0 && price <= g_supplyRoadHi[0] + 2.0)
         score += 15.0;
      
      // Jebol Suport Bawah (Breakdown Valid) -> Bonus Breakdown
      else if(g_demandRoadCount > 0 && price < g_demandRoadLo[0] && g_sdBrkState_D == "BREAK_CONFIRMED")
         score += 15.0;
      
      // BAHAYA: Harga persis di dalam Suport tapi belum jebol -> Penalti keras (jebakan jual di lantai!)
      else if(g_demandRoadCount > 0 && price >= g_demandRoadLo[0] - 1.0 && price <= g_demandRoadHi[0] + 1.5)
         score = MathMax(0.0, score - 20.0);
   }
   ...
   {
      maxScore += 10.0;
      if(isBuy) { if(g_bookmapBidSz[0] >= 10.0) score += 10.0; }
      else      { if(g_bookmapAskSz[0] >= 10.0) score += 10.0; }
   }

   // 4. Runway Clearance / Barrier Check (Max 20%)
   maxScore += 20.0;
   string barReason = "";
   if(!BarrierVetoes(isBuy, barReason)) score += 20.0;

   double pct = (maxScore > 0.0) ? (100.0 * score / maxScore) : 0.0;
   pct = MathMin(100.0, MathMax(0.0, pct));

   string bmNote = g_bookmapOnline ? "" : " (no-BM)";
   if(pct >= 85.0)      gradeOut = "A+ (FULL GAS)" + bmNote;
   else if(pct >= 70.0) gradeOut = "A (BAGUS)" + bmNote;
   else if(pct >= 50.0) gradeOut = "B (WASPADA)" + bmNote;
   else                 gradeOut = "C (HATI-HATI)" + bmNote;

   reasonsOut = StringFormat("%.0f%% [%s]", pct, gradeOut);
   return pct;
}

string ConfluenceRadarText(color &clrOut)
{
   string tradeDir = (g_scalpEntryDir != "WAIT") ? g_scalpEntryDir : g_scalpMasterDir;
   if(tradeDir == "WAIT") tradeDir = g_masterDir;
   if(tradeDir == "WAIT") { clrOut = PNL_LABEL; return "STANDBY (0%)"; }
   
   string grade = "", reasons = "";
   double score = CalculateConfluenceScore(tradeDir, grade, reasons);
   
   if(score >= 85.0)      clrOut = (tradeDir == "BUY") ? C'0,240,140' : C'255,80,80'; // Emerald / Rose
   else if(score >= 70.0) clrOut = (tradeDir == "BUY") ? C'80,220,100' : C'255,110,110'; // Lime / Red
   else if(score >= 50.0) clrOut = C'255,190,0';  // Gold / Yellow
   else                   clrOut = PNL_LABEL;
   
   return StringFormat("%.0f%% %s [%s]", score, tradeDir, grade);
}


//+------------------------------------------------------------------+
//| v53.49: TOP-CENTER CONFLUENCE RADAR HUD BADGE (STANDOUT BANNER)   |
//+------------------------------------------------------------------+
void PnlFillRoundedRect_Canvas(CCanvas &cv, int x1, int y1, int x2, int y2, int r, uint argb)
{
   cv.FillRectangle(x1 + r, y1, x2 - r, y2, argb);
   cv.FillRectangle(x1, y1 + r, x2, y2 - r, argb);
   cv.FillCircle(x1 + r, y1 + r, r, argb);
   cv.FillCircle(x2 - r, y1 + r, r, argb);
   cv.FillCircle(x1 + r, y2 - r, r, argb);
   cv.FillCircle(x2 - r, y2 - r, r, argb);
}

void UpdateTopCenterConfluenceHUD()
{
   // Bersihkan semua objek teks/kotak legacy agar chart bersih 100%
   ObjectsDeleteAll(0, "DD_HUD_RADAR_");
   ObjectsDeleteAll(0, "DD_RADAR_");

   // Evaluasi kedua arah (BUY vs SELL) berdasarkan data Order Flow & Lokasi Suport/Resisten
   string gradeBuy = "", reasonsBuy = "";
   double scoreBuy = CalculateConfluenceScore("BUY", gradeBuy, reasonsBuy);

   string gradeSell = "", reasonsSell = "";
   double scoreSell = CalculateConfluenceScore("SELL", gradeSell, reasonsSell);

   // Tentukan setup yang paling valid & dominan saat ini
   string bestDir = "BUY";
   double bestScore = scoreBuy;
   string bestGrade = gradeBuy;
   ...
   else if(scoreBuy == scoreSell && scoreBuy < 50.0)
   {
      bestDir   = "WAIT";
      bestScore = scoreBuy;
      bestGrade = "STANDBY";
   ...
   // Styling Dinamis Berdasarkan Skor & Arah
   color bgFillClr = C'12,16,24';
   color borderClr = C'70,80,95';
   ...
   if(bestScore >= 85.0)
   {
      icon = "🔥";
      borderClr = clrGold;
      textClr   = (bestDir == "BUY") ? C'0,255,140' : C'255,90,90';
   }
   else if(bestScore >= 70.0)
   {
      icon = "✅";
      borderClr = (bestDir == "BUY") ? C'0,220,120' : C'245,60,60';
      textClr   = (bestDir == "BUY") ? C'180,255,200' : C'255,190,190';
   }
   else if(bestScore >= 50.0)
   {
      icon = "⚠️";
      borderClr = C'255,190,0'; // Gold Warning
      textClr   = C'255,230,100';
   }
   else
   {
      icon = "⛔";
      borderClr = C'80,90,100';
      textClr   = clrSilver;
   }

   int chartWidth = (int)ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
   int hudW = 600;  // Lebih Lebar & Mewah
   int hudH = 46;   // Lebih Tinggi & Tebal
   int posX = (chartWidth / 2) - (hudW / 2);
   if(posX < 380) posX = 380; // Jangan tabrak panel kiri (PNL_W = 350)
   int posY = 15;

   string canvasName = "DD_HUD_RADAR_CANVAS";
   if(!g_hudCanvasCreated || ObjectFind(0, canvasName) < 0 || g_hudCanvas.Width() != hudW || g_hudCanvas.Height() != hudH)
   {
      g_hudCanvas.Destroy();
      g_hudCanvas.CreateBitmapLabel(canvasName, posX, posY, hudW, hudH, COLOR_FORMAT_ARGB_NORMALIZE);
      ObjectSetInteger(0, canvasName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, canvasName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, canvasName, OBJPROP_HIDDEN, true);
      g_hudCanvasCreated = true;
   }
   else
   {
      ObjectSetInteger(0, canvasName, OBJPROP_XDISTANCE, posX);
      ObjectSetInteger(0, canvasName, OBJPROP_YDISTANCE, posY);
   }

   // Render background solid & rounded rectangle ke dalam CCanvas bitmap
   g_hudCanvas.Erase(0);
   
   // Border Tebal Bercahaya (2px border) & Background Dark Obsidian Solid
   PnlFillRoundedRect_Canvas(g_hudCanvas, 0, 0, hudW - 1, hudH - 1, 10, ColorToARGB(borderClr, 255));
   PnlFillRoundedRect_Canvas(g_hudCanvas, 2, 2, hudW - 3, hudH - 3, 8, ColorToARGB(bgFillClr, 255));

   // Text Presisi Tepat di Tengah dengan Ukuran Besar & Font Gagah
   string hudText;
   if(bestDir == "WAIT")
      hudText = StringFormat("%s RADAR KONFLUENSI: %.0f%% STANDBY", icon, bestScore);
   else
      hudText = StringFormat("%s RADAR KONFLUENSI: %.0f%% %s [%s]", icon, bestScore, bestDir, bestGrade);

   g_hudCanvas.FontSet("Segoe UI", 16, PNL_WEIGHT_BOLD);
   // Fake-bold outline shadow for high-contrast visibility
   uint argbOutline = ColorToARGB(C'0,0,0', 255);
   uint argbText    = ColorToARGB(textClr, 255);
   g_hudCanvas.TextOut(hudW / 2 - 1, hudH / 2, hudText, argbOutline, TA_CENTER | TA_VCENTER);
   g_hudCanvas.TextOut(hudW / 2 + 1, hudH / 2, hudText, argbOutline, TA_CENTER | TA_VCENTER);
   g_hudCanvas.TextOut(hudW / 2, hudH / 2 - 1, hudText, argbOutline, TA_CENTER | TA_VCENTER);
   g_hudCanvas.TextOut(hudW / 2, hudH / 2 + 1, hudText, argbOutline, TA_CENTER | TA_VCENTER);
   g_hudCanvas.TextOut(hudW / 2, hudH / 2, hudText, argbText, TA_CENTER | TA_VCENTER);
...
string GetTfCountdownStr(ENUM_TIMEFRAMES tf)
{
   ...
   int s = remSec % 60;
   if(h > 0) return StringFormat("%02dh %02dm %02ds", h, m, s);
   return StringFormat("%02dm %02ds", m, s);
}

// Color Lerp for Smooth Gradient Semicircle
uint LerpARGB(uint c1, uint c2, double t)
{
   t = MathMax(0.0, MathMin(1.0, t));
   uchar a1 = (uchar)((c1 >> 24) & 0xFF);
   uchar r1 = (uchar)((c1 >> 16) & 0xFF);
   uchar g1 = (uchar)((c1 >> 8) & 0xFF);
   uchar b1 = (uchar)(c1 & 0xFF);

   uchar a2 = (uchar)((c2 >> 24) & 0xFF);
   uchar r2 = (uchar)((c2 >> 16) & 0xFF);
   uchar g2 = (uchar)((c2 >> 8) & 0xFF);
   uchar b2 = (uchar)(c2 & 0xFF);
...
// Rounded Card with Thick Border & Dark Glass Backdrop
void DrawGlassCard(int x1, int y1, int x2, int y2, int r, uint bgClr, uint borderClr, int borderThick = 1)
{
   PnlFillRoundedRect(x1, y1, x2, y2, r, borderClr);
...
void DrawPillButton(int x1, int y1, int x2, int y2, int r, string txt, uint bgClr, uint txtClr, uint borderClr, int fontSz = 10, bool isBold = true)
{
   DrawGlassCard(x1, y1, x2, y2, r, bgClr, borderClr, 1);
   int cx = (x1 + x2) / 2;
   int cy = (y1 + y2) / 2 - (fontSz / 2) - 1;
   g_panelCanvas.FontSet("Tahoma", fontSz, isBold ? 700 : 400);
   g_panelCanvas.TextOut(cx, cy, txt, txtClr, TA_CENTER | TA_TOP);
}
...
void DrawSpeedometerGauge(int cx, int cy, int radius, double cvdVal, double pulseVal)
{
   ...
   for(double a = 180.0; a <= 360.0; a += 2.5)
   {
      double rad = a * M_PI / 180.0;
      int px = cx + (int)MathRound(arcR * MathCos(rad));
      int py = cy + (int)MathRound(arcR * MathSin(rad));
      ...
      g_panelCanvas.FillCircle(px, py, 4, c);
   }
   ...
   int tipx = cx + (int)MathRound((arcR - 12) * MathCos(nrad));
   int tipy = cy + (int)MathRound((arcR - 12) * MathSin(nrad));
   ...
   g_panelCanvas.Line((int)(cx - nx), (int)(cy - ny), (int)(tipx - nx), (int)(tipy - ny), needleClr);
   g_panelCanvas.FillCircle(cx, cy, 7, needleClr);
   g_panelCanvas.FillCircle(cx, cy, 3, 0xFF0E131A);

   // 3. Glowing Big Values below Gauge (Large HD Fonts)
   uint cvdClr = (cvdVal >= 0) ? 0xFF00FF8C : 0xFFFF4862;
   g_panelCanvas.FontSet("Tahoma", 16, 700);
   g_panelCanvas.TextOut(cx - 65, cy + 12, StringFormat("%+.1f", cvdVal), cvdClr, TA_CENTER | TA_TOP);
   g_panelCanvas.FontSet("Tahoma", 10, 700);
   g_panelCanvas.TextOut(cx - 65, cy + 34, "CVD FLOW", 0xFF8E9EB5, TA_CENTER | TA_TOP);

   g_panelCanvas.FontSet("Tahoma", 16, 700);
   g_panelCanvas.TextOut(cx + 65, cy + 12, StringFormat("%.1f%%", pulseVal), 0xFF00E5FF, TA_CENTER | TA_TOP);
   g_panelCanvas.FontSet("Tahoma", 10, 700);
   g_panelCanvas.TextOut(cx + 65, cy + 34, "PULSE PRESSURE", 0xFF8E9EB5, TA_CENTER | TA_TOP);
}
...
void UpdatePanel()
{
   ...
   int curH = g_cockpitMinimized ? 42 : g_cockpitH;
   ...
   if(ObjectFind(0, "SULTAN_MASTER_COCKPIT_V5") < 0 || g_panelCanvas.Width() != g_cockpitW || g_panelCanvas.Height() != curH)
   {
      ...
      ObjectSetInteger(0, "SULTAN_MASTER_COCKPIT_V5", OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, "SULTAN_MASTER_COCKPIT_V5", OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, "SULTAN_MASTER_COCKPIT_V5", OBJPROP_HIDDEN, true);
      ObjectSetInteger(0, "SULTAN_MASTER_COCKPIT_V5", OBJPROP_ZORDER, 1);
   }
   else
   {
   ...
   // 2. Master Outer Glass Window with BOLD Gold Neon Rim (Thick 2px, Radius: 12)
   DrawGlassCard(0, 0, g_cockpitW, curH, 12, 0xFF0D1219, 0xFFFFC745, 2);

   // 3. Header Drag Bar with DADANG TRADEMARK SIGNATURE (Radius: 8)
   DrawGlassCard(4, 4, g_cockpitW - 4, 38, 8, 0xFF17202C, 0xFF2A3648, 1);
   
   g_panelCanvas.FontSet("Tahoma", 10, 700);
   g_panelCanvas.TextOut(16, 11, "✥ DRAG", 0xFF8E9EB5, TA_LEFT | TA_TOP);
   
   g_panelCanvas.FontSet("Tahoma", 13, 700);
   g_panelCanvas.TextOut(90, 9, "👑 DADANG MASTER COCKPIT V5  |  CHAIN REACTION PRO", 0xFFFFC745, TA_LEFT | TA_TOP);
   
   g_panelCanvas.FontSet("Tahoma", 9, 700);
   g_panelCanvas.TextOut(505, 12, "ZERO-CLIENT", 0xFF00E5FF, TA_LEFT | TA_TOP);
   
   uint liveClr = g_bookmapOnline ? 0xFF00FF8C : 0xFFFF4862;
   g_panelCanvas.FillCircle(620, 19, 5, liveClr);
   g_panelCanvas.FontSet("Tahoma", 11, 700);
   g_panelCanvas.TextOut(632, 10, g_bookmapOnline ? "BOOKMAP LIVE" : "OFFLINE", liveClr, TA_LEFT | TA_TOP);
   
   DrawPillButton(742, 7, 766, 33, 4, g_cockpitMinimized ? "[+]" : "[-]", 0xFF222D3D, 0xFF8E9EB5, 0xFF35445A, 10, true);
   DrawPillButton(770, 7, 794, 33, 4, "✕", 0xFF3D151B, 0xFFFF4862, 0xFF7A202A, 10, true);
   ...
   // 4. Top Navigation Tabs (5 Rich Pill Tabs with Radius: 6)
   int tabX[5] = {12, 166, 324, 480, 630};
   int tabW[5] = {150, 154, 152, 146, 158};
   ...
   for(int t = 0; t < 5; t++)
   {
      ...
      DrawPillButton(tabX[t], 44, tabX[t] + tabW[t], 78, 6, tabLabels[t], tBg, tTxt, tBrd, 10, active);
   }

   // 5. Multi-TF Candle Close Countdown Ribbon (Radius: 6)
   DrawGlassCard(12, 84, g_cockpitW - 12, 114, 6, 0xFF121924, 0xFF263345, 1);
   ...
   g_panelCanvas.FontSet("Tahoma", 10, 700);
   g_panelCanvas.TextOut(22, 91, cdRibbon, 0xFF00E5FF, TA_LEFT | TA_TOP);

   datetime dummyT;
   string d1Dir  = ReadCMP(g_hExportD1, dummyT);
   string h4Dir  = ReadCMP(g_hMaster, dummyT);
   string h1Dir  = ReadCMP(g_hExportH1, dummyT);
   string m30Dir = ReadCMP(g_hScalpMaster, dummyT);
   string m15Dir = ReadCMP(g_hExportM15, dummyT);
   ...
   if(g_activeCockpitTab == TAB_ORDER_FLOW_INTEL)
   {
      ...
      DrawGlassCard(12, 122, 370, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 132, "MULTI-TF CMP REGIME & MOMENTUM", 0xFFFFC745, TA_LEFT | TA_TOP);
      ...
      for(int i = 0; i < 6; i++)
      {
         ...
         g_panelCanvas.FontSet("Tahoma", 10, 700);
         g_panelCanvas.TextOut(24, tfY, tfNames[i], 0xFFFFFFFF, TA_LEFT | TA_TOP);
         
         DrawPillButton(146, tfY - 2, 204, tfY + 20, 4, tfDirs[i], dirBg, dirClr, dirClr, 10, true);
         DrawVisualBar(212, tfY + 4, 45, 10, tfMoms[i], dirClr, 0xFF0E141D);

         g_panelCanvas.FontSet("Tahoma", 10, 700);
         g_panelCanvas.TextOut(265, tfY, "⏳ " + tfTimes[i], 0xFF00E5FF, TA_LEFT | TA_TOP);
         
         tfY += 32;
      }

      // Card 2: Market Pressure & Pulse Speedometer Gauge
      DrawGlassCard(12, 374, 370, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 384, "MARKET PRESSURE & CVD GAUGE", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      DrawSpeedometerGauge(191, 474, 70, g_bookmapCvd, g_bookmapPulsePct);
      
      double bullPct = MathMax(10.0, MathMin(90.0, 50.0 + (g_bookmapCvd / 2.0)));
      g_panelCanvas.FontSet("Tahoma", 9, 700);
      g_panelCanvas.TextOut(24, 532, "BUY ABSORPTION", 0xFF00FF8C, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(358, 532, "SELL PRESSURE", 0xFFFF4862, TA_RIGHT | TA_TOP);
      DrawVisualBar(24, 550, 334, 10, bullPct, 0xFF00FF8C, 0xFF351216);

      string narrVerdict = StringFormat("Verdict: %s", g_bmNarrVerdict);
      g_panelCanvas.FontSet("Tahoma", 10, 700);
      g_panelCanvas.TextOut(24, 574, narrVerdict, 0xFFFFC745, TA_LEFT | TA_TOP);

      // Card 3: CME Wall Intelligence with Visual Liquidity Depth
      DrawGlassCard(380, 122, g_cockpitW - 12, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 132, "BOOKMAP CME WALL & LIQUIDITY DEPTH", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int wy = 162;
      double bidWall = g_bookmapBidPx[0];
      double askWall = g_bookmapAskPx[0];
      
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, wy, "Nearest Bid Wall (Suport):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, wy, StringFormat("%.2f (%0.fL)", bidWall, g_bookmapBidSz[0]), 0xFF00FF8C, TA_LEFT | TA_TOP);
      DrawVisualBar(700, wy + 2, 75, 10, MathMin(100.0, g_bookmapBidSz[0] * 5.0), 0xFF00FF8C, 0xFF0E2E1D);
      wy += 30;
      
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, wy, "Nearest Ask Wall (Resisten):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, wy, StringFormat("%.2f (%0.fL)", askWall, g_bookmapAskSz[0]), 0xFFFF4862, TA_LEFT | TA_TOP);
      DrawVisualBar(700, wy + 2, 75, 10, MathMin(100.0, g_bookmapAskSz[0] * 5.0), 0xFFFF4862, 0xFF351216);
      wy += 30;
      
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, wy, "Point of Control (VPOC):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, wy, StringFormat("%.2f (%0.fL)", g_bookmapPocPrice, g_bookmapPocVolume), 0xFFFFC745, TA_LEFT | TA_TOP);
      DrawVisualBar(700, wy + 2, 75, 10, MathMin(100.0, g_bookmapPocVolume * 2.0), 0xFFFFC745, 0xFF2A2012);
      wy += 30;
      
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, wy, "Value Area (VAH / VAL):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, wy, StringFormat("%.2f / %.2f", g_bookmapVah, g_bookmapVal), 0xFF00E5FF, TA_LEFT | TA_TOP);
      wy += 32;

      DrawPillButton(395, wy, 660, wy + 28, 6, "🧲 LTHL MAGNET ACCUMULATION ACTIVE", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 10, true);

      // Card 4: Sierra Chart Suite & Dadang Institutional Narrative
      DrawGlassCard(380, 374, g_cockpitW - 12, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 384, "SIERRA CHART SUITE & INSTITUTIONAL NARRATIVE", 0xFFFFC745, TA_LEFT | TA_TOP);
      ...
      if(g_scWhaleLots >= 50.0)
      {
         ...
         DrawPillButton(395, sy, 675, sy + 28, 6, whTxt, whBg, whClr, whClr, 10, true);
      }
      else
      {
         g_panelCanvas.FontSet("Tahoma", 10, 400);
         g_panelCanvas.TextOut(395, sy + 4, "Whale Activity: Standby (< 50 Lot Threshold)", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      }
      sy += 40;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, sy, "Unfinished Auction:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, sy, g_scUnfinishedFound ? "Poor High (Unfinished)" : "Auction Balanced", g_scUnfinishedFound ? 0xFFFFC745 : 0xFF8E9EB5, TA_LEFT | TA_TOP);
      sy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, sy, "S&D Genesis Status:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(590, sy, "👑 GENESIS H4 PRIMARY", 0xFFFFC745, TA_LEFT | TA_TOP);
      sy += 30;
      ...
      g_panelCanvas.FontSet("Tahoma", 9, 400);
      g_panelCanvas.TextOut(395, sy + 4, narr, 0xFF8E9EB5, TA_LEFT | TA_TOP);
   }
   ...
   else if(g_activeCockpitTab == TAB_BOOKMAP_SUITE)
   {
      DrawGlassCard(12, 122, 370, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 132, "🌪️ DELTA MOMENTUM & VELOCITY RADAR", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int dmy = 168;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, dmy, "CVD Velocity (Rate of Change):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(255, dmy, StringFormat("%+.2f/sec", g_bookmapCvd / 10.0), 0xFF00FF8C, TA_LEFT | TA_TOP);
      dmy += 28;
      
      DrawVisualBar(24, dmy, 320, 10, MathMin(100.0, MathAbs(g_bookmapCvd) * 2.0), 0xFF00FF8C, 0xFF141C27);
      dmy += 26;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, dmy, "Delta Acceleration:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(255, dmy, "STRONG EXPANSION", 0xFF00E5FF, TA_LEFT | TA_TOP);
      dmy += 28;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, dmy, "Exhaustion Risk Index:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(255, dmy, "LOW (Healthy Flow)", 0xFF00FF8C, TA_LEFT | TA_TOP);
      dmy += 34;

      DrawPillButton(24, dmy, 344, dmy + 28, 6, "⚡ MOMENTUM AGGRESSIVE ACTIVE", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);

      // Card 2: 🧊 Iceberg Order Accumulation Hub
      DrawGlassCard(12, 374, 370, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 384, "🧊 ICEBERG HIDDEN LIQUIDITY RADAR", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int icy = 420;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, icy, "Hidden Iceberg Orders Detected:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(270, icy, "3 Active Nodes", 0xFFFFC745, TA_LEFT | TA_TOP);
      icy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, icy, "Estimated Hidden Volume:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(270, icy, "185.0 Lots CME", 0xFF00E5FF, TA_LEFT | TA_TOP);
      icy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, icy, "Dominant Iceberg Side:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(230, icy - 2, 340, icy + 22, 4, "BUY ICEBERG", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      icy += 34;

      DrawVisualBar(24, icy, 320, 10, 75.0, 0xFF00E5FF, 0xFF141C27);

      // Card 3: 🧹 Liquidity Sweep & Stop Hunts Radar
      DrawGlassCard(380, 122, g_cockpitW - 12, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 132, "🧹 LIQUIDITY SWEEP & STOP HUNT DETECTOR", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int swy = 168;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, swy, "Recent Sweep Activity:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(580, swy - 2, 760, swy + 22, 4, "BUY-SIDE SWEEP CLEAR", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      swy += 32;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, swy, "Swept Liquidity Volume:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, swy, "94.5 Lots Rested", 0xFFFFC745, TA_LEFT | TA_TOP);
      swy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, swy, "Trap Formation Status:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, swy, "REVERSAL PENDING", 0xFF00E5FF, TA_LEFT | TA_TOP);
      swy += 34;

      DrawVisualBar(395, swy, 365, 10, 85.0, 0xFFFFC745, 0xFF141C27);
      swy += 28;

      DrawPillButton(395, swy, 720, swy + 28, 6, "🧲 LIQUIDITY POOL CLEARED -> REVERSAL BIAS", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 10, true);

      // Card 4: 🛡️ Passive Absorption vs Aggressive Market Orders
      DrawGlassCard(380, 374, g_cockpitW - 12, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 384, "🛡️ PASSIVE ABSORPTION VS MARKET EXHAUSTION", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int aby = 420;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, aby, "Absorption State:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(580, aby - 2, 760, aby + 22, 4, "HEAVY BUY ABSORPTION", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      aby += 32;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, aby, "Seller Aggression Depletion:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, aby, "82% Depleted", 0xFF00FF8C, TA_LEFT | TA_TOP);
      aby += 30;

      DrawVisualBar(395, aby, 365, 10, 82.0, 0xFF00FF8C, 0xFF351216);
      aby += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, aby, "Institutional Tape Verdict:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, aby, "ABSORBED -> PUSH UP", 0xFF00FF8C, TA_LEFT | TA_TOP);
   }
   ...
   else if(g_activeCockpitTab == TAB_FOOTPRINT_SIERRA)
   {
      DrawGlassCard(12, 122, 370, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 132, "👣 DIAGONAL FOOTPRINT IMBALANCE", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int fpy = 168;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, fpy, "Stacked Imbalances (300%):", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(235, fpy - 2, 345, fpy + 22, 4, "3X BUY STACKED", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      fpy += 32;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, fpy, "Imbalance Price Level:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(235, fpy, StringFormat("%.2f", g_bookmapBidPx[0]), 0xFF00FF8C, TA_LEFT | TA_TOP);
      fpy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, fpy, "Dominant Imbalance Ratio:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(235, fpy, "4.2 : 1.0 (Heavy Bid)", 0xFFFFC745, TA_LEFT | TA_TOP);
      fpy += 34;

      DrawVisualBar(24, fpy, 320, 10, 80.0, 0xFF00FF8C, 0xFF351216);
      fpy += 28;

      DrawPillButton(24, fpy, 345, fpy + 28, 6, "🟢 STRONG INTRADAY BID SUPPORT", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);

      // Card 2: Delta Divergence Radar
      DrawGlassCard(12, 374, 370, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(24, 384, "⚡ DELTA DIVERGENCE RADAR", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int dvy = 420;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, dvy, "Divergence Detection:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(210, dvy - 2, 345, dvy + 22, 4, "BULLISH DELTA DIV", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      dvy += 34;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(24, dvy, "Price vs Delta Relationship:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 10, 700);
      g_panelCanvas.TextOut(24, dvy + 24, "Price Lower Low  <-->  Delta Higher High", 0xFFFFC745, TA_LEFT | TA_TOP);
      dvy += 52;

      DrawPillButton(24, dvy, 345, dvy + 28, 6, "🎯 HIGH PROBABILITY REVERSAL ALERT", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 10, true);

      // Card 3: Market Profile Shape
      DrawGlassCard(380, 122, g_cockpitW - 12, 366, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 132, "📈 MARKET PROFILE STRUCTURE SHAPE", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int mpy = 168;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, mpy, "Detected Profile Type:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      DrawPillButton(580, mpy - 2, 760, mpy + 22, 4, "P-SHAPE (Short Covering)", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 10, true);
      mpy += 32;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, mpy, "Value Area Migration:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, mpy, "SHIFTING HIGHER", 0xFF00FF8C, TA_LEFT | TA_TOP);
      mpy += 30;

      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, mpy, "Auction Balance Status:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(580, mpy, "IMBALANCED (Trending)", 0xFF00E5FF, TA_LEFT | TA_TOP);
      mpy += 34;

      DrawPillButton(395, mpy, 720, mpy + 28, 6, "👑 PROFILE BIAS: BULLISH EXPANSION", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);

      // Card 4: Multi-Bar Cumulative Delta Histogram
      DrawGlassCard(380, 374, g_cockpitW - 12, 604, 8, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(395, 384, "📊 MULTI-BAR CUMULATIVE DELTA HISTOGRAM", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int cdy = 420;
      g_panelCanvas.FontSet("Tahoma", 10, 400);
      g_panelCanvas.TextOut(395, cdy, "5-Bar Delta Accumulation:", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 12, 700);
      g_panelCanvas.TextOut(590, cdy, "+420.5 Lots", 0xFF00FF8C, TA_LEFT | TA_TOP);
      cdy += 32;

      int barX = 420;
      double barH[5] = {30, 45, 25, 60, 50};
      for(int b = 0; b < 5; b++)
      {
         PnlFillRoundedRect(barX, cdy + (int)(60 - barH[b]), barX + 42, cdy + 60, 2, 0xFF00FF8C);
         g_panelCanvas.FontSet("Tahoma", 9, 700);
         g_panelCanvas.TextOut(barX + 21, cdy + 65, StringFormat("B%d", 5 - b), 0xFF8E9EB5, TA_CENTER | TA_TOP);
         barX += 65;
      }
   }
   ...
   {
      DrawGlassCard(12, 122, g_cockpitW - 12, 604, 10, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 13, 700);
      g_panelCanvas.TextOut(28, 134, "🧱 CME ORDER FLOW WALL LADDER & S&D GENESIS ZONES", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int ly = 170;
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(28, ly, "LEVEL TYPE", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(170, ly, "PRICE LEVEL", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(320, ly, "ORDER VOLUME / STRENGTH", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(560, ly, "ZONE SCORE", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(680, ly, "STATUS", 0xFF8E9EB5, TA_LEFT | TA_TOP);
      ly += 28;

      g_panelCanvas.Line(28, ly, g_cockpitW - 28, ly, 0xFF263345);
      ly += 12;

      // 1. Ask Wall #1
      DrawPillButton(28, ly - 2, 150, ly + 22, 4, "ASK WALL #1", 0xFF351216, 0xFFFF4862, 0xFFFF4862, 10, true);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(170, ly, StringFormat("%.2f", g_bookmapAskPx[0]), 0xFFFF4862, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 400);
      g_panelCanvas.TextOut(320, ly, StringFormat("%.0f Lots CME Limit", g_bookmapAskSz[0]), 0xFFFFFFFF, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(560, ly, "95 pts", 0xFF00E5FF, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(680, ly, "RESISTEN", 0xFFFF4862, TA_LEFT | TA_TOP);
      ly += 34;

      // 2. VPOC Magnet
      DrawPillButton(28, ly - 2, 150, ly + 22, 4, "VPOC MAGNET", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 10, true);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(170, ly, StringFormat("%.2f", g_bookmapPocPrice), 0xFFFFC745, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 400);
      g_panelCanvas.TextOut(320, ly, StringFormat("%.0f Lots High Volume", g_bookmapPocVolume), 0xFFFFFFFF, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(560, ly, "85 pts", 0xFF00E5FF, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(680, ly, "MAGNET POC", 0xFFFFC745, TA_LEFT | TA_TOP);
      ly += 34;

      // 3. Bid Wall #1
      DrawPillButton(28, ly - 2, 150, ly + 22, 4, "BID WALL #1", 0xFF0E2E1D, 0xFF00FF8C, 0xFF00FF8C, 10, true);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(170, ly, StringFormat("%.2f", g_bookmapBidPx[0]), 0xFF00FF8C, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 400);
      g_panelCanvas.TextOut(320, ly, StringFormat("%.0f Lots CME Limit", g_bookmapBidSz[0]), 0xFFFFFFFF, TA_LEFT | TA_TOP);
      g_panelCanvas.TextOut(560, ly, "90 pts", 0xFF00E5FF, TA_LEFT | TA_TOP);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(680, ly, "SUPORT", 0xFF00FF8C, TA_LEFT | TA_TOP);
      ly += 36;
      ...
      for(int i = 0; i < maxZ; i++)
      {
         ...
         DrawPillButton(28, ly - 2, 150, ly + 22, 4, "S&D " + g_zoneSide[i], sBg, sClr, sClr, 10, true);
         g_panelCanvas.FontSet("Tahoma", 11, 400);
         g_panelCanvas.TextOut(170, ly, StringFormat("%.2f - %.2f", g_zoneLo[i], g_zoneHi[i]), 0xFFFFFFFF, TA_LEFT | TA_TOP);
         g_panelCanvas.FontSet("Tahoma", 11, 700);
         g_panelCanvas.TextOut(320, ly, g_zoneStatus[i], 0xFFFFC745, TA_LEFT | TA_TOP);
         g_panelCanvas.FontSet("Tahoma", 11, 400);
         g_panelCanvas.TextOut(560, ly, StringFormat("%.0f pts", g_zoneScore[i]), 0xFF00E5FF, TA_LEFT | TA_TOP);
         g_panelCanvas.TextOut(680, ly, StringFormat("%dx Retest", g_zoneRetest[i]), 0xFF8E9EB5, TA_LEFT | TA_TOP);
         ly += 32;
      }
   }
   ...
   else if(g_activeCockpitTab == TAB_SNIPER_SULTAN)
   {
      DrawGlassCard(12, 122, g_cockpitW - 12, 604, 10, 0xFF141C27, 0xFF263345, 1);
      g_panelCanvas.FontSet("Tahoma", 13, 700);
      g_panelCanvas.TextOut(28, 134, "SNIPER RISK SIZING & PROTOCOL EXECUTION", 0xFFFFC745, TA_LEFT | TA_TOP);
      
      int ey = 162;
      DrawPillButton(28, ey, 168, ey + 30, 6, "[ Fixed Lot ]", 0xFF1D2635, 0xFF8E9EB5, 0xFF2E3D52, 10, false);
      DrawPillButton(178, ey, 418, ey + 30, 6, "% Balance (2.0% Risk)", 0xFF2A2214, 0xFFFFC745, 0xFFFFC745, 11, true);
      DrawPillButton(428, ey, 588, ey + 30, 6, "[ % Equity ]", 0xFF1D2635, 0xFF8E9EB5, 0xFF2E3D52, 10, false);
      ey += 40;

      // Risk Summary Ribbon
      DrawGlassCard(28, ey, g_cockpitW - 28, ey + 36, 6, 0xFF0E141D, 0xFF263345, 1);
      ...
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(40, ey + 9, riskSummary, 0xFFFFFFFF, TA_LEFT | TA_TOP);
      ey += 48;

      // Protocol Selector Cards
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(28, ey, "ENTRY TRIGGER MODE (DOKTRIN B4):", 0xFFFFC745, TA_LEFT | TA_TOP);
      ey += 22;
      ...
      DrawPillButton(28, ey, 388, ey + 34, 6, "⚡ FAST SNIPER (Wick Sweep CME/SND)", fBg, fTxt, fBrd, 11, true);
      ...
      DrawPillButton(398, ey, 772, ey + 34, 6, "🛡️ SAFE DISCIPLINE (Wait Body Close)", sBg, sTxt, sBrd, 11, true);
      ey += 46;
      ...
      DrawPillButton(28, ey, 388, ey + 54, 10, StringFormat("🔴 SELL SNIPER @ %.2f", bidPx), 0xFF3B1015, 0xFFFF4862, 0xFFFF4862, 13, true);
      DrawPillButton(398, ey, 772, ey + 54, 10, StringFormat("🟢 BUY SNIPER @ %.2f", askPx), 0xFF0D3020, 0xFF00FF8C, 0xFF00FF8C, 13, true);
      ey += 66;

      // ⚡ DEDICATED 1-CLICK NEWS STRADDLE TRAP
      DrawPillButton(28, ey, 772, ey + 42, 8, "⚡ 1-CLICK NEWS STRADDLE: BUY STOP + SELL STOP (+20p / SL:20p / TP:50p)", 0xFF261238, 0xFFD8B4FE, 0xFFA855F7, 11, true);
      ey += 52;

      // Sultan Actions (Close 50%, Auto-BE, Flip, Close All)
      DrawPillButton(28, ey, 208, ey + 36, 6, "✂️ CLOSE 50%", 0xFF2A2012, 0xFFFFC745, 0xFFFFC745, 11, true);
      DrawPillButton(218, ey, 398, ey + 36, 6, "🛡️ AUTO-BE LOCK", 0xFF0E2232, 0xFF00E5FF, 0xFF00E5FF, 11, true);
      DrawPillButton(408, ey, 588, ey + 36, 6, "🔄 REVERSAL FLIP", 0xFF28123A, 0xFFD8B4FE, 0xFF9333EA, 11, true);
      DrawPillButton(598, ey, 772, ey + 36, 6, "🚨 CLOSE ALL", 0xFF3B1015, 0xFFFF4862, 0xFFFF4862, 11, true);
      ey += 46;

      // Floating Account PnL
      DrawGlassCard(28, ey, g_cockpitW - 28, ey + 40, 8, 0xFF0E141D, 0xFF263345, 1);
      ...
      string accStatus = StringFormat("Balance: $%.2f  |  Equity: $%.2f  |  Floating P&L: %s$%.2f",
                                      bl, eq, flPnl >= 0 ? "+" : "", flPnl);
      g_panelCanvas.FontSet("Tahoma", 11, 700);
      g_panelCanvas.TextOut(40, ey + 10, accStatus, flPnl >= 0 ? 0xFF00FF8C : 0xFFFF4862, TA_LEFT | TA_TOP);
   }

   // 6. Royal Dadang Watermark Signature (Bottom Center)
   g_panelCanvas.FontSet("Tahoma", 10, 700);
   g_panelCanvas.TextOut(g_cockpitW / 2, curH - 18, "👑 PROPERTY OF DADANG WAHYU ONO — SINGLE SOURCE OF TRUTH — ALL RIGHTS RESERVED", 0x88E5A93C, TA_CENTER | TA_TOP);
...
void QueuePush(BarrierEntry &q[], string dir, double level, string tag = "", datetime flipTime = 0, bool isGenesis = false)
{
   ...
   if(n >= MAX_BARRIER_QUEUE)
   {
      ...
      q[n-1].flipTime = flipTime; q[n-1].isGenesis = isGenesis;   // v54.11 fix: these 2 fields were added to the struct but never assigned here - same class of bug as v53.61's retestCount leak, "Zona Bawaan H4" flag was being silently dropped every push
   }
   else
   {
      ArrayResize(q, n + 1);
      q[n].dir = dir; q[n].level = level; q[n].retestCount = 0; q[n].wasInside = false;
      q[n].flipTime = flipTime; q[n].isGenesis = isGenesis;
   }
...
bool BarrierVetoes(bool buy, string &reasonOut)
{
   ...
   // In Strategy Tester without pre-seeded barrier queues, don't hard-block initial signals
   if(MQLInfoInteger(MQL_TESTER) && (ArraySize(g_h4Queue) == 0 && ArraySize(g_m30Queue) == 0)) return false;
...
//+------------------------------------------------------------------+
//| v53.30: Dadang TradingView Masterclass:
//| D1 = Garis Horizontal Hijau (Daily SNR)
//| H4 = Garis Horizontal Merah (H4 Master SNR)
//| H1 = Garis Horizontal Gold (H1 SNR)
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.30: Dadang TradingView Masterclass:
//| D1 = Garis Horizontal Hijau (Daily SNR)
...
//+------------------------------------------------------------------+
//| v53.30: Dadang TradingView Masterclass:
//| D1 = Garis Solid Tebal Hijau Neon (Daily SNR)
//| H4 = Garis Solid Tebal Merah Neon (H4 Master SNR)
//| H1 = Garis Solid Tebal Gold (H1 SNR)
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.30: Dadang TradingView Masterclass:
//| Gambar SEMUA Garis Keramat Pucuk Lancip D1 (Hijau), H4 (Merah), H1 (Gold)
//| tanpa dihapus walau sudah ditembus (SNR Role-Reversal)!
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.30: Dadang - "gimana gw tau itu area kuat?"
//| Tampilkan Skor Kekuatan & Total Lot Bookmap LANGSUNG DI UJUNG LABEL GARIS!
//+------------------------------------------------------------------+
void DrawTfCmpLevels(int handle, string tfTag, color clr, int lineWidth, int maxLevels=6)
{
   if(!g_toggleCmp)
   {
      ObjectsDeleteAll(0, "DD_D1_LINE_"); ObjectsDeleteAll(0, "DD_D1_TXT_");
      ObjectsDeleteAll(0, "DD_H4_LINE_"); ObjectsDeleteAll(0, "DD_H4_TXT_");
      ObjectsDeleteAll(0, "DD_H1_LINE_"); ObjectsDeleteAll(0, "DD_H1_TXT_");
      ObjectsDeleteAll(0, "DD_M30_LINE_");ObjectsDeleteAll(0, "DD_M30_TXT_");
      return;
   }
   if(handle == INVALID_HANDLE) return;
   int n = 150;
   double lvlBuf[], dirBuf[];
   ...
   for(int i = 0; i < n && drawnCnt < maxLevels; i++)
   {
      double lvl = lvlBuf[i];
      if(lvl <= 0) continue;

      bool duplicate = false;
      for(int k = 0; k < drawnCnt; k++)
      {
         if(MathAbs(drawnLevels[k] - lvl) < 1.00) { duplicate = true; break; }
      }
      if(duplicate) continue;

      ArrayResize(drawnLevels, drawnCnt + 1);
      drawnLevels[drawnCnt] = lvl;
      drawnCnt++;

      string dir = (dirBuf[i] > 0.5) ? "BUY" : "SELL";
      string hlineName = StringFormat("DD_%s_LINE_%d", tfTag, drawnCnt);
      string txtName   = StringFormat("DD_%s_TXT_%d", tfTag, drawnCnt);

      // Cek apakah ada antrean Uang Fisik Bookmap di sekitar level ini (+- 2.5 USD)
      double lotNear = 0;
      double scoreNear = 0;
      int retestNear = 0;
      for(int z = 0; z < g_zoneCount; z++)
      {
         if(g_zoneStatus[z] == "BROKEN") continue;
         if(lvl >= g_zoneLo[z] - 2.5 && lvl <= g_zoneHi[z] + 2.5)
         {
            lotNear += g_zoneTotalLot[z];
            scoreNear = MathMax(scoreNear, g_zoneScore[z]);
            retestNear = MathMax(retestNear, g_zoneRetest[z]);
         }
      }

      string strengthTag = (scoreNear >= 70.0 || lotNear >= 250.0) ? "KUAT" : (scoreNear >= 40.0 ? "SEDANG" : "BIASA");
      string bookmapInfo = (lotNear > 0)
         ? StringFormat(" [🏰 %.0fL • %s %.0f]", lotNear, strengthTag, scoreNear)
         : " [STRUKTUR]";

      // 1. Gambar Garis Horizontal Solid Tebal
      if(ObjectFind(0, hlineName) < 0)
         ObjectCreate(0, hlineName, OBJ_HLINE, 0, 0, lvl);
      else
         ObjectMove(0, hlineName, 0, 0, lvl);

      ObjectSetInteger(0, hlineName, OBJPROP_COLOR, clr);
      ObjectSetInteger(0, hlineName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSetInteger(0, hlineName, OBJPROP_WIDTH, (strengthTag == "KUAT" ? 3 : 2));
      ...
      // 2. Label Teks Jelas di Ujung Kanan Garis
      datetime t = TimeCurrent() + PeriodSeconds(_Period) * 3;
      if(ObjectFind(0, txtName) < 0)
         ObjectCreate(0, txtName, OBJ_TEXT, 0, t, lvl);
      else
         ObjectMove(0, txtName, 0, t, lvl);

      string symbolIcon = (tfTag == "D1") ? "🟢 [D1]" : (tfTag == "H4") ? "🔴 [H4]" : (tfTag == "H1") ? "🟡 [H1]" : "🔹 [M30]";
      ObjectSetString(0, txtName, OBJPROP_TEXT, StringFormat("%s %.2f%s", symbolIcon, lvl, bookmapInfo));
      ObjectSetInteger(0, txtName, OBJPROP_COLOR, clr);
      ObjectSetInteger(0, txtName, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, txtName, OBJPROP_FONT, "Segoe UI Black");
...
{
   if(!ShowCMPLevels || !g_toggleCmp) return;
   // Bersihkan barier lama M30/M15/H1 agar chart tidak penuh
   ObjectsDeleteAll(0, "DD_M5Barrier");
   ObjectsDeleteAll(0, "DD_M15Barrier");
   ObjectsDeleteAll(0, "DD_M30Barrier");
   ObjectsDeleteAll(0, "DD_H1Barrier");
   ObjectsDeleteAll(0, "DD_M30_LINE_");
   ObjectsDeleteAll(0, "DD_M30_TXT_");

   // 1. D1 DAILY: Garis Hijau Solid Tebal (Maksimal 4 level pucuk Daily)
   DrawTfCmpLevels(g_hExportD1, "D1", C'0,255,128', 3, 4);

   // 2. H4 MASTER: Garis Merah Solid Tebal (Maksimal 4 level pucuk H4)
   DrawTfCmpLevels(g_hMaster, "H4", C'255,50,50', 3, 4);
}
...
{
   if(!g_bookmapOnline || g_bookmapPocPrice <= 0) { clrOut = PNL_LABEL; return "-"; }
   double offset   = SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice;
   ...
   // 1. Jika harga jebol di bawah VAL atau di bawah Daily Open & POC -> Murni BEARISH DUMP
   if(price < valMt5 - 1.0 || (price < todayPoc && price < dOpen))
   {
      clrOut = PNL_ROSE;
      return "b-SHAPE (BEAR DUMP)";
   }
   
   // 2. Jika harga melesat di atas VAH atau di atas Daily Open & POC -> Murni BULLISH RUN
   if(price > vahMt5 + 1.0 || (price > todayPoc && price > dOpen))
   {
      clrOut = PNL_EMERALD;
      return "P-SHAPE (BULL RUN)";
   }
   
   // 3. Di dalam Value Area (Range Konsolidasi)
   if(vahMt5 > valMt5 && vahMt5 > 0)
   {
      double span = vahMt5 - valMt5;
      double pocPos = (todayPoc - valMt5) / span;
      if(pocPos >= 0.65 && price >= todayPoc) { clrOut = PNL_EMERALD; return "P-SHAPE (BULL RUN)"; }
      if(pocPos <= 0.35 && price <= todayPoc) { clrOut = PNL_ROSE;    return "b-SHAPE (BEAR DUMP)"; }
      clrOut = PNL_LABEL; return "D-SHAPE (BALANCED)";
   }
   
   clrOut = (price >= todayPoc) ? PNL_EMERALD : PNL_ROSE;
   return (price >= todayPoc) ? "BULLISH POC" : "BEARISH POC";
}
...
//+------------------------------------------------------------------+
//| 🌐 NATIVE HTTP/HTTPS WEBREQUEST ENGINE (ZERO-CLIENT PIPELINE)    |
//| Created for Commander Dadang Wahyuono                           |
//| Fetches live Order Flow CSV directly from Mini PC 100.71.97.6    |
//| Runs strictly in OnTimer() - 0ms latency impact on OnTick()      |
//+------------------------------------------------------------------+
string   g_lastHttpError = "";
datetime g_lastBookmapSuccessTime = 0;
ulong    g_fetchSuccessCount = 0;
ulong    g_fetchFailCount = 0;

bool FetchBookmapDataWebRequest()
{
   if(!Diag_BookmapData || !InpShowBookmapPanel) return false;
   if(MQLInfoInteger(MQL_TESTER) || MQLInfoInteger(MQL_OPTIMIZATION)) return false;
   if(InpBookmapSource != BM_SOURCE_WEBREQUEST) return false;
   
   string cookie = NULL, headers;
   char post[], result[];
   string url = InpBookmapURL;
   
   ResetLastError();
   int res = WebRequest("GET", url, cookie, NULL, InpFetchTimeoutMs, post, 0, result, headers);
   
   if(res == -1)
   {
      int err = GetLastError();
      g_lastHttpError = StringFormat("Err %d", err);
      g_fetchFailCount++;
      if(err == 4060)
      {
         static bool warned4060 = false;
         if(!warned4060)
         {
            Print("⚠️ [WEBREQUEST 4060] URL belum diizinkan di MT5! Masukkan ", url, " ke Tools -> Options -> Expert Advisors -> Allow WebRequest");
            warned4060 = true;
         }
      }
      return false;
   ...
   if(res != 200)
   {
      g_lastHttpError = StringFormat("HTTP %d", res);
      g_fetchFailCount++;
      return false;
   }
   
   string csvText = CharArrayToString(result, 0, WHOLE_ARRAY, CP_UTF8);
   if(StringLen(csvText) < 50) 
   {
      g_fetchFailCount++;
      return false;
   }
   
   // Parse CSV lines (split header and data)
   string lines[];
   ...
   StringTrimRight(dataRow);
   StringTrimLeft(dataRow);
   
   string tokens[];
   int fieldCount = StringSplit(dataRow, ',', tokens);
   
   // 🛡️ VALIDASI KETAT: Jumlah kolom harus mencukupi (minimal 18 base fields)
   if(fieldCount < 18)
   {
      g_fetchFailCount++;
      Print("⚠️ [BOOKMAP REJECT] Payload tidak lengkap (hanya ", fieldCount, " kolom). Data ditolak!");
      return false;
   }
   
   // 🛡️ VALIDASI NUMERIK: Timestamp & Price harus bernilai positif
   double ts    = StringToDouble(tokens[0]);
   double price = StringToDouble(tokens[1]);
   if(ts <= 0 || price <= 0.0)
   {
      g_fetchFailCount++;
      Print("⚠️ [BOOKMAP REJECT] Nilai harga/timestamp cacat (ts=", ts, ", px=", price, "). Data ditolak!");
      return false;
   }
   
   // Map base order flow fields
   g_bookmapPrice          = price;
   g_bookmapCvd            = StringToDouble(tokens[2]);
   g_bookmapPulsePct       = StringToDouble(tokens[3]);
   g_bookmapAbsorption     = tokens[4];
   g_bookmapPocPrice       = StringToDouble(tokens[5]);
   g_bookmapPocVolume      = StringToDouble(tokens[6]);
   g_bookmapVolRatioBuyPct = StringToDouble(tokens[7]);
   g_bookmapBuyVolSession  = StringToDouble(tokens[8]);
   g_bookmapSellVolSession = StringToDouble(tokens[9]);
   g_bookmapVal            = StringToDouble(tokens[10]);
   g_bookmapVah            = StringToDouble(tokens[11]);
   
   g_bookmapBidIcePx       = StringToDouble(tokens[12]);
   g_bookmapBidIceSz       = StringToDouble(tokens[13]);
   g_bookmapBidIceRatio    = StringToDouble(tokens[14]);
   g_bookmapAskIcePx       = StringToDouble(tokens[15]);
   g_bookmapAskIceSz       = StringToDouble(tokens[16]);
   g_bookmapAskIceRatio    = StringToDouble(tokens[17]);
   
   int curIdx = 18;
   double bidPx[WALL_SLOTS_PER_SIDE], bidSz[WALL_SLOTS_PER_SIDE], bidAge[WALL_SLOTS_PER_SIDE];
   double askPx[WALL_SLOTS_PER_SIDE], askSz[WALL_SLOTS_PER_SIDE], askAge[WALL_SLOTS_PER_SIDE];
   
   // Parse bid wall slots (px, sz, age)
   for(int i = 0; i < WALL_SLOTS_PER_SIDE && curIdx + 2 < fieldCount; i++)
   {
      bidPx[i]  = StringToDouble(tokens[curIdx++]);
      bidSz[i]  = StringToDouble(tokens[curIdx++]);
      bidAge[i] = StringToDouble(tokens[curIdx++]);
   }
   // Parse ask wall slots (px, sz, age)
   for(int i = 0; i < WALL_SLOTS_PER_SIDE && curIdx + 2 < fieldCount; i++)
   {
      askPx[i]  = StringToDouble(tokens[curIdx++]);
      askSz[i]  = StringToDouble(tokens[curIdx++]);
      askAge[i] = StringToDouble(tokens[curIdx++]);
   }
   
   ArrayCopy(g_bookmapBidPx, bidPx);
   ArrayCopy(g_bookmapBidSz, bidSz);
   ArrayCopy(g_bookmapBidAge, bidAge);
   ArrayCopy(g_bookmapAskPx, askPx);
   ArrayCopy(g_bookmapAskSz, askSz);
   ArrayCopy(g_bookmapAskAge, askAge);
   
   // Parse sweep fields
   if(curIdx + 5 < fieldCount)
   {
      g_bmSweepSide     = tokens[curIdx++];
      g_bmSweepPrice    = StringToDouble(tokens[curIdx++]);
      g_bmSweepSize     = StringToDouble(tokens[curIdx++]);
      g_bmSweepAgeSec   = StringToDouble(tokens[curIdx++]);
      g_bmSweepStatus   = tokens[curIdx++];
      g_bmSweepSinceSec = StringToDouble(tokens[curIdx++]);
   }
   
   // Parse footprint fields
   if(curIdx + 3 < fieldCount)
   {
      g_bookmapFootBuyVol    = StringToDouble(tokens[curIdx++]);
      g_bookmapFootSellVol   = StringToDouble(tokens[curIdx++]);
      g_bookmapFootM1BuyVol  = StringToDouble(tokens[curIdx++]);
      g_bookmapFootM1SellVol = StringToDouble(tokens[curIdx++]);
   }
   
   // Parse HVN / LVN fields
   if(curIdx + 3 < fieldCount)
   {
      g_bookmapHvnPrice  = StringToDouble(tokens[curIdx++]);
      g_bookmapHvnVolume = StringToDouble(tokens[curIdx++]);
      g_bookmapLvnPrice  = StringToDouble(tokens[curIdx++]);
      g_bookmapLvnVolume = StringToDouble(tokens[curIdx++]);
   }
   
   g_bookmapOnline = true;
   g_lastBookmapSuccessTime = TimeLocal();
   g_bookmapAgeMs = 0.0;
   g_fetchSuccessCount++;
...
{
   if(!Diag_BookmapData) return; // [DIAG] Data Stream OFF -> Freeze on last known cached values
   ...
   // Mode Native WebRequest: Cek Heartbeat TimeLocal() vs last success fetch
   if(InpBookmapSource == BM_SOURCE_WEBREQUEST)
   {
      if(g_lastBookmapSuccessTime > 0 && (TimeLocal() - g_lastBookmapSuccessTime <= (int)InpBookmapStaleSec))
      {
         g_bookmapOnline = true;
      }
      else
   ...
   if(handle == INVALID_HANDLE) { return; }   // v53.31: no delete on tick
   ...
   if(FileIsEnding(handle)) { FileClose(handle); return; } // v53.31: no delete on tick
   ...
   if(MathAbs(ageSec) > InpBookmapStaleSec) { return; } // v53.31: no delete on tick - stale
   ...
   // This eliminates 100% of the on-tick flickering while keeping all trading data 100% real-time.
}
...
void BuildZonesFromBarrierCMP()
{
   ...
   for(int qi = 0; qi < 4; qi++)
   {
      ...
      for(int i = 0; i < cnt; i++)
      {
         string dir; double level; int retest; bool isGenesis;
         if(qi == 0)      { dir = g_d1Queue[i].dir;  level = g_d1Queue[i].level;  retest = g_d1Queue[i].retestCount;  isGenesis = g_d1Queue[i].isGenesis;  }
         else if(qi == 1) { dir = g_h4Queue[i].dir;  level = g_h4Queue[i].level;  retest = g_h4Queue[i].retestCount;  isGenesis = g_h4Queue[i].isGenesis;  }
         else if(qi == 2) { dir = g_h1Queue[i].dir;  level = g_h1Queue[i].level;  retest = g_h1Queue[i].retestCount;  isGenesis = g_h1Queue[i].isGenesis;  }
         else             { dir = g_m30Queue[i].dir; level = g_m30Queue[i].level; retest = g_m30Queue[i].retestCount; isGenesis = g_m30Queue[i].isGenesis; }
         if(level <= 0 || dir == "") continue;

         string side = (dir == "SELL") ? "SUPPLY" : "DEMAND";

         // v54.11: "Zona Bawaan H4" (Doktrin Bagian A4) - barrier yang lahir
         // BARENGAN sama flip H4 (isGenesis, dihitung di UpdateBarrierTracking())
         // itu lebih kuat daripada breakout TF kecil yang nyusul belakangan -
         // dikasih bonus skor +10 (dicap 100) biar kelihatan bedanya di
         // ZoneStrengthID(), bukan cuma dicap "sama kuat" kayak zona noise biasa.
         double score = baseSc[qi] + (isGenesis ? 10.0 : 0.0);
...
//+------------------------------------------------------------------+
//| v53.30: Dadang - "semakin besar TF semakin kuat SNR-nya,
//| cocokkan zona S&D Bookmap dengan Area CMP D1/H4/H1"
//+------------------------------------------------------------------+
string GetCmpConfluenceTag(double lo, double hi, double &cmpScoreBoost)
{
   cmpScoreBoost = 0.0;

   // 1. Cek D1 Queue (SNR Makro Terkuat)
   for(int i = 0; i < ArraySize(g_d1Queue); i++)
   {
      if(g_d1Queue[i].level >= lo - 2.0 && g_d1Queue[i].level <= hi + 2.0)
      {
         cmpScoreBoost += 30.0;
         return "[CMP D1] ";
      }
   ...
   // 2. Cek H4 Queue (SNR Sangat Kuat)
   for(int i = 0; i < ArraySize(g_h4Queue); i++)
   {
      if(g_h4Queue[i].level >= lo - 2.0 && g_h4Queue[i].level <= hi + 2.0)
      {
         cmpScoreBoost += 25.0;
         return "[CMP H4] ";
      }
   ...
   // 3. Cek H1 Queue (SNR Kuat)
   for(int i = 0; i < ArraySize(g_h1Queue); i++)
   {
      if(g_h1Queue[i].level >= lo - 1.5 && g_h1Queue[i].level <= hi + 1.5)
      {
         cmpScoreBoost += 15.0;
         return "[CMP H1] ";
      }
...
//+------------------------------------------------------------------+
//| v53.30: Dadang - "garis SND Bookmap tapi yang diambil area penting saja"
//| Gambar GARIS HORIZONTAL BERSIH untuk Batas S&D Bookmap Penting
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.31: DrawZoneBox - CREATE ONCE, UPDATE WHEN CHANGED
//| Stable object IDs: SD_S1_HI / SD_S1_LO / SD_S1_TXT1 etc.
//| NO ObjectsDeleteAll - objects reused across ticks
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.33: MASTER CLEAN S&D RECTANGLE ZONE + TIGHT 3-LINE CARD
//| - Kotak S&D nyata di area lilin (TIDAK tembus/masuk ke panel kiri!)
//| - Label 3 baris tertata rapat di sisi kanan kotak
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.34: EXACT MATCH DADANG BLUEPRINT
//| - Shaded Rectangle Box (Merah Supply, Hijau Demand)
//| - Diamond Marker ◆ di tepi kiri kotak
//| - Label 3 Baris Elegan di sisi kanan dalam kotak
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.35: EXACT MATCH V2 S&D VISUAL ENGINE (Clean, Crisp, In-Box Text)
//| - Kotak S&D Transparan Garis Halus (Tanpa fill pekat/hitam yang nutupin)
//| - Label Format V2: S1 221L | SEDANG 51 (AUS) | diuji 13x, serap 0x
//| - Posisi Teks: Di dalam kotak secara horizontal di atas lilin
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.38: Hitung Total Lot Limit Order (Bid/Ask Live) yang Parkir di Area S&D
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.40: Hitung Total Lot Real Live Bid/Ask Bookmap di Area Zona
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.45: HITUNG REAL-TIME FOOTPRINT ATTACK DELTA DI DALAM ZONA S&D |
//+------------------------------------------------------------------+
double GetZoneDelta(bool isDemand, double lo, double hi)
{
   if(!g_bookmapOnline) return 0.0;
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   
   // Cek apakah harga sedang berada di dalam atau sedang menguji zona (+- 0.50 USD)
   if(price >= lo - 0.50 && price <= hi + 0.50)
   {
      double m1Delta = g_bookmapFootM1BuyVol - g_bookmapFootM1SellVol;
      if(g_bookmapFootM1BuyVol + g_bookmapFootM1SellVol > 0)
         return m1Delta;
      return (g_bookmapFootBuyVol - g_bookmapFootSellVol);
   }
   return 0.0;
}

string FormatDeltaString(double delta)
{
   if(delta > 0.0) return StringFormat("Δ+%.0f", delta);
   if(delta < 0.0) return StringFormat("Δ-%.0f", MathAbs(delta));
   return "Δ:0";
}

double GetLiveWallInZone(bool isDemand, double lo, double hi)
{
   if(!g_bookmapOnline || g_bookmapPrice <= 0) return 0.0;
   double offset = SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice;
   double totalLiveLot = 0.0;

   if(isDemand)
   {
      for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
      {
         if(g_bookmapBidPx[i] > 0 && g_bookmapBidSz[i] > 0)
         {
            double mt5Px = g_bookmapBidPx[i] + offset;
            if(mt5Px >= (lo - 1.0) && mt5Px <= (hi + 1.0))
            {
               totalLiveLot += g_bookmapBidSz[i];
            }
         }
      }
      // Jika zona D1 sangat dekat dengan harga (dalam jarak $3), ambil best bid size jika belum ada
      if(totalLiveLot == 0.0 && g_bookmapBidSz[0] > 0)
      {
         double bestBidMt5 = g_bookmapBidPx[0] + offset;
         if(bestBidMt5 >= lo - 2.0 && bestBidMt5 <= hi + 2.0)
            totalLiveLot = g_bookmapBidSz[0];
      }
   }
   else
   {
      for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
      {
         if(g_bookmapAskPx[i] > 0 && g_bookmapAskSz[i] > 0)
         {
            double mt5Px = g_bookmapAskPx[i] + offset;
            if(mt5Px >= (lo - 1.0) && mt5Px <= (hi + 1.0))
            {
               totalLiveLot += g_bookmapAskSz[i];
            }
         }
      }
      // Jika zona S1 sangat dekat dengan harga (dalam jarak $3), ambil best ask size jika belum ada
      if(totalLiveLot == 0.0 && g_bookmapAskSz[0] > 0)
      {
         double bestAskMt5 = g_bookmapAskPx[0] + offset;
         if(bestAskMt5 >= lo - 2.0 && bestAskMt5 <= hi + 2.0)
            totalLiveLot = g_bookmapAskSz[0];
      }
   }
   return totalLiveLot;
}
...
//+------------------------------------------------------------------+
//| v53.44: REAL-TIME ON-TICK LIVE WALL NUMBER TICKER (ZERO FLICKER)  |
//| Hanya mengupdate teks angka lot di dalam kotak saat lot berubah    |
//+------------------------------------------------------------------+
void UpdateLiveWallLabelsOnTick()
{
   if(!InpShowZonesOnChart || !g_bookmapOnline) return;
   
   for(int a = 0; a < SD_MAX_ZONES; a++)
   {
      // Update Supply Live Ticker (Live Lot & Delta)
      if(g_supplyState[a].active && g_supplyState[a].hi > 0)
      {
         string tag = StringFormat("S%d", a + 1);
         string nameTxt = "SD_" + tag + "_TXT";
         double liveLot   = GetLiveWallInZone(false, g_supplyState[a].lo, g_supplyState[a].hi);
         double liveDelta = GetZoneDelta(false, g_supplyState[a].lo, g_supplyState[a].hi);
         
         if(MathAbs(liveLot - g_supplyState[a].liveLot) > 0.01 || MathAbs(liveDelta - g_supplyState[a].delta) > 0.5)
         {
      ...
      // Update Demand Live Ticker (Live Lot & Delta)
      if(g_demandState[a].active && g_demandState[a].hi > 0)
      {
         string tag = StringFormat("D%d", a + 1);
         string nameTxt = "SD_" + tag + "_TXT";
         double liveLot   = GetLiveWallInZone(true, g_demandState[a].lo, g_demandState[a].hi);
         double liveDelta = GetZoneDelta(true, g_demandState[a].lo, g_demandState[a].hi);
         
         if(MathAbs(liveLot - g_demandState[a].liveLot) > 0.01 || MathAbs(liveDelta - g_demandState[a].delta) > 0.5)
         {
...
{
   if(!InpShowZonesOnChart) return;
   if(rank >= SD_MAX_ZONES) return;
   if(lo <= 0.0 || hi <= 0.0 || lo >= hi) return;

   // Batasi tebal zona maksimal 7.0 USD
   if(hi - lo > 7.0) hi = lo + 7.0;
   ...
   int borderW = (rank == 0) ? 2 : 1;
   ENUM_LINE_STYLE borderS = (rank == 0) ? STYLE_SOLID : STYLE_DASH;

   double liveWallLot = GetLiveWallInZone(isDemand, lo, hi);
   double liveDelta   = GetZoneDelta(isDemand, lo, hi);

   // Cache check (Create Once, Update When Changed)
   bool prev_active   = isDemand ? g_demandState[rank].active  : g_supplyState[rank].active;
   double prev_lo     = isDemand ? g_demandState[rank].lo      : g_supplyState[rank].lo;
   double prev_hi     = isDemand ? g_demandState[rank].hi      : g_supplyState[rank].hi;
   double prev_lot    = isDemand ? g_demandState[rank].lot     : g_supplyState[rank].lot;
   double prev_live   = isDemand ? g_demandState[rank].liveLot : g_supplyState[rank].liveLot;
   double prev_delta  = isDemand ? g_demandState[rank].delta   : g_supplyState[rank].delta;
   int    prev_ret    = isDemand ? g_demandState[rank].retest  : g_supplyState[rank].retest;
   int    prev_abs    = isDemand ? g_demandState[rank].absorb  : g_supplyState[rank].absorb;
   double prev_sc     = isDemand ? g_demandState[rank].score   : g_supplyState[rank].score;

   bool changed = (!prev_active || MathAbs(prev_lo - lo) > 0.001 || MathAbs(prev_hi - hi) > 0.001 ||
                   MathAbs(prev_lot - totalLot) > 0.5 || MathAbs(prev_live - liveWallLot) > 1.0 ||
                   MathAbs(prev_delta - liveDelta) > 1.0 ||
                   prev_ret != retestCount || prev_abs != absorptionHits || prev_sc != score);

   if(!changed) return; // SKIP - nothing changed

   // Update cache
   if(isDemand) {
      g_demandState[rank].active=true; g_demandState[rank].lo=lo; g_demandState[rank].hi=hi;
      g_demandState[rank].lot=totalLot; g_demandState[rank].liveLot=liveWallLot; g_demandState[rank].delta=liveDelta;
      g_demandState[rank].score=score; g_demandState[rank].retest=retestCount; g_demandState[rank].absorb=absorptionHits;
   } else {
      g_supplyState[rank].active=true; g_supplyState[rank].lo=lo; g_supplyState[rank].hi=hi;
      g_supplyState[rank].lot=totalLot; g_supplyState[rank].liveLot=liveWallLot; g_supplyState[rank].delta=liveDelta;
...
void HideZoneSlot(string tag)
{
   HideLineObject("SD_" + tag + "_BOX");
   HideLineObject("SD_" + tag + "_TXT");
   HideLineObject("SD_" + tag + "_TX1");
   HideLineObject("SD_" + tag + "_TX2");
   HideLineObject("SD_" + tag + "_TX3");
   HideLineObject("SD_" + tag + "_DIA");
}

#define SDM_ZONE_CLUSTER_GAP_USD 3.0
//+------------------------------------------------------------------+
//| v53.46: DRAW CANDLE FOOTPRINT DELTA OVERLAY (SETIAP CANDLE M1)    |
//+------------------------------------------------------------------+
void UpdateCandleDeltaHistory()
{
   datetime curBarTime = iTime(_Symbol, _Period, 0);
   if(curBarTime != g_lastCDeltaBarTime && g_lastCDeltaBarTime > 0)
   {
      // Geser history delta ke belakang saat candle baru terbentuk
      for(int k = MAX_CDELTA_BARS - 1; k > 0; k--)
         g_candleDeltaHistory[k] = g_candleDeltaHistory[k - 1];
      
      // Simpan delta candle yang baru close
      double closedDelta = (g_bookmapFootM1BuyVol - g_bookmapFootM1SellVol);
      if(closedDelta == 0.0)
      {
         double cOpen  = iOpen(_Symbol, _Period, 1);
         double cClose = iClose(_Symbol, _Period, 1);
         long   cVol   = iVolume(_Symbol, _Period, 1);
         closedDelta   = (cClose >= cOpen) ? (double)cVol * 0.6 : -(double)cVol * 0.6;
      }
      g_candleDeltaHistory[1] = closedDelta;
   }
   g_lastCDeltaBarTime = curBarTime;
   
   // Inisialisasi awal history jika masih kosong
   if(g_candleDeltaHistory[2] == 0.0)
   {
      for(int k = 1; k < MAX_CDELTA_BARS; k++)
      {
         double cOpen  = iOpen(_Symbol, _Period, k);
         double cClose = iClose(_Symbol, _Period, k);
         long   cVol   = iVolume(_Symbol, _Period, k);
         g_candleDeltaHistory[k] = (cClose >= cOpen) ? (double)cVol * 0.55 : -(double)cVol * 0.55;
      }
   }
}

//+------------------------------------------------------------------+
//| v53.47: SIERRA CHART PRO ORDER FLOW ENGINES                       |
//+------------------------------------------------------------------+

// 1. STACKED IMBALANCE (3:1 Diagonal Imbalance Levels)
void UpdateStackedImbalanceEngine()
{
   g_scImbalanceSide = "NONE";

   if(!InpSC_StackedImbalance || !g_bookmapOnline)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "IMB_");
      return;
   }
   ...
   // Deteksi Stacked Buy Imbalance (Buyer 3x lebih agresif)
   if(g_bookmapPulsePct >= 75.0 && g_bookmapFootM1BuyVol >= g_bookmapFootM1SellVol * 3.0 && g_bookmapFootM1BuyVol >= 25.0)
   {
      g_scImbalanceSide = "BUY";
      string name = SC_PREFIX + "IMB_BUY";
      datetime t1 = TimeCurrent() - PeriodSeconds(_Period) * 15;
      datetime t2 = TimeCurrent() + PeriodSeconds(_Period) * 10;
      SetRectangle(name, t1, price + 1.0 * pip, t2, price - 1.0 * pip, C'0,200,100', 1, STYLE_SOLID, false);
      SetText(name + "_TXT", t1 + PeriodSeconds(_Period)*1, price, "🟩 BUYER MASUK (3:1)", C'180,255,200', 8, "Segoe UI Bold", ANCHOR_LEFT);
   }
   // Deteksi Stacked Sell Imbalance (Seller 3x lebih agresif)
   else if(g_bookmapPulsePct <= 25.0 && g_bookmapFootM1SellVol >= g_bookmapFootM1BuyVol * 3.0 && g_bookmapFootM1SellVol >= 25.0)
   {
      g_scImbalanceSide = "SELL";
      string name = SC_PREFIX + "IMB_SELL";
      datetime t1 = TimeCurrent() - PeriodSeconds(_Period) * 15;
      datetime t2 = TimeCurrent() + PeriodSeconds(_Period) * 10;
      SetRectangle(name, t1, price + 1.0 * pip, t2, price - 1.0 * pip, C'230,60,60', 1, STYLE_SOLID, false);
      SetText(name + "_TXT", t1 + PeriodSeconds(_Period)*1, price, "🟥 SELLER MASUK (3:1)", C'255,200,200', 8, "Segoe UI Bold", ANCHOR_LEFT);
   }
}

// 2. NAKED POC / VPOC MAGNET
double g_scVirginPocPrice = 0.0;
bool   g_scVirginPocActive = false;

void UpdateNakedPocEngine()
{
   if(!InpSC_NakedPoc)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "VPOC");
      return;
   }
   
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double offset = (g_bookmapPrice > 0) ? (price - g_bookmapPrice) : 0.0;
   
   // Catat POC kemarin jika belum tersentuh
   if(g_dpfYestPoc > 0.0 && g_scVirginPocPrice == 0.0)
   {
      g_scVirginPocPrice  = g_dpfYestPoc;
      g_scVirginPocActive = true;
   }
   
   if(g_scVirginPocActive && g_scVirginPocPrice > 0.0)
   {
      // Jika harga menyentuh VPOC, VPOC ditandai telah dites (cleared)
      if(MathAbs(price - g_scVirginPocPrice) <= 0.30)
      {
         g_scVirginPocActive = false;
         ObjectsDeleteAll(0, SC_PREFIX + "VPOC");
         return;
      }
      
      string name = SC_PREFIX + "VPOC";
      string txtName = name + "_TXT";
      SetHLine(name, g_scVirginPocPrice, clrGold, 1, STYLE_DOT);
      datetime tPos = TimeCurrent() + PeriodSeconds(_Period) * 6;
      SetText(txtName, tPos, g_scVirginPocPrice, StringFormat("🧲 MAGNET VPOC @%.2f", g_scVirginPocPrice), clrGold, 8, "Segoe UI Bold", ANCHOR_LEFT);
   }
}

// 3. DELTA DIVERGENCE (TRAPPED TRADERS / PERANGKAP BANDAR)
void UpdateDeltaDivergenceEngine()
{
   // v53.54: Dadang audit fix - marker "JEBAKAN BUYER/SELLER" ini KLAIM
   // baca order-flow beneran (delta asli), tapi g_candleDeltaHistory[] bisa
   // isinya cuma tebakan candle-color kalau Bookmap offline (lihat
   // UpdateCandleDeltaHistory()). Tanpa data order-flow asli, "divergence"
   // di sini cuma "higher high tapi candle merah" - pola price-action biasa
   // yang dikasih nama order-flow, menyesatkan. Gate penuh di g_bookmapOnline.
   g_scDivergenceType = "NONE";

   if(!InpSC_DeltaDivergence || !g_bookmapOnline)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "DIV_");
      return;
   }

   if(MAX_CDELTA_BARS < 5) return;

   double h1 = iHigh(_Symbol, _Period, 1), h2 = iHigh(_Symbol, _Period, 2);
   double l1 = iLow(_Symbol, _Period, 1),  l2 = iLow(_Symbol, _Period, 2);
   double d1 = g_candleDeltaHistory[1],   d2 = g_candleDeltaHistory[2];
   datetime t1 = iTime(_Symbol, _Period, 1);
   double pip = PipSize();

   // Bearish Divergence: Higher High tapi Delta Negatif (Buyer Terjebak!)
   if(h1 > h2 && d1 < -10.0)
   {
      g_scDivergenceType = "BEAR";
      string name = SC_PREFIX + "DIV_BEAR_" + IntegerToString((long)t1);
      if(EnsureObject(name, OBJ_TEXT))
      {
         ObjectSetString(0, name, OBJPROP_TEXT, "▼ JEBAKAN BUYER (SIAP SELL)");
         ObjectMove(0, name, 0, t1, h1 + 3.0 * pip);
         ObjectSetInteger(0, name, OBJPROP_COLOR, C'255,80,180');
   ...
   // Bullish Divergence: Lower Low tapi Delta Positif (Seller Terjebak!)
   else if(l1 < l2 && d1 > 10.0)
   {
      g_scDivergenceType = "BULL";
      string name = SC_PREFIX + "DIV_BULL_" + IntegerToString((long)t1);
      if(EnsureObject(name, OBJ_TEXT))
      {
         ObjectSetString(0, name, OBJPROP_TEXT, "▲ JEBAKAN SELLER (SIAP BUY)");
         ObjectMove(0, name, 0, t1, l1 - 3.0 * pip);
         ObjectSetInteger(0, name, OBJPROP_COLOR, C'0,230,200');
         ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 8);
         ObjectSetString(0, name, OBJPROP_FONT, "Segoe UI Bold");
...
// 4. WHALE BUBBLES / BIG TRADES MARKER
// 4. WHALE BUBBLES / BIG TRADES MARKER (HANYA 1 LABEL TERBARU, TIDAK BERTUMPUK!)
void UpdateWhaleTradesEngine()
{
   g_scWhaleSide = "NONE";
   g_scWhaleLots = 0.0;

   if(!InpSC_BigTradesWhale || !g_bookmapOnline)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "WHALE");
      return;
   }

   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   datetime curTime = iTime(_Symbol, _Period, 0);
   string name = SC_PREFIX + "WHALE_ACTIVE";

   // Deteksi Whale Buy Sejati (>= 100 Lot)
   if(g_bookmapFootM1BuyVol >= InpSC_BigTradeLotMin)
   {
      g_scWhaleSide = "BUY";
      g_scWhaleLots = g_bookmapFootM1BuyVol;
      if(EnsureObject(name, OBJ_TEXT))
      {
         ObjectSetString(0, name, OBJPROP_TEXT, StringFormat("🐋 PAUS BUY %.0fL", g_bookmapFootM1BuyVol));
         ObjectMove(0, name, 0, curTime, price);
         ObjectSetInteger(0, name, OBJPROP_COLOR, C'0,255,140');
         ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 9);
         ObjectSetString(0, name, OBJPROP_FONT, "Segoe UI Bold");
         ObjectSetInteger(0, name, OBJPROP_ANCHOR, ANCHOR_CENTER);
      }
   }
   // Deteksi Whale Sell Sejati (>= 100 Lot)
   else if(g_bookmapFootM1SellVol >= InpSC_BigTradeLotMin)
   {
      g_scWhaleSide = "SELL";
      g_scWhaleLots = g_bookmapFootM1SellVol;
      if(EnsureObject(name, OBJ_TEXT))
      {
         ObjectSetString(0, name, OBJPROP_TEXT, StringFormat("🐋 PAUS SELL %.0fL", g_bookmapFootM1SellVol));
         ObjectMove(0, name, 0, curTime, price);
         ObjectSetInteger(0, name, OBJPROP_COLOR, C'255,70,70');
         ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 9);
         ObjectSetString(0, name, OBJPROP_FONT, "Segoe UI Bold");
         ObjectSetInteger(0, name, OBJPROP_ANCHOR, ANCHOR_CENTER);
      }
...
void UpdateUnfinishedAuctionEngine()
{
   g_scUnfinishedFound = false;

   if(!InpSC_UnfinishedAuction)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "UNF_");
      return;
   }

   for(int k = 1; k <= 5; k++)
   {
      datetime bTime = iTime(_Symbol, _Period, k);
      double   bHigh = iHigh(_Symbol, _Period, k);
      double   bLow  = iLow(_Symbol, _Period, k);
      double   pip   = PipSize();

      // Jika candle memiliki body panjang tapi sumbu atas sangat tumpul (Poor High)
      if(MathAbs(iClose(_Symbol, _Period, k) - bHigh) < 0.2 * pip && iClose(_Symbol, _Period, k) > iOpen(_Symbol, _Period, k))
      {
         if(!g_scUnfinishedFound)   // k=1 duluan (paling baru) - itu yang di-export
         {
            g_scUnfinishedFound = true;
            g_scUnfinishedTime  = bTime;
            g_scUnfinishedPrice = bHigh;
         }
         string name = SC_PREFIX + "UNF_H_" + IntegerToString((long)bTime);
         if(EnsureObject(name, OBJ_TEXT))
         {
            ObjectSetString(0, name, OBJPROP_TEXT, "--- Poor High (Unfinished)");
            ObjectMove(0, name, 0, bTime, bHigh);
...
// 6. P-SHAPE & b-SHAPE AUCTION PROFILE BIAS
// v53.50: InpSC_ProfileShapeBias punya toggle-nya dari v53.47 tapi gak
// pernah disambungin ke apa-apa (dead switch) - klasifikasi P-SHAPE/
// b-SHAPE/D-SHAPE-nya sendiri SUDAH ADA dari sebelumnya di
// DailyProfileFramingText() (baris "Daily Profile" di panel), cuma belum
// ada marker di chart-nya sendiri. Reuse logic yang sama (jangan duplikat),
// tambahin badge on-chart yang di-gate toggle ini, konsisten sama 5 fitur
// SC_ lainnya di suite ini.
void UpdateProfileShapeBiasEngine()
{
   if(!InpSC_ProfileShapeBias)
   {
      ObjectsDeleteAll(0, SC_PREFIX + "SHAPE");
      return;
   }

   color shapeClr;
   string shapeTxt = DailyProfileFramingText(shapeClr);
   if(shapeTxt == "-") return;

   string name = SC_PREFIX + "SHAPE";
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   datetime tPos = TimeCurrent() + PeriodSeconds(_Period) * 6;
   if(EnsureObject(name, OBJ_TEXT))
   {
      ObjectSetString(0, name, OBJPROP_TEXT, "📐 " + shapeTxt);
      ObjectMove(0, name, 0, tPos, price);
      ObjectSetInteger(0, name, OBJPROP_COLOR, shapeClr);
      ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 8);
      ObjectSetString(0, name, OBJPROP_FONT, "Segoe UI Bold");
      ObjectSetInteger(0, name, OBJPROP_ANCHOR, ANCHOR_LEFT);
      ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
...
// 7. MASTER DISPATCHER SIERRA CHART SUITE
void RunSierraChartSuite()
{
   UpdateStackedImbalanceEngine();
   UpdateNakedPocEngine();
   UpdateDeltaDivergenceEngine();
   UpdateWhaleTradesEngine();
   UpdateUnfinishedAuctionEngine();
   UpdateProfileShapeBiasEngine();
}


void DrawCandleDeltaOverlay()
{
   if(!InpShowCandleDelta)
   {
      ObjectsDeleteAll(0, CDELTA_PREFIX);
      return;
   }
   
   // Bersihkan objek candle lama jika InpCandleDeltaBars = 1
   for(int k = InpCandleDeltaBars; k < MAX_CDELTA_BARS; k++)
   {
      string oldName = StringFormat("%s%d", CDELTA_PREFIX, k);
      if(ObjectFind(0, oldName) >= 0) ObjectDelete(0, oldName);
   }
   
   double pip = PipSize();
   int maxBars = MathMax(1, MathMin(InpCandleDeltaBars, MAX_CDELTA_BARS));
   ...
   for(int k = 0; k < maxBars; k++)
   {
      ...
      // v53.54: Dadang audit fix - dulu delta hasil TEBAKAN (candle volume x
      // 0.6, dari warna candle doang) digambar SAMA PERSIS kayak delta asli
      // dari Bookmap, gak ada tanda pembeda. Sekarang di-track `isReal`,
      // tebakan dikasih prefix "~" + warna abu-abu supaya jelas beda dari
      // data order-flow yang beneran terukur.
      double dVal = (g_bookmapFootM1BuyVol - g_bookmapFootM1SellVol);
      bool   isReal = (dVal != 0.0);
      if(dVal == 0.0 && g_bookmapOnline)
      {
         dVal = (g_bookmapFootBuyVol - g_bookmapFootSellVol);
         isReal = (dVal != 0.0);
      }
      if(dVal == 0.0)
      {
         double cO = iOpen(_Symbol, _Period, k);
         double cC = iClose(_Symbol, _Period, k);
         long   cV = iVolume(_Symbol, _Period, k);
         dVal = (cC >= cO) ? (double)cV * 0.6 : -(double)cV * 0.6;
         isReal = false;
      }

      string name = StringFormat("%s%d", CDELTA_PREFIX, k);
      string dPfx = isReal ? "Δ" : "~Δ";
      string txt  = (dVal > 0.0) ? StringFormat("%s +%.0f", dPfx, dVal) : ((dVal < 0.0) ? StringFormat("%s %.0f", dPfx, dVal) : StringFormat("%s 0", dPfx));
      color  clr  = !isReal ? clrGray : ((dVal > 0.0) ? C'0,240,130' : ((dVal < 0.0) ? C'255,80,80' : clrGray));
      
      // Posisi: Jika delta positif di atas High, negatif di bawah Low
      double posPrice;
      ENUM_ANCHOR_POINT anchor;
      if(dVal >= 0.0)
      {
         posPrice = bHigh + 1.2 * pip;
         anchor   = ANCHOR_LOWER;
      }
      else
      {
         posPrice = bLow - 1.2 * pip;
         anchor   = ANCHOR_UPPER;
      }
      ...
      {
         ObjectSetString(0, name, OBJPROP_TEXT, txt);
         ObjectMove(0, name, 0, bTime, posPrice);
         ObjectSetInteger(0, name, OBJPROP_COLOR, clr);
         ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 9);
         ObjectSetString(0, name, OBJPROP_FONT, "Segoe UI Bold");
         ObjectSetInteger(0, name, OBJPROP_ANCHOR, anchor);
...
{
   // Bersihkan objek panel pojok jika ada
...
//+------------------------------------------------------------------+
//| 3. WHALE FOOTPRINT BADGE DI EKOR LILIN (ANTI-TUMPUK & AUTO-PRUNE)|
//| Hanya simpan di 3 candle terakhir, 1 label bersih per lilin      |
//+------------------------------------------------------------------+
void UpdateWhaleFootprintBadges()
{
   if(!InpShowWhaleFootprintWick || !g_bookmapOnline)
   {
      ObjectsDeleteAll(0, "DADANG_WHALE_");
      return;
   }
   
   datetime tLatestBar = iTime(_Symbol, _Period, 0);
   datetime tOldestAllowed = tLatestBar - (PeriodSeconds(_Period) * 4);
   
   // 1. AUTO-PRUNE: Hapus badge Paus lama yang sudah lewat lebih dari 3 bar
   int totalObj = ObjectsTotal(0);
   for(int i = totalObj - 1; i >= 0; i--)
   {
      string name = ObjectName(0, i);
      if(StringFind(name, "DADANG_WHALE_") == 0)
      {
         datetime objTime = (datetime)ObjectGetInteger(0, name, OBJPROP_TIME, 0);
         if(objTime < tOldestAllowed)
         {
            ObjectDelete(0, name);
         }
      }
   ...
   // 2. ANTI-TUMPUK & NETTO MERGE: Evaluasi candle aktif (Bar #0)
   double buyVol  = g_bookmapFootM1BuyVol;
   double sellVol = g_bookmapFootM1SellVol;
   double totalVol = buyVol + sellVol;
   
   if(totalVol >= 50.0 || g_scWhaleLots >= 50.0)
   {
      ...
      // Pilih 1 PEMENANG VOLUME TERBESAR (Anti Tumpang Tindih!)
      bool isWhaleBuy = (buyVol >= sellVol);
      double dominantVol = isWhaleBuy ? MathMax(buyVol, g_scWhaleLots) : MathMax(sellVol, g_scWhaleLots);
      if(dominantVol < 50.0) dominantVol = totalVol;
      
      if(isWhaleBuy)
      {
         SetText(wName, tBar, lowPx - 0.45, StringFormat("🐋 PAUS BUY %.0fL", dominantVol), C'0,255,140', 8, "Segoe UI Bold", ANCHOR_CENTER);
      }
      else
      {
         SetText(wName, tBar, highPx + 0.45, StringFormat("👑 PAUS SELL %.0fL", dominantVol), C'255,70,70', 8, "Segoe UI Bold", ANCHOR_CENTER);
      }
   }
}
...
//+------------------------------------------------------------------+
//| DADANG FULL-DEPTH LIQUIDITY ACCUMULATOR                          |
//| Mengakumulasi seluruh lot antrean (1L, 2L, 3L, dst) per baris   |
//| harga di dalam rentang kotak Discretionary Dadang                 |
//+------------------------------------------------------------------+
double GetBookmapFullDepthInRange(double lo, double hi, int &levelsCount, double &maxAgeOut)
{
   levelsCount = 0;
   maxAgeOut = 0.0;
   int h = FileOpen("bookmap_full_depth.csv", FILE_READ | FILE_CSV | FILE_COMMON | FILE_ANSI, ',');
   if(h == INVALID_HANDLE) return 0.0;
   
   FileReadString(h); FileReadString(h); FileReadString(h); FileReadString(h); // skip header
   ...
   while(!FileIsEnding(h))
   {
      ...
      if(side == "") break;
      double p   = StringToDouble(FileReadString(h)) + offset;
      double sz  = StringToDouble(FileReadString(h));
      double age = StringToDouble(FileReadString(h));
      
      if(p >= (lo - 0.20) && p <= (hi + 0.20) && sz > 0)
      {
         totalLot += sz;
         levelsCount++;
         if(age > maxAgeOut) maxAgeOut = age;
      }
   }
   FileClose(h);
   return totalLot;
...
//+------------------------------------------------------------------+
//| 🏛️ ICT / SMC INSTITUTIONAL BIG ORDER QUANT ENGINE                |
//| Created for Commander Dadang Wahyuono                           |
//| Melacak jejak Big Order Institusi: Order Block (OB), FVG,       |
//| Liquidity Sweeps (BSL/SSL), dan menyinkronkan dengan Bookmap    |
//+------------------------------------------------------------------+

struct ICT_Zone
{
   string           type;          // "OB", "FVG", "BSL_SWEEP", "SSL_SWEEP"
   ENUM_TIMEFRAMES  tf;            // Timeframe asal
   double           top;           // Batas atas harga
   double           bottom;        // Batas bawah harga
   double           mid;           // Titik tengah
   datetime         time;          // Waktu pembentukan
   bool             isBullish;     // true=Demand/Buy, false=Supply/Sell
   bool             isFresh;       // true=Belum termitigasi
   double           bmWallLot;     // Total antrean limit order Bookmap di dalam zona
   int              bmLevels;      // Jumlah baris level harga
   double           maxWallAge;    // Usia tertua antrean di dalam zona (detik)
   string           confluenceTag; // Label ringkas
};

#define MAX_ICT_ZONES 30
ICT_Zone g_ictZones[MAX_ICT_ZONES];
int      g_ictZoneCount = 0;

//+------------------------------------------------------------------+
//| Scan ICT Fair Value Gaps (FVG) & Order Blocks (OB)               |
//+------------------------------------------------------------------+
void ScanICT_InstitutionalStructures(ENUM_TIMEFRAMES tf)
{
   int totalBars = iBars(_Symbol, tf);
   if(totalBars < 10) return;
   
   int lookback = MathMin(100, totalBars - 3);
   
   for(int i = 1; i < lookback; i++)
   {
      if(g_ictZoneCount >= MAX_ICT_ZONES) break;
      ...
      // 1. BULLISH FVG (Imbalance Void): Low[i] > High[i+2]
      if(low0 > high2 + 0.30)
      {
         double fvgTop = low0;
         double fvgBottom = high2;
         
         // Cek apakah sudah termitigasi oleh candle-candle setelahnya
         bool isMitigated = false;
         for(int j = i - 1; j >= 0; j--)
         {
            if(iLow(_Symbol, tf, j) <= fvgTop) { isMitigated = true; break; }
         }
         ...
         if(!isMitigated)
         {
            ...
            z.top = fvgTop;
            z.bottom = fvgBottom;
            z.mid = (fvgTop + fvgBottom) * 0.5;
            z.time = barTime;
            z.isBullish = true;
            z.isFresh = true;
            
            z.bmWallLot = GetBookmapFullDepthInRange(z.bottom, z.top, z.bmLevels, z.maxWallAge);
            z.confluenceTag = (z.maxWallAge >= 3600.0) ? "🧲 FVG+LTHL" : "🧲 FVG";
            
            g_ictZones[g_ictZoneCount++] = z;
         }
      }
      
      // 2. BEARISH FVG (Imbalance Void): High[i] < Low[i+2]
      if(high0 < low2 - 0.30)
      {
         double fvgTop = low2;
         double fvgBottom = high0;
         
         bool isMitigated = false;
         for(int j = i - 1; j >= 0; j--)
         {
            if(iHigh(_Symbol, tf, j) >= fvgBottom) { isMitigated = true; break; }
         }
      ...
      // 3. BULLISH ORDER BLOCK (OB): Candle merah terakhir sebelum impulsif naik kencang
      double body1 = MathAbs(close0 - open0);
      double body2 = MathAbs(iClose(_Symbol, tf, i+1) - iOpen(_Symbol, tf, i+1));
      if(close0 > open0 && (close0 - open0) >= body2 * 1.8 && iClose(_Symbol, tf, i+1) < iOpen(_Symbol, tf, i+1))
      {
         double obTop = high1;
         double obBottom = low1;
         
         bool isMitigated = false;
         for(int j = i - 1; j >= 0; j--)
         {
            if(iLow(_Symbol, tf, j) < obBottom) { isMitigated = true; break; }
         }
         ...
         if(!isMitigated)
         {
            ...
            z.top = obTop;
            z.bottom = obBottom;
            z.mid = (obTop + obBottom) * 0.5;
            z.time = iTime(_Symbol, tf, i+1);
            z.isBullish = true;
            z.isFresh = true;
            
            z.bmWallLot = GetBookmapFullDepthInRange(z.bottom, z.top, z.bmLevels, z.maxWallAge);
            z.confluenceTag = (z.maxWallAge >= 3600.0) ? "🏛️ OB+LTHL" : "🏛️ OB";
            
            g_ictZones[g_ictZoneCount++] = z;
         }
      }
      
      // 4. BEARISH ORDER BLOCK (OB): Candle hijau terakhir sebelum impulsif turun kencang
      if(close0 < open0 && (open0 - close0) >= body2 * 1.8 && iClose(_Symbol, tf, i+1) > iOpen(_Symbol, tf, i+1))
      {
         double obTop = high1;
         double obBottom = low1;
         ...
         for(int j = i - 1; j >= 0; j--)
         {
            if(iHigh(_Symbol, tf, j) > obTop) { isMitigated = true; break; }
         }
...
//+------------------------------------------------------------------+
//| Cross-Check Box Dadang dengan Struktur ICT / SMC                |
//+------------------------------------------------------------------+
string CheckICT_BoxConfluence(double lo, double hi, bool isResisten)
{
   string tags = "";
   for(int k = 0; k < g_ictZoneCount; k++)
   {
      // Cek apakah ada irisan area antara kotak Dadang dan zona ICT
      if(g_ictZones[k].top >= lo - 0.50 && g_ictZones[k].bottom <= hi + 0.50)
      {
         if(StringFind(tags, g_ictZones[k].confluenceTag) < 0)
         {
            if(tags != "") tags += " + ";
            tags += g_ictZones[k].confluenceTag;
         }
      }
   }
   return tags;
...
//+------------------------------------------------------------------+
//| Deteksi Liquidity Sweep (BSL / SSL / Stop Hunt) di Area Kotak    |
//+------------------------------------------------------------------+
bool CheckLiquiditySweepOnBox(double lo, double hi, bool isResisten)
{
   double curPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   // Cek lilin terakhir apakah membuat ekor menembus level kotak lalu close di dalam
   ...
   if(isResisten)
   {
      // BSL Sweep: High menusuk ke atas kotak tapi Close ditarik turun kembali
      if(lastHigh >= hi && lastClose < hi && (g_bookmapAbsorption != "NONE" || g_bookmapCvd < 0))
         return true;
   }
   else
   {
      // SSL Sweep: Low menusuk ke bawah kotak tapi Close ditarik naik kembali
      if(lastLow <= lo && lastClose > lo && (g_bookmapAbsorption != "NONE" || g_bookmapCvd > 0))
         return true;
   }
   return false;
}
...
//+------------------------------------------------------------------+
//| GAMBAR STRUKTUR ICT / SMC DI CHART (FVG, Order Block, Magnet)    |
//| Hanya menggambar zona-zona paling relevan & terdekat agar bersih|
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| ULTRA CLEAN CHART MODE: HAPUS SEMUA KOTAK OTOMATIS LATAR BELAKANG|
//| Chart 100% PLONG & BERSIH! Otak ICT & Bookmap HANYA menempel ke  |
//| KOTAK GAMBAR DADANG SENDIRI!                                     |
//+------------------------------------------------------------------+
void DrawICT_StructuresOnChart()
{
   // Hapus seluruh objek otomatis ICT dari chart secara permanen!
   ObjectsDeleteAll(0, "ICT_ZONE_");
   ObjectsDeleteAll(0, "ICT_");
}
...
{
   // 1. Scan Struktur ICT / SMC HANYA DARI HTF (H4 & H1) - DOKTRIN ANTI-PINGPONG COMMANDER DADANG!
   g_ictZoneCount = 0;
   ScanICT_InstitutionalStructures(PERIOD_H4);
   ScanICT_InstitutionalStructures(PERIOD_H1);
   DrawICT_StructuresOnChart();
   ...
   for(int i = 0; i < totalObj; i++)
   {
      ...
      if(objType == OBJ_RECTANGLE)
      {
         ...
         // Akumulasi seluruh baris order book bursa CME (1L, 2L, 3L, dst) di dalam rentang kotak
         int depthLevels = 0;
         double maxWallAge = 0.0;
         double fullDepthLot = GetBookmapFullDepthInRange(bottom, top, depthLevels, maxWallAge);
         if(fullDepthLot > 0.0)
         {
            totalWall = fullDepthLot;
            totalWalls = depthLevels;
         }
         
         // PENENTUAN MUTLAK SUPORT & RESISTEN BERDASARKAN POSISI HARGA:
         // 1. Jika Area di ATAS Harga Running -> RESISTEN (Tahanan Atas / Merah)
         // 2. Jika Area di BAWAH Harga Running -> SUPORT (Lantai Bawah / Hijau)
         double midBox = (top + bottom) * 0.5;
         bool isResisten = (midBox >= curPrice);
         
         string typeTag  = isResisten ? "🔴 RESISTEN" : "🟢 SUPORT";
         string sideName = isResisten ? "RESISTEN" : "SUPORT";
         string iconTag  = isResisten ? "🔴" : "🟢";
         color  tagColor = isResisten ? C'255,80,80' : C'0,230,118';
         
         // Sesuaikan warna outline kotak Dadang otomatis agar sinkron
         ObjectSetInteger(0, name, OBJPROP_COLOR, tagColor);
         
         double zoneDelta = GetZoneDelta(!isResisten, bottom, top);
         ...
         // Hitung Volume Historis Candlestick (murni dalam LOT)
          ...
          int scanBars = MathMin(500, totalBars);
          for(int b = 0; b < scanBars; b++)
          {
         ...
         GetBoxHistoricalSND(bottom, top, isResisten, patternTag, retestCount, histTf);
         string retestStr = (retestCount > 0) ? StringFormat("Uji %dx", retestCount) : "FRESH";
         string liveWallStr = "";
         if(totalWall >= 50.0)
            liveWallStr = StringFormat("🛡️ LIVE WALL: %.0f LOT (%d Wall)", totalWall, totalWalls);
         else if(totalWall > 0.0)
            liveWallStr = StringFormat("LIVE WALL: %.0f LOT (%d Wall)", totalWall, totalWalls);
         else
         {
            double distToBox = (curPrice > top) ? (curPrice - top) : ((curPrice < bottom) ? (bottom - curPrice) : 0.0);
            if(distToBox >= 10.0)
               liveWallStr = StringFormat("🏛️ BASE %s (Jarak: %.1f$) • Standby Saat Harga Dekat", objTf, distToBox);
            else
               liveWallStr = "LIVE WALL: 0 LOT (Zona Tipis)";
         }
         ...
         // 5. Klasifikasi Status & Kualitas Suport & Resisten yang Akurat (Anti-Prematur!)
         string qualityCategory = "";
         color qualityColor = tagColor;
         
         // Validasi Mutlak Breakout: Hanya disebut JEBOL jika harga SUDAH BENAR-BENAR TEMBUS KELUAR KOTAK!
         bool isTrulyBrokenAbove = (curPrice > top + 0.30);
         bool isTrulyBrokenBelow = (curPrice < bottom - 0.30);
         
         bool isSpoofInside = (TimeCurrent() - g_spoofAlertTime <= 60 && g_spoofAlertPx >= bottom - 0.50 && g_spoofAlertPx <= top + 0.50);
         
         if(isTrulyBrokenAbove && isResisten)
         {
            qualityCategory = "🚀 JEBOL ATAS";
            qualityColor = C'0,255,140'; // Breakout valid
         }
         else if(isTrulyBrokenBelow && !isResisten)
         {
            qualityCategory = "💥 JEBOL BAWAH";
            qualityColor = C'255,80,80'; // Breakdown valid
         }
         else if(isSpoofInside)
         {
            qualityCategory = "🎭 SPOOFING";
            qualityColor = C'255,180,0'; // Amber/Gold
         }
         else if(totalWall >= 40.0 || (histLot >= 150.0 && retestCount <= 1))
         {
            qualityCategory = "🛡️ KUAT";
            qualityColor = isResisten ? C'255,60,60' : C'0,240,120';
         }
         else if(retestCount >= 4)
         {
            qualityCategory = "⚠️ RAWAN (DIUJI " + IntegerToString(retestCount) + "x)";
            qualityColor = C'255,180,0';
         }
         else if(retestCount >= 2 || totalWall >= 20.0)
         {
            qualityCategory = "⚖️ SEDANG";
            qualityColor = tagColor;
         }
         else
         {
         ...
         // 6. Deteksi Interaksi Real-Time Multi-Sensor Ultra-Sensitif (Bid, Ask, High, Low Wick)
         double askPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
         double curHigh  = iHigh(_Symbol, _Period, 0);
         double curLow   = iLow(_Symbol, _Period, 0);
         
         bool isInside = ((curPrice >= bottom - 0.25 && curPrice <= top + 0.25) || (askPrice >= bottom - 0.25 && askPrice <= top + 0.25));
         bool isGrazingWick = (!isInside && curHigh >= bottom - 0.40 && curLow <= top + 0.40);
         
         string touchStatus = "";
         if(isInside || isGrazingWick)
         {
            if(!isResisten && (zoneDelta >= 10.0 || g_bookmapAbsorption == "BID"))
               touchStatus = " • 🔥 SNIPER REBOUND CONFIRMED!";
            else if(isResisten && (zoneDelta <= -10.0 || g_bookmapAbsorption == "ASK"))
               touchStatus = " • 🔥 SNIPER REJECTION CONFIRMED!";
            else if(isInside)
               touchStatus = " • 🎯 HARGA DI DALAM KOTAK!";
            else if(isGrazingWick)
               touchStatus = " • ⚡ GRAZING WICK!";
               
            // Auto Sniper Execution jika diaktifkan Dadang
            if(InpEnableBoxSniperTrap && (TimeCurrent() - g_lastSniperTradeTime > 300))
            {
               if(!isResisten && zoneDelta >= 15.0) // Demand Buy Rebound
               {
               ...
               else if(isResisten && zoneDelta <= -15.0) // Supply Sell Rejection
               {
                  double sl = top + 1.0;
                  double tp = curPrice - (slDistance * 3.5);
         ...
         string wallShort = (totalWall > 0.0) ? StringFormat("WALL %.0fL (%d)", totalWall, totalWalls) : "WALL 0L";
         string histShort = (histLot > 0.0) ? StringFormat("HIST %.0fL", histLot) : "HIST 0L";
         string patShort  = (patternTag != "") ? StringFormat("%s [%s]", patternTag, retestStr) : retestStr;
         
         // Cross-check dengan jejak Big Order ICT / SMC (Order Block, FVG, Liquidity Sweeps)
         string ictConfluence = CheckICT_BoxConfluence(bottom, top, isResisten);
         if(ictConfluence != "") qualityCategory = ictConfluence + " • " + qualityCategory;
         
         bool isLiqSwept = CheckLiquiditySweepOnBox(bottom, top, isResisten);
         if(isLiqSwept) touchStatus += " • ⚡ LIQ SWEEP CONFIRMED (SNIPER ENTRY!)";
         
         // FORMAT LABEL SUPER MINIMALIS & SULTAN CLEAN (DOKTRIN COMMANDER DADANG)
         string ageBadge = (maxWallAge >= 3600.0) ? " • 🧲 LTHL" : "";
         string labelText = StringFormat("%s %s [%s] : %s • %s%s",
                                         iconTag, sideName, objTf, wallShort, histShort, ageBadge);
         
         string lblName = "DADANG_BOX_LBL_" + name;
         // Presisi Titik Tengah Kotak (Time & Price Midpoint)
         datetime labelPosT = (datetime)((t1 + t2) / 2);
         double labelPosP = (top + bottom) * 0.5;
         ...
         ObjectSetInteger(0, lblName, OBJPROP_COLOR, qualityColor);
...
void DrawAllZones()
{
   ...
   {
      for(int _h = 0; _h < SD_MAX_ZONES; _h++) {
         HideZoneSlot(StringFormat("S%d", _h+1));
         HideZoneSlot(StringFormat("D%d", _h+1));
      }
      return;
   }
   ...
   // 1. Kumpulkan zona Supply (DI ATAS HARGA) dan Demand (DI BAWAH HARGA)
   double rawSLo[], rawSHi[], rawSLot[], rawSScore[]; int rawSRetest[], rawSAbsorb[]; string rawSStatus[]; int sCnt = 0;
   double rawDLo[], rawDHi[], rawDLot[], rawDScore[]; int rawDRetest[], rawDAbsorb[]; string rawDStatus[]; int dCnt = 0;

   for(int i = 0; i < g_zoneCount; i++)
   {
      if(g_zoneStatus[i] == "BROKEN") continue;
      double lo = g_zoneLo[i];
      double hi = g_zoneHi[i];

      // Kualitas: Hanya zona yang valid dan berbobot
      double boost = 0.0;
      string cmpTag = GetCmpConfluenceTag(lo, hi, boost);
      bool isMajor = (g_zoneScore[i] + boost >= 45.0 || g_zoneTotalLot[i] >= 80.0 || cmpTag != "");
      if(!isMajor) continue;

      // STRICT PARTITION: Supply strictly above price, Demand strictly below price
      if(lo >= price + 0.30) // SUPPLY (ATAS)
      {
         ArrayResize(rawSLo, sCnt + 1);    rawSLo[sCnt]    = lo;
         ArrayResize(rawSHi, sCnt + 1);    rawSHi[sCnt]    = hi;
         ArrayResize(rawSLot, sCnt + 1);   rawSLot[sCnt]   = g_zoneTotalLot[i];
         ArrayResize(rawSScore, sCnt + 1); rawSScore[sCnt] = g_zoneScore[i];
         ArrayResize(rawSRetest, sCnt + 1);rawSRetest[sCnt]= g_zoneRetest[i];
         ArrayResize(rawSAbsorb, sCnt + 1);rawSAbsorb[sCnt]= g_zoneAbsorb[i];
         ArrayResize(rawSStatus, sCnt + 1);rawSStatus[sCnt]= g_zoneStatus[i];
         sCnt++;
      }
      else if(hi <= price - 0.30) // DEMAND (BAWAH)
      {
         ArrayResize(rawDLo, dCnt + 1);    rawDLo[dCnt]    = lo;
         ArrayResize(rawDHi, dCnt + 1);    rawDHi[dCnt]    = hi;
         ArrayResize(rawDLot, dCnt + 1);   rawDLot[dCnt]   = g_zoneTotalLot[i];
         ArrayResize(rawDScore, dCnt + 1); rawDScore[dCnt] = g_zoneScore[i];
         ArrayResize(rawDRetest, dCnt + 1);rawDRetest[dCnt]= g_zoneRetest[i];
         ArrayResize(rawDAbsorb, dCnt + 1);rawDAbsorb[dCnt]= g_zoneAbsorb[i];
         ArrayResize(rawDStatus, dCnt + 1);rawDStatus[dCnt]= g_zoneStatus[i];
         dCnt++;
      }
   }

   // 2. Urutkan Supply dari harga running NAIK KE ATAS (S1 terdekat di lo paling kecil)
   for(int a = 1; a < sCnt; a++)
   {
      double kLo = rawSLo[a], kHi = rawSHi[a], kLot = rawSLot[a], kSc = rawSScore[a];
      int kRet = rawSRetest[a], kAbs = rawSAbsorb[a]; string kSt = rawSStatus[a];
      int b = a - 1;
      while(b >= 0 && rawSLo[b] > kLo)
      {
         rawSLo[b+1] = rawSLo[b]; rawSHi[b+1] = rawSHi[b]; rawSLot[b+1] = rawSLot[b];
         rawSScore[b+1] = rawSScore[b]; rawSRetest[b+1] = rawSRetest[b]; rawSAbsorb[b+1] = rawSAbsorb[b];
         rawSStatus[b+1] = rawSStatus[b];
         b--;
      }
      rawSLo[b+1] = kLo; rawSHi[b+1] = kHi; rawSLot[b+1] = kLot; rawSScore[b+1] = kSc;
      rawSRetest[b+1] = kRet; rawSAbsorb[b+1] = kAbs; rawSStatus[b+1] = kSt;
   }

   // 3. Urutkan Demand dari harga running TURUN KE BAWAH (D1 terdekat di hi paling besar)
   for(int a = 1; a < dCnt; a++)
   {
      double kLo = rawDLo[a], kHi = rawDHi[a], kLot = rawDLot[a], kSc = rawDScore[a];
      int kRet = rawDRetest[a], kAbs = rawDAbsorb[a]; string kSt = rawDStatus[a];
      int b = a - 1;
      while(b >= 0 && rawDHi[b] < kHi)
      {
         rawDLo[b+1] = rawDLo[b]; rawDHi[b+1] = rawDHi[b]; rawDLot[b+1] = rawDLot[b];
         rawDScore[b+1] = rawDScore[b]; rawDRetest[b+1] = rawDRetest[b]; rawDAbsorb[b+1] = rawDAbsorb[b];
         rawDStatus[b+1] = rawDStatus[b];
         b--;
      }
      rawDLo[b+1] = kLo; rawDHi[b+1] = kHi; rawDLot[b+1] = kLot; rawDScore[b+1] = kSc;
      rawDRetest[b+1] = kRet; rawDAbsorb[b+1] = kAbs; rawDStatus[b+1] = kSt;
   }

   // 4. FILTER KETAT ANTI-TUMPANG TINDIH: Wajib ada gap minimal 1.50 USD antar kotak!
   double sLo[], sHi[], sLot[], sScore[]; int sRet[], sAbs[]; string sSt[]; int sFinal = 0;
   for(int i = 0; i < sCnt && sFinal < SD_MAX_ZONES; i++)
   {
      ...
      ArrayResize(sSt, sFinal + 1);    sSt[sFinal]    = rawSStatus[i];
      sFinal++;
   }

   double dLo[], dHi[], dLot[], dScore[]; int dRet[], dAbs[]; string dSt[]; int dFinal = 0;
   for(int i = 0; i < dCnt && dFinal < SD_MAX_ZONES; i++)
   {
      ...
      ArrayResize(dSt, dFinal + 1);    dSt[dFinal]    = rawDStatus[i];
      dFinal++;
   }

   // 5. Simpan ke Roadmap Decision Engine
   for(int a = 0; a < sFinal && a < SDPM_MAX_ROADMAP; a++)
   {
      g_supplyRoadLo[a] = sLo[a]; g_supplyRoadHi[a] = sHi[a]; g_supplyRoadScore[a] = sScore[a];
      g_supplyRoadLot[a] = sLot[a];
      g_supplyRoadCount = a + 1;
   }
   for(int a = 0; a < dFinal && a < SDPM_MAX_ROADMAP; a++)
   {
      g_demandRoadLo[a] = dLo[a]; g_demandRoadHi[a] = dHi[a]; g_demandRoadScore[a] = dScore[a];
      g_demandRoadLot[a] = dLot[a];
      g_demandRoadCount = a + 1;
   }

   // 6. Gambar Tiap Zona Diskrit Ramping (S1..S6 & D1..D6)
   if(InpShowZonesOnChart)
   {
      for(int a = 0; a < SD_MAX_ZONES; a++)
      {
         if(a < sFinal) DrawZoneBox(a, "SUPPLY", sLo[a], sHi[a], sLot[a], sSt[a], sRet[a], sAbs[a], sScore[a]);
         else { HideZoneSlot(StringFormat("S%d", a+1)); g_supplyState[a].active = false; }
      }
      for(int a = 0; a < SD_MAX_ZONES; a++)
      {
...
{
   if(loc == "INSIDE_SUPPLY")        return "DI AREA RESISTEN (ATAS)";
   if(loc == "INSIDE_DEMAND")        return "DI AREA SUPORT (BAWAH)";
   if(loc == "NEAR_SUPPLY")          return "DEKAT RESISTEN (ATAS)";
   if(loc == "NEAR_DEMAND")          return "DEKAT SUPORT (BAWAH)";
   if(loc == "OUTSIDE_ACTIVE_RANGE") return "GAK ADA LEVEL VALID DI SEKITAR";
   return "DI ANTARA SUPORT & RESISTEN";
}

string ReasonText(string reason)
{
   if(reason == "SUPPLY_DEMAND_SQUEEZE_WAIT_FOR_BREAK") return "Suport & Resisten sempit (Konsolidasi) - tunggu jebol";
   if(reason == "DEMAND_REACTION_BUYER_LEAN")            return "Di Suport, Buyer dominan - siap Rebound BUY";
   if(reason == "SUPPLY_REACTION_SELLER_LEAN")           return "Di Resisten, Seller dominan - siap Rejection SELL";
   if(reason == "BUYER_SELLER_BALANCED")                 return "Buyer & Seller seimbang - belum ada arah jelas";
   if(reason == "APPROACHING_SUPPLY")                    return "Harga mendekati Resisten atas";
   if(reason == "APPROACHING_DEMAND")                    return "Harga mendekati Suport bawah";
   if(reason == "NO_VALID_ZONE")                         return "Gak ada level suport/resisten valid";
   return "Harga di antara level, tunggu respon";
}
...
{
   if(g_sdBrkState_S == "BREAK_CONFIRMED") return StringFormat("RESISTEN JEBOL (BUY, %s)", g_sdBrkMomentum_S);
   if(g_sdBrkState_D == "BREAK_CONFIRMED") return StringFormat("SUPORT JEBOL (SELL, %s)", g_sdBrkMomentum_D);
   if(g_sdMarketRead == "CHOPPY")          return "CHOPPY - WAIT";
   if(g_sdMarketRead == "COMPRESSION")     return "KONSOLIDASI SEMPIT";
   if(g_sdBrkState_S == "FOLLOW_THROUGH")  return "RESISTEN TEMBUS...";
   if(g_sdBrkState_D == "FOLLOW_THROUGH")  return "SUPORT TEMBUS...";
   if(g_sdBrkState_S == "MOMENTUM_BREAK")  return "TES RESISTEN (MOMENTUM)";
   if(g_sdBrkState_D == "MOMENTUM_BREAK")  return "TES SUPORT (MOMENTUM)";
   if(g_sdBrkState_S == "CLOSE_BREAK")     return "TES RESISTEN (CLOSE)";
   if(g_sdBrkState_D == "CLOSE_BREAK")     return "TES SUPORT (CLOSE)";
   if(g_sdBrkState_S == "WICK_BREAK")      return "TES RESISTEN (EKOR)";
   if(g_sdBrkState_D == "WICK_BREAK")      return "TES SUPORT (EKOR)";
   if(g_sdBrkState_S == "REJECTION")       return "REJEKSI RESISTEN (SELL)";
   if(g_sdBrkState_D == "REJECTION")       return "REBOUND SUPORT (BUY)";
   if(g_sdBrkState_S == "FALSE_BREAK")     return "FALSE BREAK RESISTEN";
   if(g_sdBrkState_D == "FALSE_BREAK")     return "FALSE BREAK SUPORT";
   if(g_sdBrkState_S == "RETEST")          return "RETEST RESISTEN (RBS)";
   if(g_sdBrkState_D == "RETEST")          return "RETEST SUPORT (SBR)";
   if(g_sdBrkState_S == "ZONE_TEST")       return "UJI RESISTEN";
   if(g_sdBrkState_D == "ZONE_TEST")       return "UJI SUPORT";
   return "NONE";
}
...
{
   // v53.76: Dadang - "YANG GAK TERAKUMULASI JANGAN NAMPIL SAMPAI TERAKUMULASI"
   // Hapus total semua garis eceran 10 lot (BID1..20 & ASK1..20) agar chart super bersih!
   for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
   {
      ObjectDelete(0, WALL_PREFIX + StringFormat("BID%d", i+1));
      ObjectDelete(0, WALL_PREFIX + StringFormat("BID%d_TXT", i+1));
      ObjectDelete(0, WALL_PREFIX + StringFormat("ASK%d", i+1));
      ObjectDelete(0, WALL_PREFIX + StringFormat("ASK%d_TXT", i+1));
   }
}

string FormatWallAge(double ageSec)
{
   if(ageSec >= 3600)
   {
      int h = (int)(ageSec / 3600);
      int m = (int)((ageSec - h * 3600) / 60);
      return StringFormat("%dj %dm", h, m);
   }
   if(ageSec >= 60)
   {
      int m = (int)(ageSec / 60);
   ...
   return StringFormat("%.0fs", ageSec);
}

void DrawWallZone(string sideName, int zoneIdx, double loPriceBm, double hiPriceBm, double totalLot, int wallCount, double maxAgeSec, double offset, bool isBid)
{
   string key      = WALL_PREFIX + WALL_ZONE_PREFIX + sideName + IntegerToString(zoneIdx);
   string lineName = key + "_LINE";
   string textName = key + "_TXT";

   double loPrice  = loPriceBm + offset, hiPrice = hiPriceBm + offset;
   double midPrice = (loPrice + hiPrice) / 2.0;
   double curPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double distUsd  = MathAbs(midPrice - curPrice);
   double distPips = distUsd * 10.0;
   bool   isFarMagnet = (distUsd >= 10.0);   // Wall jauh (> 10 USD / > 100 pips) adalah MAGNET LIKUIDITAS

   color clr;
   if(isFarMagnet)
   {
      // Warna Magnet Likuiditas Jauh: Cyan untuk Bid Magnet Bawah, Gold/Orange untuk Ask Magnet Atas
      clr = isBid ? C'0,220,255' : C'255,170,0';
   }
   else
   {
      // Warna Area Dekat: Emerald untuk Bid Support, Crimson untuk Ask Resistance
      clr = isBid ? C'0,230,130' : C'255,50,75';
   }

   int lineWidth = (totalLot >= 150.0) ? 4 : ((totalLot >= 100.0) ? 3 : 2);
   string ageStr = FormatWallAge(maxAgeSec);

   string labelText = "";
   if(isFarMagnet)
   {
      labelText = StringFormat(" 🧲 %s MAGNET [%s]: %.0f LOT @%.2f (Jarak: +%.1f$ / %.0fp)", 
                               sideName, ageStr, totalLot, midPrice, distUsd, distPips);
   }
   else
   {
      labelText = StringFormat(" 🛡️ %s WALL [%s]: %.0f LOT [%.2f - %.2f]", 
                               sideName, ageStr, totalLot, loPrice, hiPrice);
   }

   datetime t1 = TimeCurrent() - PeriodSeconds(_Period) * 25;
   datetime t2 = TimeCurrent() + PeriodSeconds(_Period) * 15;

   // 1. Kotak Tipis Bounding Area
   if(ObjectFind(0, key) < 0)
   {
      ...
      ObjectSetInteger(0, key, OBJPROP_STYLE, isFarMagnet ? STYLE_DASH : STYLE_SOLID);
   }
   ObjectSetInteger(0, key, OBJPROP_TIME, 0, t1);
   ObjectSetDouble(0, key, OBJPROP_PRICE, 0, hiPrice);
   ObjectSetInteger(0, key, OBJPROP_TIME, 1, t2);
   ObjectSetDouble(0, key, OBJPROP_PRICE, 1, loPrice);
   ...
   // 2. Garis Horizontal Utama di Level Tengah (Center Line)
   if(ObjectFind(0, lineName) < 0)
   {
      ObjectCreate(0, lineName, OBJ_HLINE, 0, 0, midPrice);
      ObjectSetInteger(0, lineName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, lineName, OBJPROP_HIDDEN, true);
      ObjectSetInteger(0, lineName, OBJPROP_BACK, true);
   }
   ObjectSetDouble(0, lineName, OBJPROP_PRICE, midPrice);
   ObjectSetInteger(0, lineName, OBJPROP_COLOR, clr);
   ObjectSetInteger(0, lineName, OBJPROP_STYLE, isFarMagnet ? STYLE_DOT : STYLE_SOLID);
   ObjectSetInteger(0, lineName, OBJPROP_WIDTH, lineWidth);
   ObjectSetString(0, lineName, OBJPROP_TEXT, labelText);

   // 3. Label Teks Tebal Putih
   datetime labelTime = TimeCurrent() + PeriodSeconds(_Period) * 2;
   if(ObjectFind(0, textName) < 0)
   {
   ...
   ObjectSetInteger(0, textName, OBJPROP_TIME, labelTime);
   ObjectSetDouble(0, textName, OBJPROP_PRICE, midPrice);
   ObjectSetInteger(0, textName, OBJPROP_COLOR, clrWhite);
   ObjectSetInteger(0, textName, OBJPROP_FONTSIZE, 11 + (lineWidth - 1));
   ObjectSetString(0, textName, OBJPROP_TEXT, labelText);
}

void ClusterAndDrawWallZones(double &pxArr[], double &szArr[], double &ageArr[], int n, double offset, bool isBid, string sideName, bool &clusteredOut[])
{
   ObjectsDeleteAll(0, WALL_PREFIX + WALL_ZONE_PREFIX + sideName);
   ArrayInitialize(clusteredOut, false);

   double cp[]; double cs[]; double ca[]; int ci[];
   ArrayResize(cp, n); ArrayResize(cs, n); ArrayResize(ca, n); ArrayResize(ci, n);
   int cnt = 0;
   for(int i = 0; i < n; i++)
   {
      if(pxArr[i] <= 0 || szArr[i] <= 0) continue;
      cp[cnt] = pxArr[i]; cs[cnt] = szArr[i]; ca[cnt] = (i < ArraySize(ageArr)) ? ageArr[i] : 0; ci[cnt] = i;
      cnt++;
   }
   if(cnt == 0) return;
   ...
   for(int i = 1; i < cnt; i++)
   {
      double kp = cp[i], ks = cs[i], ka = ca[i]; int ki = ci[i];
      int j = i - 1;
      while(j >= 0 && cp[j] > kp) { cp[j+1] = cp[j]; cs[j+1] = cs[j]; ca[j+1] = ca[j]; ci[j+1] = ci[j]; j--; }
      cp[j+1] = kp; cs[j+1] = ks; ca[j+1] = ka; ci[j+1] = ki;
   }
   ...
   {
      int j = i;
      // Tight cluster width: max $2.5 USD range
      ...
      { 
         totalLot += cs[k]; 
         if(ca[k] > maxAge) maxAge = ca[k];
      }
      
      // AKUMULASI: Gambar jika akumulasi area mencapai minimal InpWallMinDrawLot (>= 50 Lot) ATAU wall tunggal jauh >= 40 lot
      if(totalLot >= InpWallMinDrawLot || (j == i && cs[i] >= 40.0))
      {
         zoneIdx++;
         for(int k = i; k <= j; k++) { clusteredOut[ci[k]] = true; }
         DrawWallZone(sideName, zoneIdx, cp[i], cp[j], totalLot, j - i + 1, maxAge, offset, isBid);
      }
      i = j + 1;
   }
}
...
{
   // ULTRA CLEAN CHART: Bersihkan tumpukan garis/kotak otomatis yang bikin chart M5 sempit/mepet.
   // Hanya tampilkan 2 Garis Mega Wall Speedometer Ramping dan Kotak Discretionary Dadang.
...
{
   if(!Diag_BookmapVisual) { ObjectDelete(0, WALL_PREFIX + "SWEEP_FALLBACK"); ObjectDelete(0, WALL_PREFIX + "SWEEP_FALLBACK_TXT"); return; }
...
void UpdatePocLine()
{
   if(!Diag_BookmapVisual) { ObjectDelete(0, WALL_PREFIX + "POC"); ObjectDelete(0, WALL_PREFIX + "POC_TXT"); return; }
   if(!ShowProfileLevels || !g_togglePocVa)
   {
      ObjectDelete(0, WALL_PREFIX + "POC");
      ObjectDelete(0, WALL_PREFIX + "POC_TXT");
      ObjectDelete(0, WALL_PREFIX + "VAH");
      ObjectDelete(0, WALL_PREFIX + "VAH_TXT");
      ObjectDelete(0, WALL_PREFIX + "VAL");
      ObjectDelete(0, WALL_PREFIX + "VAL_TXT");
      return;
   }
...
void WriteSultanStatus()
{
   ...
   // v53.56 - Sierra Chart Pro Order Flow Suite export. Straight passthrough
   // of g_sc*/g_scVirginPoc* state, set at the SAME point each Update*Engine()
   // decides to draw on the MT5 chart (see RunSierraChartSuite() and its 6
   // callees) - web chart never recomputes these conditions itself. Dadang:
   // "di candle juga samain bro sierra chart kita dong".
   color scShapeClrUnused; string scShapeTxt = DailyProfileFramingText(scShapeClrUnused);
   json += "\"sierra_chart\":{";
   json += StringFormat("\"imbalance_side\":\"%s\",", g_scImbalanceSide);
   json += StringFormat("\"naked_poc_active\":%s,\"naked_poc_price\":%.2f,",
                         g_scVirginPocActive ? "true" : "false", g_scVirginPocPrice);
   json += StringFormat("\"divergence_type\":\"%s\",", g_scDivergenceType);
   json += StringFormat("\"whale_side\":\"%s\",\"whale_lots\":%.0f,", g_scWhaleSide, g_scWhaleLots);
   json += StringFormat("\"unfinished_found\":%s,\"unfinished_price\":%.2f,\"unfinished_time\":%d,",
                         g_scUnfinishedFound ? "true" : "false", g_scUnfinishedPrice, (long)g_scUnfinishedTime);
   json += StringFormat("\"profile_shape\":\"%s\"", scShapeTxt);
   json += "},";

   // v53.57 - Confluence Radar export. Dadang: "isi aja bro karena gw
   // fokusnya di chart [web] sekarang" - dulu web pakai `conviction`
   // (chain-depth scorer, beda metric) buat banner "RADAR KONFLUENSI"-nya,
   // padahal itu nama yang sama kayak banner MT5 yang beda sumber
   // (CalculateConfluenceScore(), 0-100 multi-factor). Sekarang export
   // yang ASLI - tradeDir resolution SAMA PERSIS kayak ConfluenceRadarText()/
   // UpdateTopCenterConfluenceHUD() biar angka di web = angka di MT5, gak
   // dihitung ulang beda cara.
   string cfTradeDir = (g_scalpEntryDir != "WAIT") ? g_scalpEntryDir : g_scalpMasterDir;
   if(cfTradeDir == "WAIT") cfTradeDir = g_masterDir;
   string cfGrade = "", cfReasons = "";
   double cfScore = CalculateConfluenceScore(cfTradeDir, cfGrade, cfReasons);
...
{
   if(!Diag_BookmapVisual) { ObjectDelete(0, WALL_PREFIX + "VAH"); ObjectDelete(0, WALL_PREFIX + "VAH_TXT"); ObjectDelete(0, WALL_PREFIX + "VAL"); ObjectDelete(0, WALL_PREFIX + "VAL_TXT"); return; }
...
void DrawValueAreaLine(string name, string textName, double mt5Price, string label, int marginPx = 20)
{
   ...
   double curPx = ObjectGetDouble(0, name, OBJPROP_PRICE, 0);
   if(MathAbs(curPx - mt5Price) > 0.05)
   {
      ObjectMove(0, name, 0, 0, mt5Price);
      ObjectSetDouble(0, name, OBJPROP_PRICE, mt5Price);
   ...
   if(!EnsureObject(textName, OBJ_TEXT)) return;

   double curTxtPx = ObjectGetDouble(0, textName, OBJPROP_PRICE, 0);
   if(MathAbs(curTxtPx - mt5Price) > 0.05)
   {
      ObjectMove(0, textName, 0, labelTime, mt5Price);
      ObjectSetDouble(0, textName, OBJPROP_PRICE, mt5Price);
   }
   ObjectSetInteger(0, textName, OBJPROP_TIME, labelTime);
...
{
   if(!Diag_BookmapVisual) { ObjectDelete(0, WALL_PREFIX + "BIDICE"); ObjectDelete(0, WALL_PREFIX + "BIDICE_TXT"); ObjectDelete(0, WALL_PREFIX + "ASKICE"); ObjectDelete(0, WALL_PREFIX + "ASKICE_TXT"); return; }
...
//+------------------------------------------------------------------+
//| DADANG ON-CHART SNIPER ENTRY BADGE                               |
//+------------------------------------------------------------------+
void DrawSniperEntryBadge(string dir, string tag, double price, double lot, double sl, double tp)
{
   datetime tPos = TimeCurrent();
   string badgeName = StringFormat("DD_ENTRY_BADGE_%d", (int)tPos);
   bool isBuy = (dir == "BUY");
   color badgeClr = isBuy ? C'0,230,120' : C'255,80,80';
   string icon = isBuy ? "🎯 [SNIPER BUY]" : "🎯 [SNIPER SELL]";
   
   string badgeText = StringFormat("%s %.2fL @%.2f | SL:%.2f TP:%.2f", icon, lot, price, sl, tp);
   
   double offsetPip = 1.5 * PipSize();
   double plotPrice = isBuy ? (price - offsetPip * 2.0) : (price + offsetPip * 2.0);
   ENUM_ANCHOR_POINT anchor = isBuy ? ANCHOR_UPPER : ANCHOR_LOWER;
   
   if(ObjectFind(0, badgeName) < 0)
   {
      ObjectCreate(0, badgeName, OBJ_TEXT, 0, tPos, plotPrice);
   }
   ObjectSetString(0, badgeName, OBJPROP_TEXT, badgeText);
   ObjectMove(0, badgeName, 0, tPos, plotPrice);
   ObjectSetInteger(0, badgeName, OBJPROP_COLOR, badgeClr);
   ObjectSetInteger(0, badgeName, OBJPROP_FONTSIZE, 10);
   ObjectSetString(0, badgeName, OBJPROP_FONT, "Segoe UI Black");
   ObjectSetInteger(0, badgeName, OBJPROP_ANCHOR, anchor);
...
bool TryOpen(string dir, string tag, bool isScalp, datetime parentFlipTime = 0, datetime childFlipTime = 0, double slOverride = 0.0, double tpOverride = 0.0)
{
   ...
   // MARKET_READING_LOGIC_Konsep_Murni.md Bagian 2 - CF = boleh nambah
   // posisi, VR = TIDAK boleh nambah posisi (biarpun lagi floating plus).
   // Ini cuma buat LAYER TAMBAHAN di atas posisi yang udah ada - entry
   // PERTAMA di arah manapun tetap selalu boleh (VR bukan larangan entry,
   // cuma mode risiko yang beda - liat Bagian 2 & 4 dokumen).
   if(CountOpenPositions(dir) > 0)
   {
      bool dirOpposesD1 = (dir == "BUY" && g_d1LastDir == "SELL") || (dir == "SELL" && g_d1LastDir == "BUY");
      if(dirOpposesD1) return false;
   }
   ...
   if(trade.PositionOpen(_Symbol, orderType, lot, px, sl, tp, comment))
   {
      // 1. Gambar Badge Sniper Visual langsung di atas/bawah lilin eksekusi
      DrawSniperEntryBadge(dir, tag, px, lot, sl, tp);
      
      // 2. Alert Audio, Modal Popup & Mobile Notification ke Android
      string alertMsg = StringFormat("🚀 [DD-V3 ENTRY %s]: %s (%.2fL) @ %.2f | SL=%.2f | TP=%.2f", dir, tag, lot, px, sl, tp);
      Alert(alertMsg);
      SendNotification(alertMsg);
      Print("✅ SUCCESS ENTRY (", tag, ") ", dir, " lot=", lot, " @ ", px, " SL=", sl, " TP=", tp);
      return true;
   }
   Print("❌ ENTRY FAILED (", tag, ") ", dir, " Code=", trade.ResultRetcode(), " Desc=", trade.ResultRetcodeDescription());
   return false;
}
...
{
   // CHAIN SIGNAL DIBUANG DARI CHART PER PERINTAH DADANG (TIDAK PRESISI / CLUTTER)
   return;
}
...
{
   // Bersihkan semua objek Chain Signal & Fail lama dari chart
   ObjectsDeleteAll(0, "DD_WALL_CHAINSIG_");
   ObjectsDeleteAll(0, "DD_WALL_CHAINFAIL_");
   ObjectsDeleteAll(0, "CHAINSIG_");
   ObjectsDeleteAll(0, "CHAINFAIL_");
}
...
datetime g_sdBreakoutLastEventTime = 0;
...
void CheckZoneBreakoutEntry()
{
   ...
   // 1. VETO MUTLAK: Zone Compression (<1 USD) -> NO TRADE ZONE!
   if(ZoneCompressionActive()) return;

   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   datetime curBarTime = iTime(_Symbol, InpScalpEntryTF, 0);

   // =========================================================================
   // STRATEGI 1: S&D BREAKOUT CONTINUATION (ZONA JEBOL SAH DENGAN CLOSE + CVD + FOOTPRINT)
   // =========================================================================
   if(curBarTime != g_sdBreakoutLastEventTime)
   {
      // --- KASUS A: SUPPLY S1 JEBOL NAIK (BREAK_CONFIRMED) ---
      if(g_sdBrkState_S == "BREAK_CONFIRMED")
      {
         bool cvdOk = (!g_bookmapOnline || g_bookmapCvd >= 0.0 || g_bookmapPulsePct >= 50.0);
         bool footOk = (!g_bookmapOnline || g_bookmapFootBuyVol >= g_bookmapFootSellVol);
         bool volOk = (g_sdmBuyerPct >= 48.0);

         if(cvdOk && footOk && volOk)
         {
            double tpEdge = 0.0, tpScore = 0.0;
            // Target ke Supply S2 di atasnya
            bool haveTp = NearestOpposingZone(true, price + 1.0, tpEdge, tpScore);
            double tp = haveTp ? tpEdge : 0.0;

            if(TryOpen("BUY", "S1_BREAKOUT_CONFLUENCE", false, 0, 0, 0.0, tp))
            {
               g_sdBreakoutLastEventTime = curBarTime;
               Print("🟢 [S1 BREAKOUT BUY ENTRY]: Price=", price, " | CVD=", g_bookmapCvd, 
                     " | Buyer%=", g_sdmBuyerPct, "% | FootBuy=", g_bookmapFootBuyVol, " | TP=", tp);
               return;
            }
         }
      }

      // --- KASUS B: DEMAND D1 JEBOL TURUN (BREAK_CONFIRMED) ---
      if(g_sdBrkState_D == "BREAK_CONFIRMED")
      {
         bool cvdOk = (!g_bookmapOnline || g_bookmapCvd <= 0.0 || g_bookmapPulsePct <= 50.0);
         bool footOk = (!g_bookmapOnline || g_bookmapFootSellVol >= g_bookmapFootBuyVol);
         bool volOk = (g_sdmSellerPct >= 48.0);

         if(cvdOk && footOk && volOk)
         {
            double tpEdge = 0.0, tpScore = 0.0;
            // Target ke Demand D2 di bawahnya
            bool haveTp = NearestOpposingZone(false, price - 1.0, tpEdge, tpScore);
            double tp = haveTp ? tpEdge : 0.0;

            if(TryOpen("SELL", "D1_BREAKDOWN_CONFLUENCE", false, 0, 0, 0.0, tp))
            {
               g_sdBreakoutLastEventTime = curBarTime;
               Print("🔴 [D1 BREAKDOWN SELL ENTRY]: Price=", price, " | CVD=", g_bookmapCvd, 
                     " | Seller%=", g_sdmSellerPct, "% | FootSell=", g_bookmapFootSellVol, " | TP=", tp);
               return;
            }
         }
      }
   ...
   // =========================================================================
   // STRATEGI 2: S&D BOUNCE / REVERSAL DI DALAM ZONA (M5 BREAKOUT + ORDER-FLOW)
   // =========================================================================
   datetime evtTime = ReadBreakoutEventTime(g_hScalpEntry);
   ...
   {
      // VETO: BUY di Demand wajib Buyer >= 50%, SELL di Supply wajib Seller >= 50%
      if(m5Dir == "BUY"  && g_sdmBuyerPct < 50.0) return;
      if(m5Dir == "SELL" && g_sdmSellerPct < 50.0) return;

      double tpEdge = 0.0, tpScore = 0.0;
      bool haveTp = NearestOpposingZone(!wantSupply, price, tpEdge, tpScore);
      if(haveTp && MathAbs(tpEdge - price) < 1.50) return; // Reward terlalu sempit

      double tp = haveTp ? tpEdge : 0.0;

      if(TryOpen(m5Dir, "ZONE_BOUNCE_SNIPER", false, 0, 0, 0.0, tp))
      {
         string tpNote = haveTp ? StringFormat(" | TP=zona lawan %.2f (score %.0f)", tpEdge, tpScore)
                                  : " | TP=fallback pip";
         Print("🎯 [ZONE BOUNCE SNIPER]: ", m5Dir, " @ ", price,
               " | zone=", zLo, "-", zHi, " (score ", zScore, ") | BuyerPct=", g_sdmBuyerPct, "%", tpNote);
      }
   }
}
...
void ClosePositionsOppositeTo(string newDir)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // MARKET_READING_LOGIC_Konsep_Murni.md Bagian 4 - root (D1) menang atas
      // local cascade. H4 sendiri cuma "aligned" beneran kalau D1 (root)
      // gak melawannya - kalau H4 sendiri lagi jadi VR terhadap D1, cascade
      // M30-searah-H4 ini gak punya backing root, jadi JANGAN dikasih
      // proteksi hold yang sama kayak kalau H4 punya D1 di belakangnya.
      bool h4OpposesD1 = (g_masterDir == "BUY" && g_d1LastDir == "SELL") ||
                         (g_masterDir == "SELL" && g_d1LastDir == "BUY");

      bool shouldCutLoss = true;
      if(positionWasAlignedWithH4 && !h1OpposesH4 && !h4OpposesD1)
      {
         shouldCutLoss = false; // HOLD! H1 belum flip lawan H4, DAN H4 sendiri masih align/netral ke D1 (root)
      }
...
void UpdateStateAndExecute()
{
   ...
   if(m5EventTime != g_lastM5EventTime)
   {
      ...
      // v53.80: BLIND CHAIN ENTRIES DIHAPUS TOTAL PER PERINTAH DADANG!
      // EA TIDAK BOLEH MEMBUKA ORDER TANPA DI AREA S&D / SUPORT & RESISTEN ASLI.
      // Seluruh eksekusi sekarang hanya melalui:
      // 1. CheckZoneBreakoutEntry() (Entri di Area S&D Riil)
      // 2. InpEnableBoxSniperTrap (Entri Kotak Suport & Resisten Dadang + Delta Rebound)
      // 3. CheckVaRetestTrigger() (Retest Value Area VAH/VAL/POC)
      // 4. CheckFusionH1H4Trigger() (Fusion H1/H4)
      if(false)
      {
         // Disabled legacy blind cascade
      }
   }
}
...
//+------------------------------------------------------------------+
//| v53.30: Dadang - "Garis SND Bookmap untuk Area-Area Penting Saja"
//| Gambar Garis Horizontal Bersih & Tegas untuk S&D Bookmap Utama
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.30: Dadang - "Demand Wajib di Bawah Harga, Supply Wajib di Atas Harga"
//| Penentuan Warna & Sisi 100% Berdasarkan Posisi Nyata Terhadap Harga Running
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| v53.30: Dadang - "Hapus kebalik-balik/selang-seling warna"
//| ULTIMATE S&D PARTITION:
//| - SEMUA DI ATAS HARGA = 100% MERAH (SUPPLY 1, 2, 3)
//| - SEMUA DI BAWAH HARGA = 100% HIJAU (DEMAND 1, 2, 3)
//| - TIDAK AKAN PERNAH BISA SELANG-SELING ATAU TERTUKAR!
//+------------------------------------------------------------------+
void DrawBookmapSndLines()
{
   if(!g_toggleSnd) { ObjectsDeleteAll(0, "DD_BM_SND_"); return; }
   if(!g_zoneDataAvailable || g_zoneCount == 0) return;

   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   ObjectsDeleteAll(0, "DD_BM_SND_");

   // 1. Kumpulkan level harga unik di atas dan di bawah harga
   double abovePx[], aboveLot[], aboveScore[];
   int aboveCnt = 0;

   double belowPx[], belowLot[], belowScore[];
   int belowCnt = 0;

   for(int i = 0; i < g_zoneCount; i++)
   {
      if(g_zoneStatus[i] == "BROKEN") continue;
      if(g_zoneScore[i] < 60.0 && g_zoneTotalLot[i] < 120.0) continue;

      double targetLo = g_zoneLo[i];
      double targetHi = g_zoneHi[i];
      double mid = (targetLo + targetHi) * 0.5;

      if(mid > price) // DI ATAS HARGA = JADI SUPPLY
      {
         double px = targetLo; // Tepi bawah supply yang paling dekat dengan harga
         bool dup = false;
         for(int k = 0; k < aboveCnt; k++)
         {
            if(MathAbs(abovePx[k] - px) < 2.5) { dup = true; break; }
         }
         if(!dup)
         {
            ArrayResize(abovePx, aboveCnt + 1);    abovePx[aboveCnt]    = px;
            ArrayResize(aboveLot, aboveCnt + 1);   aboveLot[aboveCnt]   = g_zoneTotalLot[i];
            ArrayResize(aboveScore, aboveCnt + 1); aboveScore[aboveCnt] = g_zoneScore[i];
            aboveCnt++;
         }
      }
      else // DI BAWAH HARGA = JADI DEMAND
      {
         double px = targetHi; // Tepi atas demand yang paling dekat dengan harga
         bool dup = false;
         for(int k = 0; k < belowCnt; k++)
         {
            if(MathAbs(belowPx[k] - px) < 2.5) { dup = true; break; }
         }
         if(!dup)
         {
            ArrayResize(belowPx, belowCnt + 1);    belowPx[belowCnt]    = px;
            ArrayResize(belowLot, belowCnt + 1);   belowLot[belowCnt]   = g_zoneTotalLot[i];
            ArrayResize(belowScore, belowCnt + 1); belowScore[belowCnt] = g_zoneScore[i];
            belowCnt++;
         }
      }
   }

   // 2. Urutkan Supply di atas harga (dari yang paling dekat dengan harga naik ke atas)
   for(int a = 1; a < aboveCnt; a++)
   {
      double keyPx = abovePx[a], keyLot = aboveLot[a], keyScore = aboveScore[a];
      int b = a - 1;
      while(b >= 0 && abovePx[b] > keyPx)
      {
         abovePx[b+1] = abovePx[b]; aboveLot[b+1] = aboveLot[b]; aboveScore[b+1] = aboveScore[b];
         b--;
      }
      abovePx[b+1] = keyPx; aboveLot[b+1] = keyLot; aboveScore[b+1] = keyScore;
   }

   // 3. Urutkan Demand di bawah harga (dari yang paling dekat dengan harga turun ke bawah)
   for(int a = 1; a < belowCnt; a++)
   {
      double keyPx = belowPx[a], keyLot = belowLot[a], keyScore = belowScore[a];
      int b = a - 1;
      while(b >= 0 && belowPx[b] < keyPx)
      {
         belowPx[b+1] = belowPx[b]; belowLot[b+1] = belowLot[b]; belowScore[b+1] = belowScore[b];
         b--;
      }
      belowPx[b+1] = keyPx; belowLot[b+1] = keyLot; belowScore[b+1] = keyScore;
   }

   // 4. Gambar Garis SUPPLY (100% MERAH SOLID DI ATAS HARGA)
   for(int a = 0; a < aboveCnt && a < 3; a++)
   {
      string hlineName = StringFormat("DD_BM_SND_SUPPLY_%d", a + 1);
      string txtName   = hlineName + "_TXT";
      color clr = C'255,50,50'; // MERAH SOLID

      if(ObjectFind(0, hlineName) < 0) ObjectCreate(0, hlineName, OBJ_HLINE, 0, 0, abovePx[a]);
      else ObjectMove(0, hlineName, 0, 0, abovePx[a]);

      ObjectSetInteger(0, hlineName, OBJPROP_COLOR, clr);
      ObjectSetInteger(0, hlineName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSetInteger(0, hlineName, OBJPROP_WIDTH, (a == 0 ? 3 : 2));
      ObjectSetInteger(0, hlineName, OBJPROP_BACK, false);
      ObjectSetInteger(0, hlineName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, hlineName, OBJPROP_HIDDEN, true);

      datetime t = TimeCurrent() + PeriodSeconds(_Period) * 3;
      if(ObjectFind(0, txtName) < 0) ObjectCreate(0, txtName, OBJ_TEXT, 0, t, abovePx[a]);
      else ObjectMove(0, txtName, 0, t, abovePx[a]);

      ObjectSetString(0, txtName, OBJPROP_TEXT, StringFormat("🟥 [SUPPLY %d] %.2f [🏰 %.0fL • %s %.0f]",
                                                            a + 1, abovePx[a], aboveLot[a],
                                                            ZoneStrengthID(aboveScore[a]), aboveScore[a]));
      ObjectSetInteger(0, txtName, OBJPROP_COLOR, clr);
      ObjectSetInteger(0, txtName, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, txtName, OBJPROP_FONT, "Segoe UI Black");
      ObjectSetInteger(0, txtName, OBJPROP_ANCHOR, ANCHOR_LEFT);
   ...
   // 5. Gambar Garis DEMAND (100% HIJAU SOLID DI BAWAH HARGA)
   for(int a = 0; a < belowCnt && a < 3; a++)
   {
      string hlineName = StringFormat("DD_BM_SND_DEMAND_%d", a + 1);
      string txtName   = hlineName + "_TXT";
      color clr = C'0,255,128'; // HIJAU SOLID

      if(ObjectFind(0, hlineName) < 0) ObjectCreate(0, hlineName, OBJ_HLINE, 0, 0, belowPx[a]);
      else ObjectMove(0, hlineName, 0, 0, belowPx[a]);
      ...
      datetime t = TimeCurrent() + PeriodSeconds(_Period) * 3;
      if(ObjectFind(0, txtName) < 0) ObjectCreate(0, txtName, OBJ_TEXT, 0, t, belowPx[a]);
      else ObjectMove(0, txtName, 0, t, belowPx[a]);

      ObjectSetString(0, txtName, OBJPROP_TEXT, StringFormat("🟩 [DEMAND %d] %.2f [🏰 %.0fL • %s %.0f]",
                                                            a + 1, belowPx[a], belowLot[a],
                                                            ZoneStrengthID(belowScore[a]), belowScore[a]));
      ObjectSetInteger(0, txtName, OBJPROP_COLOR, clr);
      ObjectSetInteger(0, txtName, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, txtName, OBJPROP_FONT, "Segoe UI Black");
      ObjectSetInteger(0, txtName, OBJPROP_ANCHOR, ANCHOR_LEFT);
...
//+------------------------------------------------------------------+
//| v53.30: Dadang - "Tombol ON/OFF Mini Panel di Chart untuk Semua Garis"
//| Interactive HUD Switchboard: Klik tombol langsung di chart MT5
//| untuk Nyalakan/Matikan S&D, CMP, WALL, POC/VA secara instan!
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| DADANG INTERACTIVE HUD: TOMBOL HIDE / UNHIDE POC, VAH, VAL      |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| DADANG INTERACTIVE HUD: TOMBOL HIDE / UNHIDE PANEL & POC/VA      |
//+------------------------------------------------------------------+
bool g_toggleSnd       = true;  // S&D Bookmap Major Lines
bool g_toggleCmp       = true;  // D1/H4/H1/M30 CMP Lines
bool g_toggleWalls     = true;  // Live Bookmap Bid/Ask Walls & Zones
bool g_togglePocVa     = false; // Default HIDE POC/VA (0 garis!), klik tombol untuk UNHIDE!
bool g_toggleShowPanel = true;  // Default SHOW Panel Kiri, klik tombol untuk HIDE (Chart Plong)!
 // Default HIDE (0 garis di chart!), klik tombol untuk UNHIDE!

void UpdateHudToggleButtons()
{
   int btnW = 115;
   int btnH = 26;
   int yPos = 15;
   
   // 1. TOMBOL PANEL HIDE / UNHIDE (X=365 dari kanan)
   string pnlBtn = "DD_BTN_PANEL";
   int pnlX = 365;
   if(ObjectFind(0, pnlBtn) < 0)
   {
      ObjectCreate(0, pnlBtn, OBJ_BUTTON, 0, 0, 0);
      ObjectSetInteger(0, pnlBtn, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
      ObjectSetInteger(0, pnlBtn, OBJPROP_XDISTANCE, pnlX);
      ObjectSetInteger(0, pnlBtn, OBJPROP_YDISTANCE, yPos);
      ObjectSetInteger(0, pnlBtn, OBJPROP_XSIZE, btnW);
      ObjectSetInteger(0, pnlBtn, OBJPROP_YSIZE, btnH);
      ObjectSetInteger(0, pnlBtn, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, pnlBtn, OBJPROP_FONT, "Segoe UI Bold");
      ObjectSetInteger(0, pnlBtn, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, pnlBtn, OBJPROP_HIDDEN, false);
      ObjectSetInteger(0, pnlBtn, OBJPROP_ZORDER, 9999);
      ObjectSetInteger(0, pnlBtn, OBJPROP_BACK, false);
   }
   else
   {
      ObjectSetInteger(0, pnlBtn, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
   ...
   string pnlTxt = g_toggleShowPanel ? "📋 PANEL: SHOW" : "📋 PANEL: HIDE";
   color pnlBg   = g_toggleShowPanel ? C'0,180,255' : C'25,28,35'; // Cyan aktif / Dark Charcoal
   color pnlTxtC = g_toggleShowPanel ? clrBlack : C'220,225,235';
   color pnlBrd  = g_toggleShowPanel ? clrWhite : C'80,90,110';
   ObjectSetString(0, pnlBtn, OBJPROP_TEXT, pnlTxt);
   ObjectSetInteger(0, pnlBtn, OBJPROP_BGCOLOR, pnlBg);
   ObjectSetInteger(0, pnlBtn, OBJPROP_COLOR, pnlTxtC);
   ObjectSetInteger(0, pnlBtn, OBJPROP_BORDER_COLOR, pnlBrd);
   ObjectSetInteger(0, pnlBtn, OBJPROP_STATE, g_toggleShowPanel);

   // 2. TOMBOL POC/VA HIDE / UNHIDE (X=240 dari kanan)
   string pocBtn = "DD_BTN_POC_VA";
   int pocX = 240;
   if(ObjectFind(0, pocBtn) < 0)
   {
      ObjectCreate(0, pocBtn, OBJ_BUTTON, 0, 0, 0);
      ObjectSetInteger(0, pocBtn, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
      ObjectSetInteger(0, pocBtn, OBJPROP_XDISTANCE, pocX);
      ObjectSetInteger(0, pocBtn, OBJPROP_YDISTANCE, yPos);
      ObjectSetInteger(0, pocBtn, OBJPROP_XSIZE, btnW);
      ObjectSetInteger(0, pocBtn, OBJPROP_YSIZE, btnH);
      ObjectSetInteger(0, pocBtn, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, pocBtn, OBJPROP_FONT, "Segoe UI Bold");
      ObjectSetInteger(0, pocBtn, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, pocBtn, OBJPROP_HIDDEN, false);
      ObjectSetInteger(0, pocBtn, OBJPROP_ZORDER, 9999);
      ObjectSetInteger(0, pocBtn, OBJPROP_BACK, false);
   }
   else
   {
      ObjectSetInteger(0, pocBtn, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
   ...
   string pocTxt = g_togglePocVa ? "📊 POC/VA: SHOW" : "📊 POC/VA: HIDE";
   color pocBg   = g_togglePocVa ? C'255,190,40' : C'25,28,35'; // Gold aktif / Dark Charcoal
   color pocTxtC = g_togglePocVa ? clrBlack : C'220,225,235';
   color pocBrd  = g_togglePocVa ? clrWhite : C'80,90,110';
   ObjectSetString(0, pocBtn, OBJPROP_TEXT, pocTxt);
   ObjectSetInteger(0, pocBtn, OBJPROP_BGCOLOR, pocBg);
   ObjectSetInteger(0, pocBtn, OBJPROP_COLOR, pocTxtC);
   ObjectSetInteger(0, pocBtn, OBJPROP_BORDER_COLOR, pocBrd);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_MOUSE_MOVE)
   {
      ...
      if(leftButtonDown)
      {
         ...
         {
            if(mouseX >= g_cockpitX && mouseX <= g_cockpitX + 720 && mouseY >= g_cockpitY && mouseY <= g_cockpitY + 40)
            {
               g_isDraggingCockpit = true;
               g_dragOffsetX = mouseX - g_cockpitX;
               g_dragOffsetY = mouseY - g_cockpitY;
            }
         }
         else
         {
            g_cockpitX = MathMax(5, mouseX - g_dragOffsetX);
            g_cockpitY = MathMax(5, mouseY - g_dragOffsetY);
            UpdatePanel();
         }
      }
      else
      {
         g_isDraggingCockpit = false;
      }
   }
   
   // 2. Click Events (Canvas Hit Testing)
   else if(id == CHARTEVENT_CLICK)
   {
      int clickX = (int)lparam;
      int clickY = (int)dparam;
      
      int relX = clickX - g_cockpitX;
      int relY = clickY - g_cockpitY;
      
      // Minimize & Close buttons
      if(relY >= 5 && relY <= 38)
      {
         if(relX >= 742 && relX <= 766) { g_cockpitMinimized = !g_cockpitMinimized; UpdatePanel(); return; }
         if(relX >= 770 && relX <= 794) { g_toggleShowPanel = false; g_panelCanvas.Destroy(); UpdateHudToggleButtons(); return; }
      }
      
      if(g_cockpitMinimized) return;
      
      // 5 Tabs Clicking (y: 44..78)
      if(relY >= 44 && relY <= 78)
      {
         if(relX >= 12 && relX <= 162)       { g_activeCockpitTab = TAB_ORDER_FLOW_INTEL;  UpdatePanel(); return; }
         else if(relX >= 166 && relX <= 320) { g_activeCockpitTab = TAB_BOOKMAP_SUITE;     UpdatePanel(); return; }
         else if(relX >= 324 && relX <= 476) { g_activeCockpitTab = TAB_FOOTPRINT_SIERRA;  UpdatePanel(); return; }
         else if(relX >= 480 && relX <= 626) { g_activeCockpitTab = TAB_SD_GENESIS_LADDER; UpdatePanel(); return; }
         else if(relX >= 630 && relX <= 788) { g_activeCockpitTab = TAB_SNIPER_SULTAN;     UpdatePanel(); return; }
      }
      
      // Tab 4: Sniper & Sultan Action Clicks
      if(g_activeCockpitTab == TAB_SNIPER_SULTAN)
      {
         if(relX >= 28 && relX <= 388 && relY >= 262 && relY <= 296) { g_execFastSniper = true;  UpdatePanel(); return; }
         if(relX >= 398 && relX <= 772 && relY >= 262 && relY <= 296) { g_execFastSniper = false; UpdatePanel(); return; }

         if(relX >= 28 && relX <= 388 && relY >= 306 && relY <= 360)
         {
            Print("🚀 [MANUAL SNIPER] SELL BUTTON CLICKED!");
            double sl = SymbolInfoDouble(_Symbol, SYMBOL_ASK) + (g_execSlPips * PipSize());
            double tp = SymbolInfoDouble(_Symbol, SYMBOL_BID) - (g_execTpPips * PipSize());
            TryOpen("SELL", "MANUAL-SNIPER-SELL", false, 0, 0, sl, tp);
            return;
         }
         if(relX >= 398 && relX <= 772 && relY >= 306 && relY <= 360)
         {
            Print("🚀 [MANUAL SNIPER] BUY BUTTON CLICKED!");
            double sl = SymbolInfoDouble(_Symbol, SYMBOL_BID) - (g_execSlPips * PipSize());
            double tp = SymbolInfoDouble(_Symbol, SYMBOL_ASK) + (g_execTpPips * PipSize());
            TryOpen("BUY", "MANUAL-SNIPER-BUY", false, 0, 0, sl, tp);
            return;
         }
         if(relX >= 28 && relX <= 772 && relY >= 370 && relY <= 412)
         {
            ExecuteNewsStraddle();
            return;
         }
         if(relX >= 28 && relX <= 208 && relY >= 420 && relY <= 456)
         {
            ...
            for(int i = PositionsTotal() - 1; i >= 0; i--)
            {
               if(PositionGetSymbol(i) == _Symbol)
               {
                  ...
                  if(partVol >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
                     trade.PositionClosePartial(ticket, partVol);
               }
            }
            return;
         }
         if(relX >= 598 && relX <= 772 && relY >= 420 && relY <= 456)
         {
   ...
   // 3. Toggle Buttons Top Right
   else if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == "DD_BTN_PANEL" || sparam == "DD_BTN_PANEL_TOGGLE")
      {
         g_toggleShowPanel = !g_toggleShowPanel;
         if(!g_toggleShowPanel) g_panelCanvas.Destroy();
         else UpdatePanel();
         UpdateHudToggleButtons();
         ChartRedraw(0);
         return;
      }
      if(sparam == "DD_BTN_POC_VA" || sparam == "DD_BTN_POC_TOGGLE")
      {
         g_togglePocVa = !g_togglePocVa;
         if(!g_togglePocVa)
         {
            ObjectDelete(0, WALL_PREFIX + "POC");
            ObjectDelete(0, WALL_PREFIX + "POC_TXT");

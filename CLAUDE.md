...
**CONTI = kelanjutan arah CMP master secara umum, bukan kategori sinyal terpisah dari CF.** Ada 2 fase:
- **Pre-VR CONTI** (VR di TF bawah belum terjadi) — breakout searah master TF diambil sebagai kelanjutan, tapi **TIDAK dieksekusi otomatis** karena retracement-nya belum confirmed kelar (`engine/core.py` nyebut ini eksplisit: *"trade every direction-aligned BO at VR TF before VR occurs"*).
- **Post-VR CONTI = CF** (VR udah terjadi, breakout balik searah master) — **INI yang dieksekusi**. `CF_LOW`/`CF_HIGH` di tabel atas itu bentuk CONTI juga, cuma versi yang udah lolos siklus VR→CF (konfirmasi), bukan sinyal beda jenis dari CONTI.

`main.py` filter eksekusinya: `dd_signal.get("type") in ("CF_LOW", "CF_HIGH")` — literal skip type `"CONTI"` (pre-VR) doang, sesuai poin di atas.

...
// Dadang: "warna putihnya buat kayak jarum itu bro" (wants text as solid
// as the gauge needle, which is a filled shape so it's trivially opaque) -
// "Arial Bold" (a face-name lookup that has to resolve a style variant)
// apparently wasn't landing either, still read thin. "Arial Black" is a
// genuinely separate, always-installed Windows font FILE (not a style
// lookup) - much heavier stroke weight, guaranteed to actually be bold
// rather than hoping GDI resolves a compound name. Still one TextOut
// call, same safe code path as PnlTxt().
void PnlTxtB(int x, int y, string s, color clr, int size = 14, uint anchor = TA_LEFT | TA_TOP)
{
   g_panelCanvas.FontSet("Arial Black", size);
   g_panelCanvas.TextOut(x, y, s, ColorToARGB(clr, 255), anchor);
}
...
{
   PnlTxt(x, y, label, PNL_SILVER, 11);
   PnlTxtB(x + PNL_VALUE_COL, y - 1, value, valueClr, 14);
}
...
void UpdatePanel()
{
   ...
   PnlTxtB(x0, y, StringFormat("%s  >  %s  >  %s", EnumToString(InpMasterTF), EnumToString(InpScalpMasterTF), EnumToString(InpScalpEntryTF)), PNL_WHITE, 12); y += 19;

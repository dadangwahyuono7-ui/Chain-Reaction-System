...
string   g_scalpEntryPrevDir = "WAIT";  // M5's direction just BEFORE its most recent flip - used to detect "M5 tested M30 (opposed it) then recovered"
...
//--- M5-tested-M30 filter. Dadang: "M30 buy, tapi nunggu CMP M30 diuji
//--- oleh M5 sell, terus M5 berubah jadi buy, baru kita buy" - don't trust
//--- the first M5 signal that already agrees with M30; require M5 to have
//--- genuinely OPPOSED M30 first (a real test of M30's structure) and then
//--- recovered back to M30's direction. Filters out entries where M5 never
//--- pulled back at all - a stronger confirmation than plain "M5 matches
//--- direction now". Uses g_scalpEntryPrevDir (M5's state just before its
//--- current flip, tracked in OnTick()).
bool M5TestedM30(string dir)
{
   if(!InpUseM5TestM30) return true;
   return g_scalpEntryPrevDir == OppositeDir(dir);
}
...
void OnTick()
{
   ...
   if(InpEnableScalp)
   {
      ...
      datetime newScalpEntryChangeTime;
      string newScalpEntryDir = ReadCMP(g_hScalpEntry, newScalpEntryChangeTime);
      if(newScalpEntryDir != g_scalpEntryDir)
         g_scalpEntryPrevDir = g_scalpEntryDir;   // remember what M5 was RIGHT BEFORE this flip
      g_scalpEntryDir = newScalpEntryDir;
      g_scalpEntryChangeTime = newScalpEntryChangeTime;
   }
   ...
   if(g_mode == MODE_NORMAL)
   {
         ...
         && M1Ok(g_masterDir, g_scalpEntryChangeTime))
      {
         ...
         if(m5BarTime != g_lastM5NormalBarTime)
         {
            ...
            if(ADXOk(g_hADX_ScalpEntry) && RSIOk(g_hRSI_ScalpEntry, g_masterDir) && M30MomentumOk(g_masterDir) && M5TestedM30(g_masterDir))

...
void OnTick()
{
   ...
   if(g_mode == MODE_NORMAL)
   {
      ...
      // though M15 hasn't flipped yet - NORMAL entries must stay blocked
      // until M30 realigns (M30==Master or M30==WAIT is fine, only the
      // opposite direction blocks).
      ...
      // Only re-evaluate NORMAL entries once per Entry-TF bar close - this
      // IS the entry trigger: Entry TF's BO closes confirming direction
      // (Time Law above already checked), the moment the NEXT candle opens
      // is where we enter. Dadang: "M5 BO buy close, open new candle, ini
      // kita entri, caranya open... ini namanya pullback." No separate
      // retest-of-old-level wait - the open of the very next bar IS it.

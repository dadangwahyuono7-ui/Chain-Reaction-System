...
//--- Barrier-BE: Master TF's (H4) own LIVE res/sup are old local
//--- highs/lows that price hasn't retested yet - "area pantul" (bounce
//--- zones). Dadang: "setiap sampai barrier pertama sebaiknya langsung set
...
//--- direction, not which tier opened it).
void CheckBarrierBE()
{
   if(!InpUseBarrierBE || g_hMaster == INVALID_HANDLE) return;
   double pip = PipSize();
   double liveRes = ReadLiveLevel(g_hMaster, 6);
   double liveSup = ReadLiveLevel(g_hMaster, 5);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      bool touchedBarrier = isBuy
         ? (liveRes > 0 && SymbolInfoDouble(_Symbol, SYMBOL_BID) >= liveRes)
         : (liveSup > 0 && SymbolInfoDouble(_Symbol, SYMBOL_ASK) <= liveSup);

...
//--- Zone-precision entry filter: require price to actually come back and
//--- RETEST the exact breakout level that put CMP into this direction
//--- (within InpRetestTolerancePips), AND still close on the correct side
//--- of it (proof the level held, wasn't able to flip). Dadang: "entrinya
//--- dibuat di area breakout, buat memastikan area cmp nya sudah diuji dan
//--- tidak sanggup flip." Corrected 2026-08-09: the pullback is about the
//--- WICK touching the zone (Open->Low->Entry->High->Close for BUY), NOT
//--- about the candle's body color - a candle that dips into the zone and
//--- still closes bullish is a VALID (in fact the cleanest) retest, not a
//--- rejected one. Previously required close<open first, which wrongly
//--- filtered out exactly these strong V-shaped continuation candles.
...
void OnTick()
{
   ...
   if(g_mode == MODE_NORMAL)
   {
      ...
      // A same-direction match that's actually STALE (existed before Master
      // itself flipped) doesn't count as a fresh child confirmation.
      if(eChangeTime <= g_masterChangeTime) return;

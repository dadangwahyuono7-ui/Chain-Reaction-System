...
//--- true if the last CLOSED bar on tf went the SAME way as dir (used to
//--- require 1 pullback candle before chasing an entry - see doctrine note
//--- at the call sites below).
bool PulledBack(ENUM_TIMEFRAMES tf, string dir)
{
   double o = iOpen(_Symbol, tf, 1);
   double c = iClose(_Symbol, tf, 1);
   return (dir == "BUY") ? (c < o) : (c > o);   // BUY wants a down candle first, SELL wants an up candle first
}
...
void OnTick()
{
   ...
   if(g_mode == MODE_NORMAL)
   {
      ...
      // Pullback confirmation: don't chase - only enter right after a CLOSED
      // candle that went AGAINST the trade direction while CMP still held
      // (proof the dip/bounce didn't flip it). Stops buying straight into an
      // extended rally's peak (or selling straight into a dump's bottom).
      if(!PulledBack(InpEntryTF, g_masterDir)) return;
   ...
   else // MODE_SCALP - trade the OPPOSITE direction of the top Master
   {
      ...
      if(!PulledBack(InpScalpEntryTF, scalpDir)) return;

...
input int                 InpMaxOpenPos   = 1;              // Maximum posisi terbuka bersamaan
input bool                InpAllowMultiple= false;          // Boleh entry baru walau udah ada posisi searah
input ENUM_TRADE_DIRECTION InpTradeDir    = DIR_BOTH;       // Filter arah trading

input group "=== STOP LOSS / TAKE PROFIT (NORMAL tier) ==="
input double              InpSL_Pips      = 150;            // Stop Loss (pips, 0 = tanpa SL) - dipakai KEDUA tier (backstop katastropik)
input double              InpTP_Pips      = 0;               // Take Profit NORMAL (pips, 0 = tanpa TP)

input group "=== TRAILING STOP / BREAK EVEN (NORMAL tier) ==="
input bool                InpUseTrailing  = false;          // Aktifkan trailing stop NORMAL
input double              InpTrailStartPips= 100;            // Trailing mulai aktif setelah profit sekian pip
input double              InpTrailStepPips = 20;             // Jarak trailing dari harga sekarang (pip)
input bool                InpUseBreakEven = false;          // Aktifkan break-even NORMAL
input double              InpBE_TriggerPips= 50;             // BE aktif setelah profit sekian pip
input double              InpBE_LockPips   = 5;              // SL dikunci sejauh ini dari entry (pip) pas BE
...
input group "=== RSI FILTER (eksperimen, default OFF - di luar doktrin CMP murni) ==="
input bool                InpUseRSI         = false;    // Filter overbought/oversold RSI
...
input group "=== EMA TREND FILTER (eksperimen, ganti ADX, default OFF) ==="
input bool                InpUseEMA    = false;    // Aktifkan filter trend EMA 25/50/100
input int                 InpEMA_Fast  = 25;       // EMA cepat
input int                 InpEMA_Mid   = 50;       // EMA menengah
input int                 InpEMA_Slow  = 100;      // EMA lambat - break ini = cancel setup
...
// RSI/EMA filter handles - only created when InpUseRSI/InpUseEMA enabled.
...
int      g_hEMA25_Entry     = INVALID_HANDLE;
int      g_hEMA50_Entry     = INVALID_HANDLE;
int      g_hEMA100_Entry    = INVALID_HANDLE;
int      g_hEMA25_ScalpEntry  = INVALID_HANDLE;
int      g_hEMA50_ScalpEntry  = INVALID_HANDLE;
int      g_hEMA100_ScalpEntry = INVALID_HANDLE;
...
int OnInit()
{
   ...
   if(InpUseEMA)
   {
      g_hEMA25_Entry  = iMA(_Symbol, InpEntryTF, InpEMA_Fast, 0, MODE_EMA, PRICE_CLOSE);
      g_hEMA50_Entry  = iMA(_Symbol, InpEntryTF, InpEMA_Mid,  0, MODE_EMA, PRICE_CLOSE);
      g_hEMA100_Entry = iMA(_Symbol, InpEntryTF, InpEMA_Slow, 0, MODE_EMA, PRICE_CLOSE);
      if(InpEnableScalp)
      {
         g_hEMA25_ScalpEntry  = iMA(_Symbol, InpScalpEntryTF, InpEMA_Fast, 0, MODE_EMA, PRICE_CLOSE);
         g_hEMA50_ScalpEntry  = iMA(_Symbol, InpScalpEntryTF, InpEMA_Mid,  0, MODE_EMA, PRICE_CLOSE);
         g_hEMA100_ScalpEntry = iMA(_Symbol, InpScalpEntryTF, InpEMA_Slow, 0, MODE_EMA, PRICE_CLOSE);
      }
   }
   ...
   Print("DD Chain Reaction Multi-TF EA aktif: Master=", EnumToString(InpMasterTF),
         ...
         InpUseEMA ? StringFormat("  |  EMA filter ON (%d/%d/%d)", InpEMA_Fast, InpEMA_Mid, InpEMA_Slow) : "",
...
void OnDeinit(const int reason)
{
   ...
   if(g_hEMA25_Entry     != INVALID_HANDLE) IndicatorRelease(g_hEMA25_Entry);
   if(g_hEMA50_Entry     != INVALID_HANDLE) IndicatorRelease(g_hEMA50_Entry);
   if(g_hEMA100_Entry    != INVALID_HANDLE) IndicatorRelease(g_hEMA100_Entry);
   if(g_hEMA25_ScalpEntry  != INVALID_HANDLE) IndicatorRelease(g_hEMA25_ScalpEntry);
   if(g_hEMA50_ScalpEntry  != INVALID_HANDLE) IndicatorRelease(g_hEMA50_ScalpEntry);
   if(g_hEMA100_ScalpEntry != INVALID_HANDLE) IndicatorRelease(g_hEMA100_ScalpEntry);
...
//--- EMA trend filter (eksperimen, off by default, replaces ADX). Dadang's
//--- spec: EMA25/50/100 stacked in order = trend direction (avoids ranging
//--- market and crossed/tangled EMA lines implicitly - if they're not
//--- cleanly stacked, this returns false = no trade). Entry only allowed
//--- after price pulls back and touches EMA25 or EMA50 (setup). Price
//--- breaking EMA100 invalidates the setup (cancel). Always true if
//--- InpUseEMA is off or handles weren't created.
bool EMAOk(ENUM_TIMEFRAMES tf, int h25, int h50, int h100, string dir)
{
   if(!InpUseEMA) return true;
   if(h25 == INVALID_HANDLE || h50 == INVALID_HANDLE || h100 == INVALID_HANDLE) return true;

   double e25[1], e50[1], e100[1];
   if(CopyBuffer(h25, 0, 1, 1, e25) <= 0)  return true;
   if(CopyBuffer(h50, 0, 1, 1, e50) <= 0)  return true;
   if(CopyBuffer(h100, 0, 1, 1, e100) <= 0) return true;
   ...
   if(dir == "BUY")
   {
      bool stacked      = (e25[0] > e50[0] && e50[0] > e100[0]);   // uptrend: EMA25>EMA50>EMA100
      bool pulledToEMA  = (lo <= e25[0] || lo <= e50[0]);           // pullback touched EMA25 or EMA50
      bool aboveEMA100  = (close > e100[0]);                        // hasn't broken EMA100 - setup still valid
      return stacked && pulledToEMA && aboveEMA100;
   }
   else // SELL
   {
      bool stacked      = (e25[0] < e50[0] && e50[0] < e100[0]);   // downtrend: EMA25<EMA50<EMA100
      bool pulledToEMA  = (hi >= e25[0] || hi >= e50[0]);
      bool belowEMA100  = (close < e100[0]);
      return stacked && pulledToEMA && belowEMA100;
   }
}
...
void TryOpen(string dir, string tag, bool isScalp)
{
   ...
   if(!SpreadOk()) return;
   if(CountOpenPositions() >= InpMaxOpenPos) return;
...
void OnTick()
{
   ...
   if(g_mode == MODE_NORMAL)
   {
      ...
      // Eksperimen (off by default): EMA 25/50/100 trend+pullback + RSI overbought/oversold.
      if(!EMAOk(InpEntryTF, g_hEMA25_Entry, g_hEMA50_Entry, g_hEMA100_Entry, g_masterDir)) return;
   ...
   else // MODE_SCALP - trade the OPPOSITE direction of the top Master
   {
      ...
      // Eksperimen (off by default): EMA 25/50/100 trend+pullback + RSI overbought/oversold.
      if(!EMAOk(InpScalpEntryTF, g_hEMA25_ScalpEntry, g_hEMA50_ScalpEntry, g_hEMA100_ScalpEntry, scalpDir)) return;

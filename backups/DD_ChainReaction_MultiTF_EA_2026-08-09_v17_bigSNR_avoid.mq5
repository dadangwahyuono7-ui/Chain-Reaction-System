...
//--- Big-TF SNR avoidance. Dadang 2026-08-09: "hindari entri di area SNR TF
//--- besar Weekly Daily H4, area itu pasti balik arah" - checks proximity to
//--- the level that would REJECT this trade direction (live resistance for
//--- BUY, live support for SELL) across W1/D1/H4. Any of the three being
//--- within InpBigSNR_ZonePips of current price blocks the entry. Returns
//--- true (OK to enter) if the filter is off or handles failed to load.
bool NearBigSNR(string dir)
{
   if(!InpAvoidBigSNR) return false;   // "near" check disabled = never blocks
   double px  = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double pip = PipSize();
   int bufIdx = (dir == "BUY") ? 6 : 5;   // BUY checks resistance ahead, SELL checks support ahead

   int handles[3] = {g_hWeekly, g_hDaily, g_hMaster};
   for(int i = 0; i < 3; i++)
   {
      if(handles[i] == INVALID_HANDLE) continue;
      double level = ReadLiveLevel(handles[i], bufIdx);
      if(level <= 0) continue;
      double distPips = MathAbs(level - px) / pip;
      if(distPips <= InpBigSNR_ZonePips) return true;
   }
   return false;
}

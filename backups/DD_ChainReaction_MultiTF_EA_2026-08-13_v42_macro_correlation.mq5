...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         if(g_macroActive)
         {
            color macroClr = (g_macroVerdict == "SEARAH") ? PNL_EMERALD
                             : (g_macroVerdict == "LAWAN") ? PNL_ROSE : PNL_LABEL;
            string arrow = (g_macroDirection == "UP") ? "^" : (g_macroDirection == "DOWN") ? "v" : "-";
            PnlRow(x0, y, contentW, "Macro (EUR)",
                   StringFormat("%s %s  USD %s", arrow, g_macroDirection, g_macroUsd), PNL_WHITE); y += 16;
            PnlRow(x0, y, contentW, "Dollar vs Setup",
                   g_macroVerdict + (g_macroFlowOk ? "" : "  (flow lemah)"), macroClr); y += 18;
         }
         else
         {
            PnlRow(x0, y, contentW, "Macro (EUR)", "off", PNL_LABEL); y += 18;
         }
         PnlRule(x0, y, contentW); y += 12;

...
//|                                                                   |
//| DOCTRINE (ANCHOR/CF cascade per TEHINIX transcripts, D:\bunga\    |
//| TEHINIX BERIKUTNYA - Dadang 2026-08-09: "entri nya masak langsung |
//| kelima2nya... lo harus baca ulang TEHINIX BERIKUTNYA"):           |
//|   - Master TF sets the trading BIAS. EA only ever opens in the    |
//|     Master's current CMP direction.                                |
//|   - "CONTI" (Entry TF's CMP simply staying aligned with Master)   |
//|     is explicitly NOT an entry signal per doctrine: "Conti...     |
//|     bukan entry ya." As long as Master hasn't flipped, every      |
//|     opposite-color candle on Entry TF is just retracement noise,  |
//|     not an independent signal either.                              |
//|   - A trigger only ARMS once Entry TF's CMP genuinely flips        |
//|     OPPOSITE to Master first - the "kandar retracement" / anchor  |
//|     candle (a real confirmed breakout the other way, since CMP    |
//|     only flips on a closed-bar breakout, not raw candle color).   |
//|   - Entry fires ONCE Entry TF then flips BACK to Master's         |
//|     direction - a genuine CF - then must re-anchor before firing  |
//|     again. See UpdateArm(). This replaces the old "fire every new |
//|     bar CMP stays aligned" behavior that spammed 5 entries at     |
//|     once on pure continuation.                                     |
//|                                                                    |
...
input bool                InpShowBookmapPanel = true;    // Tampilin CVD/Pulse/Absorption Bookmap di panel (informational, BELUM dipakai buat filter entry)
input double              InpBookmapStaleSec  = 5.0;     // Anggap data basi/Bookmap gak jalan kalau lebih tua dari ini (detik)
...
// ANCHOR/CF arm state (TEHINIX doctrine, see header doctrine comment and
// UpdateArm() below) - one per cascade trigger. "armed" means the trigger's
// child TF has produced a genuine anchor (CMP flip opposite to the parent
// direction it's meant to trade) and is now waiting for a CF (flip back) to
// fire. prevChildDir tracks the child's last-seen CMP so UpdateArm() can
// detect the exact tick of the flip-back instead of re-firing on every tick
// alignment merely holds.
bool     g_normalArmedA = false;   string g_normalPrevA  = "WAIT";   // Trigger A: Master -> Entry TF (M15)
bool     g_normalArmedB = false;   string g_normalPrevB  = "WAIT";   // Trigger B: Master -> Scalp Entry TF (M5), inside NORMAL mode
bool     g_scalpArmed   = false;   string g_scalpPrevDir = "WAIT";   // MODE_SCALP: Scalp Master (M30) -> Scalp Entry TF (M5)
...
// Informational only for now - not wired into entry/exit logic.
...
//--- M15 must-not-oppose check. Dadang: "ketika M5 BO sell tapi M15 BO buy,
//--- kita skip entri sell. Dan kalau kita mau buy dan M15 masih sell, kita
//--- skip buy." - closes a gap the top-level M15-opposes-Master check
//--- doesn't fully cover for SCALP mode: that check only catches M15
//--- EXACTLY matching Master again (cascade-over case), but if M15 is
//--- still WAIT (never confirmed either way), SCALP entries could still
//--- fire without M15's explicit agreement. This is checked directly next
//--- to TryOpen() as a final safety net, redundant with (but stronger
//--- than) the earlier checks.
bool M15Ok(string dir)
{
   return g_entryDir != OppositeDir(dir);
}
...
void ReadBookmapBridge()
{
   ...
   if(handle == INVALID_HANDLE) return;   // bridge never ran on this machine, or udp_listener.py isn't up

   for(int i = 0; i < 5 && !FileIsEnding(handle); i++) FileReadString(handle);   // skip header row (5 fields)
   if(FileIsEnding(handle)) { FileClose(handle); return; }

   double ts = StringToDouble(FileReadString(handle));
   FileReadString(handle);   // price - not currently surfaced on panel, MT5's own bid/ask is authoritative
   ...
   if(ageSec < 0 || ageSec > InpBookmapStaleSec) return;   // stale (Bookmap/listener down, or a backtest)
...
//--- ANCHOR/CF arm-state machine (TEHINIX doctrine - see header comment).
//--- childDir = current CMP of the trigger's own entry TF. parentDir = the
//--- direction this trigger is meant to trade (Master's dir for Trigger
//--- A/B, OppositeDir(Master) for the SCALP trigger). Returns true on
//--- EXACTLY the one tick childDir transitions from OppositeDir(parentDir)
//--- (an anchored retracement) back to parentDir (a genuine CF) - i.e. once
//--- per arm cycle. Plain continuation (childDir staying == parentDir bar
//--- after bar) never re-fires, since armed is consumed the first time it's
//--- used and CMP only changes value once per closed bar, so prevChildDir
//--- catches up on the very next tick either way.
bool UpdateArm(bool &armed, string &prevChildDir, string childDir, string parentDir)
{
   bool fired = false;
   if(childDir == OppositeDir(parentDir))
   {
      armed = true;                                    // anchor candle formed (genuine opposing BO)
   }
   else if(childDir == parentDir)
   {
      if(armed && prevChildDir != parentDir)
         fired = true;                                  // fresh CF: child just broke back to parent's direction
      armed = false;                                     // consumed either way - must re-anchor for next entry
   }
   prevChildDir = childDir;
   return fired;
}

//--- Resets ALL cascade arm state - called on any full regime change (Master
//--- flip, or NORMAL<->SCALP mode switch) so a stale arm from the previous
//--- regime can never fire an entry that belongs to a doctrine cycle that no
//--- longer exists. Always safer to require a fresh anchor than to trust
//--- leftover state across a regime boundary.
void ResetArmState()
{
   g_normalArmedA = false; g_normalPrevA  = "WAIT";
   g_normalArmedB = false; g_normalPrevB  = "WAIT";
   g_scalpArmed   = false; g_scalpPrevDir = "WAIT";
}
...
void TryOpen(string dir, string tag, bool isScalp, datetime parentFlipTime = 0, datetime childFlipTime = 0)
{
   ...
   if(trade.PositionOpen(_Symbol, buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, px, sl, tp, comment))
      Print("ENTRY (", tag, ") ", dir, " lot=", lot, " @ ", px, " SL=", sl, " TP=", tp, timeLawNote);
   else
...
void OnTick()
{
   ...
   if(masterDir != g_masterDir)
   {
      ...
      g_mode = MODE_NORMAL;
      ResetArmState();   // new regime - any prior anchor/CF cycle is void, must re-anchor
   ...
   {
      // Update both NORMAL-tier arm states EVERY tick, unconditionally, so
      // an anchor forming on the same tick as a cut-loss below is never
      // missed (see UpdateArm() doctrine comment).
      bool triggerA_fired = UpdateArm(g_normalArmedA, g_normalPrevA, g_entryDir, g_masterDir);
      bool triggerB_fired = InpEnableScalp
         ? UpdateArm(g_normalArmedB, g_normalPrevB, g_scalpEntryDir, g_masterDir)
         : false;
      ...
      if(g_entryDir == OppositeDir(g_masterDir))
      {
         ...
         {
            g_mode = MODE_SCALP;
            g_scalpArmed = false; g_scalpPrevDir = "WAIT";   // fresh cascade - fresh arm cycle
      ...
      // --- Trigger A: Entry TF (M15) anchor->CF -> open of next M15 candle.
      // ANCHOR/CF gate (TEHINIX doctrine): fires only on the tick Entry TF's
      // CMP flips back to Master's direction AFTER having genuinely anchored
      // (flipped opposite first) - NOT merely "still aligned this bar" (that
      // was the old behavior and is exactly the "Conti bukan entry" bug
      // Dadang flagged: 5 entries firing at once on pure continuation).
      if(triggerA_fired && eChangeTime > g_masterChangeTime)
      {
         ...
         if(entryBarTime != g_lastEntryBarTime)
         {
            ...
            if(ADXOk(g_hADX_Entry) && RSIOk(g_hRSI_Entry, g_masterDir) && M30MomentumOk(g_masterDir))
               TryOpen(g_masterDir, StringFormat("%s>%s", EnumToString(InpMasterTF), EnumToString(InpEntryTF)), false,
                       g_masterChangeTime, eChangeTime);
         }
      }

      // --- Trigger B: Scalp Entry TF (M5) anchor->CF -> open of next M5
      // candle. Additional FASTER trigger inside NORMAL mode, requested by
      // Dadang: M15 being a single still-open bar shouldn't blind the EA to
      // M5-level BO+pullback moves happening underneath it. Same ANCHOR/CF
      // gate as Trigger A, just checked against M5's own anchor cycle -
      // independent bar-gate (g_lastM5NormalBarTime) so it doesn't collide
      // with SCALP mode's own M5 bar tracking. M1Ok() checked BEFORE the
      // bar-gate update (every tick) so a still-pending M1 confirmation
      // doesn't get "consumed" by one bar's snapshot - the EA genuinely
      // waits across bars until M1 catches up.
      if(InpEnableScalp && triggerB_fired && g_scalpEntryChangeTime > g_masterChangeTime
         && M1Ok(g_masterDir, g_scalpEntryChangeTime))
      {
         ...
         if(m5BarTime != g_lastM5NormalBarTime)
         {
            ...
            if(ADXOk(g_hADX_ScalpEntry) && RSIOk(g_hRSI_ScalpEntry, g_masterDir) && M30MomentumOk(g_masterDir) && M5TestedM30(g_masterDir) && M15Ok(g_masterDir))
               TryOpen(g_masterDir, StringFormat("%s>%s", EnumToString(InpMasterTF), EnumToString(InpScalpEntryTF)), false,
                       g_masterChangeTime, g_scalpEntryChangeTime);
         }
      }
   }
   else // MODE_SCALP - trade the OPPOSITE direction of the top Master
   {
      ...
      // Update the SCALP arm state EVERY tick, unconditionally, so an anchor
      // forming on the same tick as a cut-loss below is never missed.
      bool scalp_fired = UpdateArm(g_scalpArmed, g_scalpPrevDir, g_scalpEntryDir, scalpDir);
      ...
      if(g_entryDir == g_masterDir)
      {
         ...
         g_mode = MODE_NORMAL;
         g_normalArmedA = false; g_normalPrevA = "WAIT";   // returning to NORMAL - require fresh anchor
         g_normalArmedB = false; g_normalPrevB = "WAIT";
         return;
      }
      ...
      // ANCHOR/CF gate (TEHINIX doctrine): only fires on the tick M5 flips
      // back to scalpDir AFTER having genuinely anchored (flipped to align
      // with Master first) - not merely "still opposing Master this bar".
      if(!scalp_fired) return;
      ...
      if(!M5TestedM30(scalpDir)) return;
      if(!M15Ok(scalpDir)) return;
      ...
      TryOpen(scalpDir, StringFormat("SCALP %s>%s", EnumToString(InpScalpMasterTF), EnumToString(InpScalpEntryTF)), true,
              g_scalpMasterChangeTime, g_scalpEntryChangeTime);
   }
}

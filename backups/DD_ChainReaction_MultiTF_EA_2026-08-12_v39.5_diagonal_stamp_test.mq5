...
int PNL_PX = 12, PNL_PY = 18, PNL_W = 280, PNL_H = 520;
int PNL_MARGIN = 14, PNL_RADIUS = 10, PNL_BORDER = 2;
int PNL_VALUE_COL = 128; // fixed x-offset (from a row's left edge) where the value text starts
...
// v39.5: Dadang - "kalo gak dukung kenapa pas price bisa solid putih bro"
// - fair challenge. Isolating one variable this time: weight=700 alone
// (v39.4) rendered fine but still read soft, so this adds back exactly
// ONE extra offset TextOut (diagonal +1,+1) on top of the weight flag -
// not the 4-way stamp from the version that went fully invisible. If
// this also disappears, that proves multi-draw itself is unsafe here;
// if it renders (just bolder), the earlier failure was something else
// (most likely the "Lucida Console" font name, never retested in
// isolation from the stamping).
void PnlTxtB(int x, int y, string s, color clr, int size = 14, uint anchor = TA_LEFT | TA_TOP)
{
   ...
   g_panelCanvas.TextOut(x,     y,     s, argb, anchor);
   g_panelCanvas.TextOut(x + 1, y + 1, s, argb, anchor);
}
...
{
   PnlTxt(x, y, label, PNL_SILVER, 9);
   PnlTxtB(x + PNL_VALUE_COL, y - 1, value, valueClr, 11);
}
...
void UpdatePanel()
{
   ...
   PnlTxtB(x0, y, InpPanelName, PNL_GOLD, 12); y += 16;
   PnlTxt(x0, y, "[ " + EA_VERSION + " ]  " + InpOwnerName, PNL_SILVER, 8); y += 12;
   PnlTxt(x0, y, "Version: " + EA_VERSION, PNL_GOLD, 8); y += 14;
   ...
   PnlStatusDot(x0 + 3, y + 5, modeClr);
   PnlTxtB(x0 + 12, y, modeTxt, modeClr, 10); y += 15;
   PnlTxtB(x0, y, StringFormat("%s  >  %s  >  %s", EnumToString(InpMasterTF), EnumToString(InpScalpMasterTF), EnumToString(InpScalpEntryTF)), PNL_WHITE, 9); y += 13;
   PnlRow(x0, y, contentW, "H4  (Master)",  g_masterDir, DirColor(g_masterDir)); y += 13;
   PnlRow(x0, y, contentW, "Firing", g_m5FiringEnabled ? "ON" : "PAUSED", g_m5FiringEnabled ? PNL_EMERALD : PNL_SILVER); y += 13;
   PnlRow(x0, y, contentW, "M30 (Cascade)", g_scalpMasterDir, DirColor(g_scalpMasterDir)); y += 13;
   PnlRow(x0, y, contentW, "M5  (Entry)",   g_scalpEntryDir,  DirColor(g_scalpEntryDir));  y += 15;
   PnlRule(x0, y, contentW); y += 8;

   PnlAccentRow(x0, y, contentW, "Open / Float P&L", StringFormat("%d   %+.2f", openCount, floatingPL),
                floatingPL >= 0 ? PNL_EMERALD : PNL_ROSE, floatingPL >= 0 ? PNL_EMERALD : PNL_ROSE); y += 13;
   PnlRow(x0, y, contentW, "Closed (W/L)", StringFormat("%d  (%d / %d)", g_statTrades, g_statWins, g_statLosses), PNL_WHITE); y += 13;
   PnlAccentRow(x0, y, contentW, "Total Profit", StringFormat("%+.2f", g_statTotalProfit),
                g_statTotalProfit >= 0 ? PNL_EMERALD : PNL_ROSE, g_statTotalProfit >= 0 ? PNL_EMERALD : PNL_ROSE); y += 13;
   PnlRow(x0, y, contentW, "Bal / Equity", StringFormat("%.2f / %.2f", AccountInfoDouble(ACCOUNT_BALANCE), AccountInfoDouble(ACCOUNT_EQUITY)), PNL_WHITE); y += 15;
   PnlRule(x0, y, contentW); y += 8;
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         PnlStatusDot(x0 + 3, y + 4, PNL_GOLD);
         PnlTxtB(x0 + 12, y, "BOOKMAP  ·  LIVE", PNL_GOLD, 9); y += 14;
         PnlRow(x0, y, contentW, "CVD", StringFormat("%+.1f", g_bookmapCvd), g_bookmapCvd >= 0 ? PNL_EMERALD : PNL_ROSE); y += 13;

         color pulseClr = (g_bookmapAbsorption != "NONE") ? C'251,191,36' : PNL_SILVER;
         PnlRow(x0, y, contentW, "Pulse / Absorb", StringFormat("%.1f%%  %s", g_bookmapPulsePct, g_bookmapAbsorption), pulseClr); y += 16;
         ...
         PnlTxt(x0, gy + gr + 8, "Price down", PNL_SILVER, 7);
         PnlTxt(x0 + contentW - 44, gy + gr + 8, "Price up", PNL_SILVER, 7);
         PnlTxtB(gx - 20, gy + gr + 6, StringFormat("%+.0f%%", pricePct), pressClr, 12);
         y = gy + gr + 24;
         PnlRule(x0, y, contentW); y += 8;
         ...
         PnlRow(x0, y, contentW, "Vol Ratio", StringFormat("%.0f%% BUY / %.0f%% SELL", g_bookmapVolRatioBuyPct, 100.0 - g_bookmapVolRatioBuyPct),
                g_bookmapVolRatioBuyPct >= 50.0 ? PNL_EMERALD : PNL_ROSE); y += 13;
         ...
         PnlRow(x0, y, contentW, "POC", pocTxt, PNL_GOLD); y += 13;
         ...
         PnlRow(x0, y, contentW, "Value Area", vaTxt, PNL_GOLD); y += 13;
         ...
         PnlRow(x0, y, contentW, "Iceberg", iceTxt, iceClr); y += 15;
         ...
         PnlTxtB(x0, y, "BIAS — " + bias, biasClr, 11);
      }
      else
      {
         PnlStatusDot(x0 + 3, y + 4, clrGray);
         PnlTxtB(x0 + 12, y, "BOOKMAP: OFFLINE", clrGray, 9); y += 14;
         PnlTxt(x0, y, "(gak jalan / backtest)", clrGray, 8); y += 16;
         PnlTxt(x0, y, "BIAS — N/A", clrGray, 9);
      }
   }

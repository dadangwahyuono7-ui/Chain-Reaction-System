...
//| a real smooth-gradient semicircle Price Pressure gauge, and a     |
//| two-font system (Segoe UI for labels/headers, Consolas monospace  |
//| for right-aligned numbers, bold simulated via 4x offset glyph     |
//| stamping since Canvas.mqh has no reliable weight flag).           |
//+------------------------------------------------------------------+
...
#define PNL_FONT_UI  "Segoe UI"        // headers, labels, running text
#define PNL_FONT_NUM "Consolas"        // right-aligned numeric values (monospace) - "Lucida Console" turned out not to resolve via Canvas's font lookup on Dadang's Windows 11 (values rendered fully invisible), Consolas is the one already proven to work
...
int PNL_PX = 12, PNL_PY = 18, PNL_W = 320, PNL_H = 720;
...
void PnlTxt(int x, int y, string s, color clr, int size = 11, string font = PNL_FONT_UI, uint anchor = TA_LEFT | TA_TOP)
{
   g_panelCanvas.FontSet(font, size);
   g_panelCanvas.TextOut(x, y, s, ColorToARGB(clr, 255), anchor);
}

void PnlTxtB(int x, int y, string s, color clr, int size = 13, string font = PNL_FONT_UI, uint anchor = TA_LEFT | TA_TOP)
{
   g_panelCanvas.FontSet(font, size);
   uint argb = ColorToARGB(clr, 255);
   g_panelCanvas.TextOut(x,     y,     s, argb, anchor);
   g_panelCanvas.TextOut(x + 1, y,     s, argb, anchor);
   g_panelCanvas.TextOut(x,     y + 1, s, argb, anchor);
...
// FONT_NUM is monospace, so each character is reliably ~0.62x the font
// size wide - lets the value column right-align by computation.
int PnlCharsWidth(string s, int size) { return (int)MathRound(StringLen(s) * size * 0.62); }

void PnlRow(int x, int y, int colW, string label, string value, color valueClr)
{
   PnlTxt(x, y, label, PNL_SILVER, 10, PNL_FONT_UI);
   int vsize = 12;
   PnlTxtB(x + colW - PnlCharsWidth(value, vsize), y - 1, value, valueClr, vsize, PNL_FONT_NUM);
}
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         PnlTxtB(gx - 30, gy + gr + 8, StringFormat("%+.0f%%", pricePct), pressClr, 18, PNL_FONT_NUM);
         y = gy + gr + 34;
         PnlRule(x0, y, contentW); y += 12;

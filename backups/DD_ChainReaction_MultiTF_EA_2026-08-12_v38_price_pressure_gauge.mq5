...
#define PANEL_PX 12          // v38: panel's fixed left inset - shared by CreatePanel()/UpdatePanel() so the Price Pressure needle can be repositioned without re-deriving px
#define PANEL_GAUGE_W 226    // v38: Price Pressure gauge track width in px
...
// v38: small filled rectangle - used for the Price Pressure gauge's colored
// track segments (fixed) and its needle marker (repositioned every tick by
// UpdatePanel(), same idiom as PanelLabel() but for OBJ_RECTANGLE_LABEL).
void PanelRect(string key, int x, int y, int w, int h, color clr)
{
   string name = DASH_PREFIX + key;
   if(ObjectFind(0, name) < 0)
   {
...
void CreatePanel()
{
   int px = PANEL_PX, py = 18, w = 250, h = 432;
   ...
   PanelLabel("BMPULSE",px, y, 9,  clrSilver);     y += 16;

   // v38: Price Pressure gauge - Dadang wanted the same "Price Change"
   // dial the web dashboard now has, mirrored on the chart panel too.
   // MQL5 objects can't draw arcs, so this is the linear-bar equivalent:
   // a fixed red->gray->green track + a needle that slides left/right.
   PanelLabel("BMPRESSLBL", px, y, 8, clrSilver); y += 11;
   PanelRect("BMGTRACKL", px,                    y, PANEL_GAUGE_W / 3,               7, C'150,45,60');
   PanelRect("BMGTRACKM", px + PANEL_GAUGE_W / 3, y, PANEL_GAUGE_W / 3,               7, C'70,70,78');
   PanelRect("BMGTRACKR", px + 2 * (PANEL_GAUGE_W / 3), y, PANEL_GAUGE_W - 2 * (PANEL_GAUGE_W / 3), 7, C'40,140,100');
   PanelRect("BMGNEEDLE", px + PANEL_GAUGE_W / 2 - 1, y - 3, 3, 13, clrWhite);
   y += 7 + 10;

   PanelLabel("BMVOL",  px, y, 9,  clrSilver);     y += 16;
   PanelLabel("BMPOC",  px, y, 9,  clrGold);       y += 16;
   PanelLabel("BMVA",   px, y, 9,  clrGoldenrod);  y += 16;
   PanelLabel("BMICE",  px, y, 9,  clrSilver);     y += 16;
   PanelLabel("BMBIAS", px, y, 10, clrSilver);
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         // v38: Price Pressure gauge - remap buyer_aggression_pct (0..100)
         // back to Bookmap's own -100..+100 "Price Change" scale (same
         // formula as sultan/logic.js's updatePulseGauge()) and slide the
         // needle across the track.
         double pricePct = MathMax(-100.0, MathMin(100.0, (g_bookmapPulsePct - 50.0) * 2.0));
         string pressDir = (pricePct > 15.0) ? "PRICE UP" : (pricePct < -15.0) ? "PRICE DOWN" : "FLAT";
         color pressClr = (pricePct > 15.0) ? clrLime : (pricePct < -15.0) ? clrTomato : clrSilver;
         ObjectSetString(0, DASH_PREFIX + "BMPRESSLBL", OBJPROP_TEXT,
                          StringFormat("Price Pressure: %+.0f%%  (%s)", pricePct, pressDir));
         ObjectSetInteger(0, DASH_PREFIX + "BMPRESSLBL", OBJPROP_COLOR, pressClr);
         int needleX = PANEL_PX + (int)MathRound(((pricePct + 100.0) / 200.0) * PANEL_GAUGE_W) - 1;
         ObjectSetInteger(0, DASH_PREFIX + "BMGNEEDLE", OBJPROP_XDISTANCE, needleX);

         // v29: session Volume Ratio (buy% of total traded volume) + POC
         // (price with the most traded volume this session).
         ObjectSetString(0, DASH_PREFIX + "BMVOL", OBJPROP_TEXT,
                          StringFormat("Vol Ratio: %.0f%% BUY / %.0f%% SELL", g_bookmapVolRatioBuyPct, 100.0 - g_bookmapVolRatioBuyPct));
         ObjectSetInteger(0, DASH_PREFIX + "BMVOL", OBJPROP_COLOR, (g_bookmapVolRatioBuyPct >= 50.0) ? clrLime : clrTomato);

         double bmOffset = SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice;
         if(g_bookmapPocPrice > 0)
            ObjectSetString(0, DASH_PREFIX + "BMPOC", OBJPROP_TEXT,
                             StringFormat("POC: %.2f (%.0f vol)", g_bookmapPocPrice + bmOffset, g_bookmapPocVolume));
         else
            ObjectSetString(0, DASH_PREFIX + "BMPOC", OBJPROP_TEXT, "POC: -");

         // v33: VAH/VAL (Value Area - 70% of session volume around POC).
         if(g_bookmapVah > g_bookmapVal && g_bookmapVal > 0)
            ObjectSetString(0, DASH_PREFIX + "BMVA", OBJPROP_TEXT,
                             StringFormat("VA: %.2f - %.2f", g_bookmapVal + bmOffset, g_bookmapVah + bmOffset));
         else
            ObjectSetString(0, DASH_PREFIX + "BMVA", OBJPROP_TEXT, "VA: -");
         ...
         {
            ObjectSetString(0, DASH_PREFIX + "BMICE", OBJPROP_TEXT,
                             StringFormat("Iceberg: BID @ %.2f (%.1fx)", g_bookmapBidIcePx + bmOffset, g_bookmapBidIceRatio));
            ObjectSetInteger(0, DASH_PREFIX + "BMICE", OBJPROP_COLOR, clrMagenta);
         }
         else if(g_bookmapAskIcePx > 0)
         {
            ObjectSetString(0, DASH_PREFIX + "BMICE", OBJPROP_TEXT,
                             StringFormat("Iceberg: ASK @ %.2f (%.1fx)", g_bookmapAskIcePx + bmOffset, g_bookmapAskIceRatio));
            ObjectSetInteger(0, DASH_PREFIX + "BMICE", OBJPROP_COLOR, clrMagenta);
         }
         else
         {
            ObjectSetString(0, DASH_PREFIX + "BMICE", OBJPROP_TEXT, "Iceberg: none");
            ObjectSetInteger(0, DASH_PREFIX + "BMICE", OBJPROP_COLOR, clrSilver);
         }
         ...
         ObjectSetString(0, DASH_PREFIX + "BMBIAS", OBJPROP_TEXT, "BIAS: " + bias);
         ObjectSetInteger(0, DASH_PREFIX + "BMBIAS", OBJPROP_COLOR,
                           (bias == "BUY") ? clrLime : (bias == "SELL") ? clrTomato : clrSilver);
      }
      else
      {
         ...
         ObjectSetString(0, DASH_PREFIX + "BMPULSE", OBJPROP_TEXT, "");
         ObjectSetString(0, DASH_PREFIX + "BMPRESSLBL", OBJPROP_TEXT, "Price Pressure: -");
         ObjectSetInteger(0, DASH_PREFIX + "BMPRESSLBL", OBJPROP_COLOR, clrGray);
         ObjectSetInteger(0, DASH_PREFIX + "BMGNEEDLE", OBJPROP_XDISTANCE, PANEL_PX + PANEL_GAUGE_W / 2 - 1);
         ObjectSetString(0, DASH_PREFIX + "BMVOL", OBJPROP_TEXT, "");
         ObjectSetString(0, DASH_PREFIX + "BMPOC", OBJPROP_TEXT, "");
         ObjectSetString(0, DASH_PREFIX + "BMVA", OBJPROP_TEXT, "");
         ObjectSetString(0, DASH_PREFIX + "BMICE", OBJPROP_TEXT, "");
         ObjectSetString(0, DASH_PREFIX + "BMBIAS", OBJPROP_TEXT, "BIAS: N/A");
         ObjectSetInteger(0, DASH_PREFIX + "BMBIAS", OBJPROP_COLOR, clrGray);
      }
   }

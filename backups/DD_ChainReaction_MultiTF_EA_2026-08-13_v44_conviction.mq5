...
int PNL_PX = 12, PNL_PY = 18, PNL_W = 310, PNL_H = 715;
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         color convClr = (g_convGrade == "KUAT")   ? PNL_EMERALD
                       : (g_convGrade == "SEDANG") ? C'251,191,36'
                       : (g_convGrade == "TAHAN - NEWS") ? PNL_ROSE
                       : (g_convGrade == "LEMAH" || g_convGrade == "HINDARI") ? PNL_ROSE : PNL_LABEL;
         if(g_convDir == "BUY" || g_convDir == "SELL")
         {
            PnlTxtB(x0, y, StringFormat("%s  %d/%d  %s", g_convDir, g_convScore, g_convMax, g_convGrade),
                    convClr, 13); y += 19;
            if(g_convAgainst1 != "") { PnlTxt(x0, y, "lawan: " + g_convAgainst1, PNL_LABEL, 9); y += 13; }
            if(g_convAgainst2 != "") { PnlTxt(x0, y, "lawan: " + g_convAgainst2, PNL_LABEL, 9); }
         }
         else
         {
   ...
   int footerY = oy + PNL_H - 46;
   PnlRule(x0, footerY, contentW);
   PnlTxtB(x0 + contentW / 2, footerY + 12, "CHAIN REACTION SYSTEM", PNL_GOLD, 11, TA_CENTER | TA_TOP);
   PnlTxtB(x0 + contentW / 2, footerY + 29, "by " + InpOwnerName, PNL_LABEL, 9, TA_CENTER | TA_TOP);
...
void ComputeConviction()
{
   ...
   g_convGrade = ""; g_convAgainst1 = ""; g_convAgainst2 = "";
   if(g_convDir != "BUY" && g_convDir != "SELL") { g_convGrade = "NO SETUP"; return; }

   bool isBuy = (g_convDir == "BUY");

   //--- 1. D1 context. Dadang: "bukan hanya 1 2 jam" - the daily frame is
   //--- what separates a scalp against the tide from a trade with it.
   datetime tD1;
   string d1 = (g_hExportD1 != INVALID_HANDLE) ? ReadCMP(g_hExportD1, tD1) : "";
   if(d1 == "BUY" || d1 == "SELL")
   {
      g_convMax++;
      if(d1 == g_convDir) g_convScore++;
      else AddAgainst("D1 lawan arah (" + d1 + ")");
   }

   //--- 2. H4 master regime
   if(g_masterDir == "BUY" || g_masterDir == "SELL")
   {
      g_convMax++;
      if(g_masterDir == g_convDir) g_convScore++;
      else AddAgainst("H4 lawan arah (" + g_masterDir + ")");
   }

   //--- 3. M5 trigger agreement
   ...
   //--- 4. USD / DXY backdrop. Weak dollar lifts gold, strong dollar sits on it.
   if(g_usdBias == "WEAK" || g_usdBias == "STRONG")
   {
      g_convMax++;
      bool usdOk = (isBuy && g_usdBias == "WEAK") || (!isBuy && g_usdBias == "STRONG");
      if(usdOk) g_convScore++;
   ...
   if(g_bookmapOnline)
   {
      ...
      //--- 7. Path to target not blocked by a wall sitting right in front.
      ...
      //--- 8. Location: chasing an already-extended move is where entries
   ...
   //--- Grade as a percentage so a missing factor (Bookmap offline) doesn't
   //--- silently look like a weak setup.
   double pct = (g_convMax > 0) ? (100.0 * g_convScore / g_convMax) : 0.0;
   if(g_convMax < 3)       g_convGrade = "DATA KURANG";
   else if(pct >= 85.0)    g_convGrade = "KUAT";
   else if(pct >= 65.0)    g_convGrade = "SEDANG";
   else if(pct >= 40.0)    g_convGrade = "LEMAH";
   else                    g_convGrade = "HINDARI";

   //--- News proximity isn't a direction vote - it overrides the grade,
   //--- because no amount of structural agreement survives a bad NFP print.
...
void WriteSultanStatus()
{
   ...
   json += StringFormat("\"conviction\":{\"dir\":\"%s\",\"score\":%d,\"max\":%d,\"grade\":\"%s\",\"against\":[",
                         g_convDir, g_convScore, g_convMax, g_convGrade);

      ...
      if(g_bookmapOnline)
      {
         ...
         if(g_bookmapPocPrice > 0)
            ObjectSetString(0, DASH_PREFIX + "BMPOC", OBJPROP_TEXT,
                             StringFormat("POC: %.2f (%.0f vol)", g_bookmapPocPrice + (SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice), g_bookmapPocVolume));
         else
...
void LoadOneBookmapHistoryFile(string relPath)
{
   ...
   for(int i = 0; i < 16 && !FileIsEnding(handle); i++) FileReadString(handle);   // skip header row
   ...
   while(!FileIsEnding(handle))
   {
      ...
      for(int i = 0; i < 6 && !FileIsEnding(handle); i++) FileReadString(handle);   // vol_ratio,buy_vol,sell_vol,best_bid_px,best_bid_sz,best_ask_px - skip
      if(!FileIsEnding(handle)) FileReadString(handle);     // best_ask_sz (last column)
...
void LoadBookmapHistory()
{
   ...
   string fname;
   long search = FileFindFirst(BM_HIST_DIR + "bookmap_history_*.csv", fname, FILE_COMMON);
...
void ReadBookmapBridge()
{
   ...
   int totalFields = 10 + WALL_SLOTS_PER_SIDE * 2 * 2;   // timestamp,price,cvd,pulse,absorption,poc_price,poc_volume,vol_ratio,buy_vol,sell_vol + (bid+ask)*(px+sz) per slot
...
void DrawWallLine(string key, double bookmapPrice, double size, double offset, bool isBid, string sideLabel)
{
...
void UpdateWallLines()
{
   ...
   for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
   {
      ...
      DrawWallLine(key, g_bookmapBidPx[i], g_bookmapBidSz[i], offset, true, label);
   }
   for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
   {
      string key   = StringFormat("ASK%d", i+1);
      string label = StringFormat("ASK WALL %d", i+1);
      DrawWallLine(key, g_bookmapAskPx[i], g_bookmapAskSz[i], offset, false, label);
   }
}
...
//--- v32: 3-state BIAS derived from POC position + H4 CMP direction.
//--- Dadang: "kalo harga di atas poc kita gimana, kalo harga di bawah poc
//--- kita gimana" - above POC = POC is support -> BUY territory; below POC
//--- = POC is resistance -> SELL territory; sitting AT POC (within
//--- InpPocSidewaysZoneUsd) = no clear edge -> SIDEWAYS. When H4 (g_masterDir)
//--- disagrees with the POC-implied side, treated as SIDEWAYS too (mixed
//--- signal, no consensus) - only shows a directional bias when POC and CMP
//--- actually AGREE, or when H4 hasn't warmed up yet (POC alone still valid).
string ComputeBias()
{
   ...
   double diff = SymbolInfoDouble(_Symbol, SYMBOL_BID) - pocMt5Price;
   ...
   if(g_masterDir == "WAIT") return pocSide;        // H4 not warmed up yet - POC alone still meaningful
   if(g_masterDir == pocSide) return pocSide;        // POC + CMP agree - strongest read
   return "SIDEWAYS";                                 // POC and CMP disagree - no clear consensus
}

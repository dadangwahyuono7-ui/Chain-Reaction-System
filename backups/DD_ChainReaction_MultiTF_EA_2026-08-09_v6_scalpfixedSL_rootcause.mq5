...
input group "=== SCALP TP / SL / TRAILING / BE (tier terpisah) ==="
input double              InpScalpSL_Pips        = 40;       // SL SCALP FIXED (pips) - JANGAN pakai struktural M30: root cause ketemu 2026-08-09, SL struktural M30 buat posisi SCALP (lawan arah M30 barusan) bisa jadi >500 pip lebar krn level M30 ketinggalan jauh pas cascade lagi kenceng - lihat memory
...
void OnTick()
{
   ...
   if(g_mode == MODE_NORMAL)
   {
      ...
      if(g_entryDir == g_masterDir && eChangeTime > g_masterChangeTime)
      {
         ...
         if(entryBarTime != g_lastEntryBarTime)
         {
            ...
            if(ADXOk(g_hADX_Entry) && RSIOk(g_hRSI_Entry, g_masterDir))
         ...
         && M1Ok(g_masterDir, g_scalpEntryChangeTime))
      {
         ...
         if(m5BarTime != g_lastM5NormalBarTime)
         {
            ...
            if(ADXOk(g_hADX_ScalpEntry) && RSIOk(g_hRSI_ScalpEntry, g_masterDir))
   ...
   else // MODE_SCALP - trade the OPPOSITE direction of the top Master
   {
      ...
      // SCALP payoff konsisten jelek (~0.26) di semua percobaan sesi ini -
      // dimatiin dulu via InpAllowScalpEntries=false sampai ketemu fix-nya.
      // Cascade detection/mode-switch tetap jalan (masih cut posisi NORMAL
      // begitu M15 lawan Master + M30 confirm) - cuma gak buka posisi BARU
      // di arah SCALP-nya.

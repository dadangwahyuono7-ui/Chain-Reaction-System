...
int PNL_PX = 12, PNL_PY = 18, PNL_W = 310, PNL_H = 735;
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         {
            // Mode first (what kind of trade), then trigger readiness (can it
            // be entered now) - two separate questions, no longer conflated.
            PnlTxtB(x0, y, StringFormat("%s  %s  ·  %s %d/%d", g_convDir, g_convMode,
                    g_convGrade, g_convScore, g_convMax), convClr, 13); y += 19;
...
int      g_convMax      = 0;
string   g_convGrade    = "";
string   g_convMode     = "";   // TREND / TREND-H4 / SCALP - trade type, not quality
...
{
   g_convDir = g_scalpMasterDir;          // M30 = arah yang beneran ditradingkan
   ...
   //=== CONTEXT (HTF) - decides the TRADE TYPE, never whether to enter =====
   // v44.3: Dadang - "harus ada pemicu di TF bawah juga biar bisa entri dari
   // TF bawah, karena meski lemah ini juga udah ratusan pip." He's right, and
   // the previous single-score version contradicted his own doctrine, which
   // this file already states at the top: M30's CMP is the direction traded
   // "independent of whether M30 agrees or disagrees with H4 - that
   // agreement only affects which SL/TP tier is used." Counter-HTF is a
   // MODE (scalp, tighter target), not a defect. So HTF agreement no longer
   // costs score; it selects TREND vs SCALP, and the score is now purely
   // about whether the lower-TF trigger is actually ready to fire.
   datetime tD1;
   string d1 = (g_hExportD1 != INVALID_HANDLE) ? ReadCMP(g_hExportD1, tD1) : "";
   bool h4Aligned = (g_masterDir == g_convDir);
   bool d1Aligned = (d1 == g_convDir);

   if(h4Aligned && d1Aligned)      g_convMode = "TREND";      // run it, wider target
   else if(h4Aligned)              g_convMode = "TREND-H4";   // H4 with, D1 against
   else                            g_convMode = "SCALP";      // counter-HTF: cepat, target pendek

   //=== TRIGGER (LTF + flow) - decides whether it can be entered NOW =======
   //--- 1. M5 trigger agreement (the actual entry trigger per doctrine)
   if(g_scalpEntryDir == "BUY" || g_scalpEntryDir == "SELL")
   {
      g_convMax++;
      if(g_scalpEntryDir == g_convDir) g_convScore++;
      else AddAgainst("M5 belum searah");
   }
   ...
   if(g_bookmapOnline)
   {
      ...
      //--- 2. Order flow (CVD) pointing the same way
      g_convMax++;
      if((isBuy && g_bookmapCvd > 0) || (!isBuy && g_bookmapCvd < 0)) g_convScore++;
      else AddAgainst(StringFormat("CVD %+.0f lawan", g_bookmapCvd));

      //--- 3. No absorption working against this direction
      g_convMax++;
      bool absorbAgainst = (isBuy  && g_bookmapAbsorption == "BUYER_ABSORBED") ||
                           (!isBuy && g_bookmapAbsorption == "SELLER_ABSORBED");
      if(!absorbAgainst) g_convScore++;
      else AddAgainst("Absorption: " + g_bookmapAbsorption);

      //--- 4. Path to target not blocked by a wall sitting right in front.
      ...
      g_convMax++;
      if(blockPx <= 0 || MathAbs(blockPx - px) > nearDist) g_convScore++;
      ...
      if(g_bookmapVah > g_bookmapVal && g_bookmapVal > 0)
      {
         ...
         if(!extended) g_convScore++;
         else AddAgainst(isBuy ? "Harga udah di atas VAH" : "Harga udah di bawah VAL");
      }
   }

   //--- Trigger readiness. Only the LTF/flow factors are counted, so a
   //--- counter-daily scalp with a clean trigger now reads SIAP instead of
   //--- being buried as "LEMAH" - which is what was hiding hundred-pip moves.
   double pct = (g_convMax > 0) ? (100.0 * g_convScore / g_convMax) : 0.0;
   if(g_convMax < 2)     g_convGrade = "DATA KURANG";
   else if(pct >= 80.0)  g_convGrade = "SIAP";
   else if(pct >= 60.0)  g_convGrade = "HATI-HATI";
   else                  g_convGrade = "TUNGGU";

   //--- HTF disagreement doesn't block, but it's worth naming so the target
   //--- expectation matches the mode.
   if(!h4Aligned && g_masterDir != "" && g_masterDir != "WAIT")
      AddAgainst("H4 " + g_masterDir + " - target pendek");
   else if(!d1Aligned && d1 != "" && d1 != "WAIT")
      AddAgainst("D1 " + d1 + " - jangan ditahan lama");

   //--- News proximity overrides everything: no trigger quality survives a
   //--- bad print, and spreads blow out regardless of direction.
...
void WriteSultanStatus()
{
   ...
   json += StringFormat("\"conviction\":{\"dir\":\"%s\",\"mode\":\"%s\",\"score\":%d,\"max\":%d,\"grade\":\"%s\",\"against\":[",

...
int OnInit()
{
   ...
   if(g_hExportD1 == INVALID_HANDLE || g_hExportH1 == INVALID_HANDLE || g_hExportM15 == INVALID_HANDLE || g_hATR == INVALID_HANDLE)
...
void WriteSultanStatus()
{
   ...
   json += StringFormat("\"d1\":\"%s\",\"h1\":\"%s\",\"m15\":\"%s\",", d1Dir, h1Dir, m15Dir);
...
void UpdateValueAreaLines()
{
   ...
   double offset = SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice;
   DrawValueAreaLine(vahName, vahTxt, g_bookmapVah + offset, "VAH");
   DrawValueAreaLine(valName, valTxt, g_bookmapVal + offset, "VAL");
}

void DrawValueAreaLine(string name, string textName, double mt5Price, string label)
{
   if(ObjectFind(0, name) < 0)
   {
      ObjectCreate(0, name, OBJ_HLINE, 0, 0, mt5Price);
...
{
   DrawIcebergLine("BIDICE", g_bookmapBidIcePx, g_bookmapBidIceSz, g_bookmapBidIceRatio, "BID");
   DrawIcebergLine("ASKICE", g_bookmapAskIcePx, g_bookmapAskIceSz, g_bookmapAskIceRatio, "ASK");
}

void DrawIcebergLine(string key, double bookmapPrice, double size, double ratio, string sideLabel)
{

...
int OnInit()
{
   ...
   // v43: EURUSD H4 CMP as the USD fundamental bias. Non-fatal if the
   // broker names the symbol differently or it isn't in Market Watch -
   // the panel just shows "-" for the USD rows.
   g_hEurUsdH4 = iCustom(USD_SYMBOL, PERIOD_H4, "DD_CMP_Indicator");
   if(g_hEurUsdH4 == INVALID_HANDLE)
      Print("INFO: gagal load DD_CMP_Indicator buat ", USD_SYMBOL,
            " - baris USD di panel bakal kosong (bukan fatal). Pastikan ", USD_SYMBOL, " ada di Market Watch.");
...
void UpdatePanel()
{
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         if(g_usdBias != "")
         {
            string setupDir = (g_scalpMasterDir == "BUY" || g_scalpMasterDir == "SELL") ? g_scalpMasterDir : "";
            // Gold rises when the dollar weakens, so a BUY setup wants USD WEAK.
            string verdict = "NEUTRAL";
            color  vClr    = PNL_LABEL;
            if(g_usdBias != "NEUTRAL" && setupDir != "")
            {
               bool aligned = (setupDir == "BUY"  && g_usdBias == "WEAK") ||
                              (setupDir == "SELL" && g_usdBias == "STRONG");
               verdict = aligned ? "SEARAH" : "LAWAN";
               vClr    = aligned ? PNL_EMERALD : PNL_ROSE;
            }
            PnlRow(x0, y, contentW, "USD (via EURUSD)",
                   StringFormat("%s  %s", g_eurDir, g_usdBias),
                   g_usdBias == "WEAK" ? PNL_EMERALD : g_usdBias == "STRONG" ? PNL_ROSE : PNL_LABEL); y += 16;
            PnlRow(x0, y, contentW, "USD vs Setup", verdict, vClr); y += 16;
         }
         else
         {
            PnlRow(x0, y, contentW, "USD (via EURUSD)", "-", PNL_LABEL); y += 16;
         }
         ...
         color biasClr = (bias == "BUY") ? PNL_EMERALD : (bias == "SELL") ? PNL_ROSE : PNL_LABEL;
         PnlTxtB(x0, y, "BIAS — " + bias, biasClr, 13);
      }
      else
      {
...
string   g_eurDir        = "";      // EURUSD H4 CMP: BUY / SELL / WAIT
...
void ReadUsdFundamental()
{
   ...
   if(g_hEurUsdH4 != INVALID_HANDLE)
   {
      ...
      if(g_eurDir == "BUY")       g_usdBias = "WEAK";     // EUR naik -> USD lemah -> dukung emas
      else if(g_eurDir == "SELL") g_usdBias = "STRONG";   // EUR turun -> USD kuat -> tekan emas

...
int PNL_PX = 12, PNL_PY = 18, PNL_W = 310, PNL_H = 610;
...
void UpdatePanel()
{
   ...
   PnlTxt(x0, y, "[ " + EA_VERSION + " ]  " + InpOwnerName, PNL_LABEL, 10); y += 15;
   PnlTxt(x0, y, "Version: " + EA_VERSION, PNL_GOLD, 10); y += 17;
   ...
   PnlAccentRow(x0, y, contentW, "Open / Float P&L", StringFormat("%d   %+.2f", openCount, floatingPL),
                floatingPL >= 0 ? PNL_EMERALD : PNL_ROSE, floatingPL >= 0 ? PNL_EMERALD : PNL_ROSE); y += 16;
   PnlRow(x0, y, contentW, "Closed (W/L)", StringFormat("%d  (%d / %d)", g_statTrades, g_statWins, g_statLosses), PNL_WHITE); y += 16;
   PnlAccentRow(x0, y, contentW, "Total Profit", StringFormat("%+.2f", g_statTotalProfit),
                g_statTotalProfit >= 0 ? PNL_EMERALD : PNL_ROSE, g_statTotalProfit >= 0 ? PNL_EMERALD : PNL_ROSE); y += 16;
   ...
   if(InpShowBookmapPanel)
   {
      if(g_bookmapOnline)
      {
         ...
         PnlTxt(x0, gy + gr + 10, "Price down", PNL_LABEL, 9);
         PnlTxt(x0 + contentW - 56, gy + gr + 10, "Price up", PNL_LABEL, 9);
      ...
      else
      {
         ...
         PnlTxt(x0, y, "(gak jalan / backtest)", clrGray, 10); y += 20;
         PnlTxt(x0, y, "BIAS — N/A", clrGray, 11);
      }
   }

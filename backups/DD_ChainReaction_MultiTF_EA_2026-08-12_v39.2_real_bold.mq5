...
// Dadang: "masih terlalu tipis... solid warna putihnya juga gak jelas" -
// real bold this time via GDI's "<Family> Bold" face-name convention
// (the standard way MQL5 Canvas panels get true bold, e.g. "Arial Bold"),
// NOT the 4x-stamp trick that made text disappear entirely. Single
// TextOut call, same safe code path as PnlTxt() - worst case if the face
// name doesn't resolve is a silent fallback to regular weight, never
// invisible text.
void PnlTxtB(int x, int y, string s, color clr, int size = 14, uint anchor = TA_LEFT | TA_TOP)
{
   g_panelCanvas.FontSet(PNL_FONT + " Bold", size);
   g_panelCanvas.TextOut(x, y, s, ColorToARGB(clr, 255), anchor);
}

...
#define EA_VERSION "v53.34-WALLOFF"
...
input bool                 InpShowWallLinesOnChart= false;  // v53.34 - Dadang, abis dites: "ketika ada price nyentuh snd lama jadi marge... snd nya jadi keliatan numpuk semua" - zona = kumpulan wall by definition, jadi pas kotak zona + garis wall individual nyala bareng PERSIS di titik yang sama (area yang lagi diuji), keduanya numpuk. Balik OFF - saklarnya (v53.32) tetap ada kalau suatu saat mau dicoba lagi
...
// v53.27: DIAGNOSTIK FLICKER CHART - Dadang: "diagnosis flicker bookmap...
// JANGAN mengubah entry/exit/Chain Reaction/S&D calculation/TP/Bookmap
// calculation... jangan langsung refactor besar". Murni instrumentasi
// sementara buat isolasi sumber flicker (bukan fix) - default OFF di
// keduanya = behavior persis sama seperti sebelumnya, nol perubahan.
// TrueBookmapDataOnly = data zona tetap diproses, kotak/label zona DIMATIKAN
// dari chart (isolasi: apakah flicker dari S&D renderer). VisualOnly =
// data zona BERHENTI di-refresh (pakai cache terakhir), tapi kotak/label
// zona TETAP digambar ulang tiap tick (isolasi: apakah redraw-ulang
// sendiri, tanpa perubahan data apa pun, sudah cukup bikin flicker).
// JANGAN dibiarkan ON pas trading beneran - InpDiagBookmapDataOnly=true
// bikin data zona basi (stale) selama diagnostik jalan, termasuk buat
// entry logic yang baca g_zone* kalau InpUseZoneCFEntry aktif.
input group "=== DIAGNOSTIK FLICKER CHART (SEMENTARA - matiin lagi abis selesai tes) ==="
input bool                 InpDiagBookmapDataOnly   = false;  // true = BOOKMAP_DATA=ON, BOOKMAP_VISUAL=OFF (matiin ReadZonesCsv+DrawAllZones -> ObjectsDeleteAll SDZ_PREFIX gak kepanggil lagi)
input bool                 InpDiagBookmapVisualOnly = false;  // true = BOOKMAP_DATA=OFF, BOOKMAP_VISUAL=ON (skip ReadZonesCsv, tapi DrawAllZones tetap jalan pakai data lama - nge-tes murni efek redraw tiap tick)
...
#define VA_ZONE_STABILITY_USD 0.30 // v53.30 - VAL/VAH/POC must move at least this much before redrawing - absorbs Bookmap's continuous micro-recalculation without visually "jumping", see DrawValueAreaZone()/UpdatePocLine()
...
void DrawZoneBox(int rank, string side, double lo, double hi, int wallCount, double totalLot,
                  string status, int retestCount, int absorptionHits, double score)
{
   // v53.28: dropped the separate "idx" param (was DrawAllZones()'s
   // ever-incrementing GLOBAL counter across both sides) - it was purely
   // redundant with "rank" (already stable/unique per side: SUPPLY_0..N,
   // DEMAND_0..N) and its cross-side sharing was what made a safe
   // leftover-object cleanup hard to reason about. See DrawAllZones() for
   // the flicker fix this enabled - Dadang: "diagnosis flicker bookmap...
   // lo atur deh bro" (go ahead and fix it).
   string key    = SDZ_PREFIX + side + "_" + IntegerToString(rank);
   ...
   // v53.26: Dadang - "zona2 yang jauh blm muncul total lotnya" - staggering
   // by the raw draw index pushed high-rank zones' labels dozens of bars
   // off to the right, past the visible chart window entirely (box still
   // drew, its text label just landed off-screen). Wrapped with %6 so the
   // stagger cycles through a small, bounded range instead of growing
   // unbounded with zone count. v53.28: now keyed off "rank" (stable
   // per-side, see function comment above) instead of the removed global
   // "idx" - same bounded behavior, just sourced from the one param that's
   // actually needed.
   datetime labelTime = t1 + (datetime)((t2 - t1) * 0.35) + PeriodSeconds(_Period) * (rank % 6) * 3;
...
// v53.28: FLICKER FIX - Dadang diagnosed live: "diagnosis flicker bookmap...
// lo atur deh bro". Root cause (confirmed by code trace before touching
// anything, see the diagnostic toggles v53.27 added): this function used
// to call ObjectsDeleteAll(SDZ_PREFIX) UNCONDITIONALLY every single call -
// and since DrawAllZones() runs every OnTick() (i.e. multiple times per
// second, independent of whether zones_v2.csv actually changed at all),
// every zone box + label was destroyed and recreated from scratch on
// every tick. DrawZoneBox()'s own "find existing, ObjectMove if present
// else ObjectCreate" logic was already there but never actually reached
// the ObjectMove branch, because the object had just been deleted moments
// earlier in THIS function.
//
// Fix: stop deleting everything up front. Each side's rank (0..N, already
// stable/unique per side - see DrawZoneBox()) now maps to a STABLE object
// name across cycles, so DrawZoneBox() naturally finds-and-moves existing
// objects instead of recreating them - zero delete/create churn on a
// normal tick where the zone set hasn't shrunk. The only remaining
// ObjectDelete calls are the explicit "clean up leftover ranks" loop
// below, which only touches objects when the zone COUNT genuinely drops
// (rare), not on every tick.
void DrawAllZones()
{
   g_supplyRoadCount = 0;
   g_demandRoadCount = 0;
   ...
   {
      if(InpShowZonesOnChart) ObjectsDeleteAll(0, SDZ_PREFIX);   // genuinely nothing to show - legitimate full clear, not the hot path
      return;
   }
   ...
   for(int s = 0; s < 2; s++)
   {
      ...
      // chart drawing - capped, relevance-tiered. Always runs (even when
      // InpShowZonesOnChart is false, with drawnCount=0) so the leftover-
      // cleanup loop below is the ONE place that reconciles what's
      // actually on screen with what should be - no separate full-wipe
      // branch needed for the "display toggled off" case anymore.
      int drawnCount = InpShowZonesOnChart ? MathMin(sc, InpMaxZonesPerSideChart) : 0;
      for(int a = 0; a < drawnCount; a++)
      {
         int i = sideIdx[a];
         DrawZoneBox(a, g_zoneSide[i], g_zoneLo[i], g_zoneHi[i], g_zoneWallCount[i], g_zoneTotalLot[i],
                     g_zoneStatus[i], g_zoneRetest[i], g_zoneAbsorb[i], g_zoneScore[i]);
      }
      // v53.28: delete only the ranks that WERE drawn on a previous cycle
      // but aren't anymore (zone count shrank, or display just got turned
      // off) - on a normal tick where nothing shrank, this touches zero
      // objects.
      for(int a = drawnCount; a < InpMaxZonesPerSideChart; a++)
      {
         string leftoverKey = SDZ_PREFIX + sides[s] + "_" + IntegerToString(a);
         string leftoverTxt = leftoverKey + "_TXT";
         if(ObjectFind(0, leftoverKey) >= 0) ObjectDelete(0, leftoverKey);
         if(ObjectFind(0, leftoverTxt) >= 0) ObjectDelete(0, leftoverTxt);
      }
   }
}
...
// v53.32 BUG FOUND: UpdateWallLines() used to call ObjectsDeleteAll(0,
// WALL_PREFIX) whenever wall lines were off (the default, InpShowZonesOnChart
// case) - but POC/VAH/VAL/Iceberg (UpdatePocLine()/UpdateValueAreaLines()/
// UpdateIcebergLines(), all called RIGHT AFTER UpdateWallLines() in OnTick())
// ALSO use WALL_PREFIX-based names. This blanket delete was silently wiping
// those objects out EVERY SINGLE TICK before they even ran - which is why
// the v53.29/30/31 flicker fix (ShouldSkipRedraw gate) never actually took
// effect: the object was always freshly deleted by THIS function moments
// earlier, so ObjectFind always came back empty and the gate never had
// anything to compare against. Replaced with a cleanup that only touches
// the wall-line-specific object names (BID1..N/ASK1..N), leaving POC/VAH/
// VAL/Iceberg's own objects alone.
void CleanupWallLineObjects()
{
   for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
   {
      string bidKey = WALL_PREFIX + StringFormat("BID%d", i + 1);
      string askKey = WALL_PREFIX + StringFormat("ASK%d", i + 1);
      if(ObjectFind(0, bidKey) >= 0)          ObjectDelete(0, bidKey);
      if(ObjectFind(0, bidKey + "_TXT") >= 0) ObjectDelete(0, bidKey + "_TXT");
      if(ObjectFind(0, askKey) >= 0)          ObjectDelete(0, askKey);
      if(ObjectFind(0, askKey + "_TXT") >= 0) ObjectDelete(0, askKey + "_TXT");
   }
}
...
void UpdateWallLines()
{
   ...
   // v53.32: Dadang wanted the OPTION to see both together again - "kalo
   // aktifin manual bro". Decoupled from InpShowZonesOnChart onto its own
   // independent switch (default off = today's behavior unchanged).
   if(!InpShowWallLinesOnChart)
   {
      CleanupWallLineObjects();
      return;
   }
...
// v53.29: FLICKER GATE - Dadang: "sekarang garis vah poc iceberg yang
// fliker bro" (after the v53.28 zone-box flicker fix). Different root
// cause than v53.28: POC/VAH/VAL/Iceberg already used the correct find-
// and-update pattern (never delete+recreate) - the flicker here comes
// from ReadBookmapBridge() overwriting g_bookmap* every single tick with
// NO smoothing at all, straight from whatever Bookmap just wrote. A tiny
// price wobble or the POC volume ticking by 1 pushed a fresh ObjectSet*
// (and implicit chart repaint) every tick even when nothing on screen
// would actually look different. Classification (Dadang's own A/B/C/D/E
// framework): this one genuinely IS "A - Bookmap data stream", unlike the
// zone boxes which turned out to be "C - renderer". Fix: cache the exact
// display signature last pushed per named object, skip the redraw
// entirely when it's unchanged - genuine changes still redraw
// immediately (no lag added), this only kills redundant no-op churn.
// Deliberately NOT touching Bookmap's own computed values (no smoothing/
// EMA on the price itself - that would make POC/VAH/VAL actually wrong,
// unlike CVD which is already legitimately smoothed elsewhere) - purely a
// display-layer "did this change enough to matter" gate.
string g_lastDrawnSigKey[];
string g_lastDrawnSigVal[];
bool ShouldSkipRedraw(string name, string signature)
{
   for(int i = 0; i < ArraySize(g_lastDrawnSigKey); i++)
   {
      if(g_lastDrawnSigKey[i] == name)
      {
         if(g_lastDrawnSigVal[i] == signature) return true;
         g_lastDrawnSigVal[i] = signature;
         return false;
      }
   }
   int n = ArraySize(g_lastDrawnSigKey);
   ArrayResize(g_lastDrawnSigKey, n + 1);
   ArrayResize(g_lastDrawnSigVal, n + 1);
   g_lastDrawnSigKey[n] = name;
   g_lastDrawnSigVal[n] = signature;
   return false;
}
...
void UpdatePocLine()
{
   ...
   // v53.29: Dadang - "sekarang garis vah poc iceberg yang fliker bro" -
   // different bug from the zone-box one (v53.28): this line already
   // find-and-updates correctly, never delete+recreates. Root cause here
   // is ReadBookmapBridge() overwriting g_bookmapPocPrice/Volume every
   // single tick with ZERO smoothing, straight from whatever Bookmap just
   // wrote. v53.30: exact-value comparison wasn't enough - Bookmap
   // genuinely recalculates POC by small real amounts continuously, not
   // just float noise, so it kept "legitimately" redrawing. Snap to the
   // same stability grid as the new VA zone box for the CHANGE-CHECK only
   // - still draws the true live price once a redraw is warranted, just
   // doesn't chase every sub-30-cent recalculation.
   double snapPocPrice = MathRound(mt5Price / VA_ZONE_STABILITY_USD) * VA_ZONE_STABILITY_USD;
   if(ObjectFind(0, name) >= 0 && ShouldSkipRedraw(name, StringFormat("%.2f|%.0f", snapPocPrice, g_bookmapPocVolume)))
      return;
...
// v53.31: Dadang - "jangan dibuat zona tetap garis aja biar gw yang baca
// itu sebagai zona ntar chart gw penuh lagi biarin seperti semula" - v53.30
// turned VAH/VAL into one box, but that's one more shape competing for
// space on a chart already carrying S&D zone boxes - he'd rather read two
// lines as bounding a zone himself than have another box drawn. Back to
// two separate HLINEs (original v52.x look) - the flicker fix (stability-
// grid snap before redrawing, v53.30) stays, that part was never the
// complaint.
void UpdateValueAreaLines()
{
   ...
   // v53.30 leftovers - the box this used to draw before v53.31 reverted
   // it back to two lines. Harmless no-op once already gone.
   if(ObjectFind(0, WALL_PREFIX + "VAZONE") >= 0)     ObjectDelete(0, WALL_PREFIX + "VAZONE");
   if(ObjectFind(0, WALL_PREFIX + "VAZONE_TXT") >= 0) ObjectDelete(0, WALL_PREFIX + "VAZONE_TXT");
   ...
   double offset = SymbolInfoDouble(_Symbol, SYMBOL_BID) - g_bookmapPrice;
   // v52.37: marginPx staggers VAH/VAL/POC (20/45/70px) so their labels
   // don't stack directly on top of each other when close together.
...
{
   // v53.30/31: stability-grid flicker gate - see VA_ZONE_STABILITY_USD's
   // comment. Still draws the true live price once a redraw is warranted,
   // just doesn't chase every sub-30-cent Bookmap recalculation.
   double snapPrice = MathRound(mt5Price / VA_ZONE_STABILITY_USD) * VA_ZONE_STABILITY_USD;
   if(ObjectFind(0, name) >= 0 && ShouldSkipRedraw(name, StringFormat("%.2f", snapPrice)))
      return;
   ...
   // ChartRightEdgeTime()) instead of a fixed bar-count lane.
...
void DrawIcebergLine(string key, double bookmapPrice, double size, double ratio, string sideLabel, int barsAhead = 8, int reloads = 0)
{
   ...
   // v53.29/v53.30: same flicker gate + stability grid as UpdatePocLine()/
   // DrawValueAreaZone() - see their comments.
   double snapIcePrice = MathRound(mt5Price / VA_ZONE_STABILITY_USD) * VA_ZONE_STABILITY_USD;
   if(ObjectFind(0, name) >= 0 && ShouldSkipRedraw(name, StringFormat("%.2f|%.0f|%.1f|%d", snapIcePrice, size, ratio, reloads)))
      return;
...
void OnTick()
{
   ...
   {
      // v53.27 diagnostik (lihat input group comment) - default kedua flag
      // FALSE, jadi baris ini persis sama seperti sebelumnya (ReadZonesCsv +
      // DrawAllZones selalu jalan). Cuma pas salah satu diagnostik dinyalain
      // manual buat tes flicker, salah satu dilewatin.
      if(!InpDiagBookmapVisualOnly) ReadZonesCsv();   // v53 - separate pipeline/file from bookmap_live_signal.csv above
      if(!InpDiagBookmapDataOnly)   DrawAllZones();

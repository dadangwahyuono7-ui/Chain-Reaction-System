...
#define EA_VERSION "v54.11-V4-GENESIS-FIX"
...
void OnTick()
{
   ...
   ComputeConviction();
   ComputeBookmapNarrative();
   // UpdatePanel() now handled by OnTimer() every 2 seconds
   WriteSultanStatus();

   UpdateAllCandleOpenMarkers();
   // DrawBookmapSndLines(); // BERSIH TOTAL v53.30 Dadang - Gambar Garis S&D Bookmap Area Penting   // v52.58: now draws active-barrier markers, not candle-open markers
   // v53.31: Throttle countdown panel to 1 update/second
   static int s_lastCdSec = -1;
   int curSec = (int)(TimeCurrent() % 60);
   if(curSec != s_lastCdSec) { UpdateCountdownMiniPanel(); s_lastCdSec = curSec; }
   // UpdateToggleMiniPanel(); // REMOVED - panel dibuang v53.30 // v53.30: Dadang - Mini Switchboard Button Panel     // v52.50

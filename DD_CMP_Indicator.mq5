...
// match prev_calculated's incremental convention.
// v53.66 CORRECTION (was wrong here for a long time, caused a real bug on
// the EA side - see DD_ChainReaction_MultiTF_EA_v3.mq5's
// BootstrapBarrierFromHistory()): CopyBuffer(handle, buf, 0, n, dst[]) does
// NOT reorder these into "index 0 = most recent" for the caller. Proven
// live: CopyBuffer(handle,3,0,2000,ctBuf) on D1 returned ctBuf[0]=2018-11-27
// (ancient) and ctBuf[1999]=2026-08-19 (near-present) - i.e. dst[] comes
// back in the SAME oldest-to-newest order as this indicator's own internal
// storage. Any EA reading these buffers across a range (not just the
// single current-bar CopyBuffer(...,0,1,...) calls, which ARE fine since
// there's only one possible order for a 1-element array) must treat
// dst[0]=oldest-of-the-fetched-window, dst[n-1]=newest, not the reverse.

...
class WallLadderTracker:
    ...
    def __init__(self, size_history_len: int = 12, event_log_len: int = 30):
        ...
        # Persistent wall MEMORY (2026-08-10) - separate from the two dicts
        # above, which get pruned back to ONLY currently-listed prices every
        # update(). This one never drops an entry just because a wall
        # scrolled out of the live top-N nearest scan (crowded out by newer
        # closer walls) - only when it hasn't been seen live in a full day
        # (WALL_MEMORY_TTL_SEC). Lets old/far walls that were significant for
        # a long time stay "mapped" - see historical_walls().
        self._wall_memory: Dict[Any, Dict[str, float]] = {}  # (side, price_rounded) -> {price,size,first_seen,last_seen,max_size}
        # Rolling log of wall lifecycle events (NEW/JEBOL/SPOOFED) - dashboard's
        # "Wall History" panel. Snapshot-only fields above don't carry a
        # timeline, this does.
        self.event_log = deque(maxlen=event_log_len)
        # Significant wall-sweep events + reversal outcome - see
        # _maybe_log_sweep()/_resolve_sweeps() below. Newest LAST (plain
        # .append(), unlike event_log's appendleft) so _resolve_sweeps() can
        # walk it in natural chronological order; latest_sweep reads [-1].
    ...
    def historical_walls(self, side: str, min_age_sec: float = WALL_HISTORY_MIN_AGE_SEC, limit: int = 10):
        """Walls that persisted >= min_age_sec at some point (first_seen to
        last_seen span), regardless of whether they're still in the live
        top-N nearest scan right now. Dadang: 'yang diatas 1 jam meski
        jauhnya ribuan pip bisa gak udah termaping' - sorted most-persistent
        (oldest first_seen) first, since that's the strongest 'magnet' zone."""
        now = time.time()
        items = []
        for (s, _p), e in self._wall_memory.items():
            if s != side:
                continue
            if (e["last_seen"] - e["first_seen"]) < min_age_sec:
                continue
            items.append((e["price"], e["size"], now - e["first_seen"]))
    ...
    def update(self, market_data, top_n: int = 8):
        ...
        self._update_memory("ASK", asks, now)
        self._update_memory("BID", bids, now)

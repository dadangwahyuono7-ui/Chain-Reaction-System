...
def _load_engine_context():
    ...
    # 2026-08-26 - Dadang: "di web kan ada AI nya lo benerin promtnya dan
    # biar dia analisa juga berdasarkan area SND bro dia harus jadi
    # analisis dan saran entri". Same "sd_zones" block v53.12 added to
    # WriteSultanStatus() for the web S&D Price Map panel - real
    # Bookmap-derived Supply/Demand zones (strength grading + diuji/serap
    # order-flow evidence), a much sharper grounding than plain POC/VAH/VAL
    # for the AI to build a "zona pantau" suggestion from. Nearest 2 per
    # side only (by distance to current price) - the AI doesn't need the
    # whole roadmap, just what's actually relevant to reason about right now.
    sd = data.get("sd_zones") or {}
    dec = sd.get("decision") or {}
    zones = sd.get("zones") or []
    # 2026-08-26 - MT5 EA v53.18+/v53.25 added the S&D Momentum Break Engine
    # (WICK_BREAK/CLOSE_BREAK/MOMENTUM_BREAK/BREAK_CONFIRMED/FALSE_BREAK/
    # REJECTION state, CLOSE-only doctrine). Dadang: "apa perubahan tadi
    # sudah lo update ke ai web dan web bro yang break out SND tadi?" -
    # straight passthrough so the AI can distinguish "harga cuma nyentuh
    # zona" from "beneran udah confirmed break", instead of guessing from
    # raw zone proximity alone like before this field existed.
    brk = sd.get("break") or {}

    def _nearest(side, edge_key):
        cands = [z for z in zones if z.get("side") == side and z.get("status") != "BROKEN"]
        if price is not None:
            cands.sort(key=lambda z: abs(z.get(edge_key, 0) - price))
        return cands[:2]
    ...
    return {
        ...
        "sdm_location_text": dec.get("location_text"),
        "sdm_setup": dec.get("setup"),
        "sdm_reason_text": dec.get("reason_text"),
        "sdm_buyer_pct": dec.get("buyer_pct"),
        "sdm_seller_pct": dec.get("seller_pct"),
        "sdm_compression": dec.get("compression"),
        "nearest_supply_zones": _nearest("SUPPLY", "lo"),
        "nearest_demand_zones": _nearest("DEMAND", "hi"),
        "sd_break_status_text": brk.get("status_text"),
        "sd_break_direction": brk.get("direction"),
        "sd_break_market_read": brk.get("market_read"),
        "sd_break_no_trade_zone": brk.get("no_trade_zone"),
    }
...
def generate_ai_analysis(items, calendar_events, engine_ctx):
    ...
    if engine_ctx:
        ...
        # 2026-08-26 - the real Supply/Demand zone read (see _load_engine_
        # context()'s comment) - this is the sharper grounding for a "zona
        # pantau"/entry-suggestion read, POC/VAH/VAL above stays as backup
        # context only.
        def _zone_line(z):
            rng = f"{z.get('lo')}-{z.get('hi')}" if z.get("lo") != z.get("hi") else f"{z.get('lo')}"
            return (f"{z.get('side')} {rng} | {z.get('strength', '-')} (skor {z.get('score', 0):.0f}) | "
                    f"{z.get('total_lot', 0):.0f} lot | diuji {z.get('retest_count', 0)}x, serap {z.get('absorption_hits', 0)}x")

        sd_zone_lines = [_zone_line(z) for z in
                          (engine_ctx.get("nearest_supply_zones") or []) + (engine_ctx.get("nearest_demand_zones") or [])]
        sd_zone_block = "\n".join(sd_zone_lines) if sd_zone_lines else "(belum ada zona supply/demand yang cukup solid kebaca)"
        sd_posisi = f"Posisi harga vs zona: {engine_ctx.get('sdm_location_text') or '-'}"
        if engine_ctx.get("sdm_buyer_pct") is not None:
            sd_posisi += f" | Buyer/Seller lean: {engine_ctx['sdm_buyer_pct']:.0f}%/{engine_ctx.get('sdm_seller_pct', 50):.0f}%"
        if engine_ctx.get("sdm_compression"):
            sd_posisi += " | ZONA SEMPIT AKTIF (supply & demand ketemu, tunggu salah satu jebol dulu)"

        # 2026-08-26 - Break Engine read (see _load_engine_context()'s
        # comment) - CLOSE-only doctrine, wick TIDAK dianggap break. Model
        # harus baca status ini persis apa adanya, jangan diperhalus jadi
        # "sudah breakout" kalau statusnya masih WICK_BREAK/CLOSE_BREAK
        # (belum confirmed).
        sd_break_line = f"Status break zona (S&D Momentum Break Engine): {engine_ctx.get('sd_break_status_text') or 'NONE'}"
        if engine_ctx.get("sd_break_market_read") and engine_ctx["sd_break_market_read"] != "NORMAL":
            sd_break_line += f" | Kondisi pasar: {engine_ctx['sd_break_market_read']}"
        if engine_ctx.get("sd_break_no_trade_zone"):
            sd_break_line += " | NO TRADE ZONE (supply-demand kegencet, ruang gerak kesempitan)"
        ...
        engine_block = (
            ...
            f"{wall_block}\n\n"
            f"ZONA SUPPLY & DEMAND (order-flow Bookmap, ini yang paling akurat buat zona pantau):\n"
            f"{sd_posisi}\n"
            f"{sd_zone_block}\n"
            f"{sd_break_line}"
        )
    ...
    system = (
        ...
        "zona SUPPLY/DEMAND dari order-flow) cukup jelas condong ke satu arah, KATAKAN "
        ...
        "zona itu HARUS diturunkan dari level yang beneran ada di data (zona SUPPLY/DEMAND, "
        "POC/VAH/VAL/wall, atau harga sekarang), jangan pernah karang angka baru yang gak ada "
        "dasarnya. Framing-nya WAJIB 'zona pantau'/'area yang diawasi sistem', BUKAN perintah "
        "eksekusi langsung seperti 'BUY SEKARANG' atau 'ENTRI DI...', dan JANGAN kasih TP/SL "
        "spesifik (itu keputusan manajemen risiko dia sendiri, bukan urusanmu). Kalau berita "
        "dan sistemnya BERTENTANGAN arah, bilang apa adanya, jangan dipaksain nyambung.\n\n"
        "CARA BACA ZONA SUPPLY & DEMAND (doktrin Commander Dadang sendiri, WAJIB dipakai kalau "
        "datanya ada - ini lebih akurat daripada POC/VAH/VAL polos): setiap zona punya label "
        "kekuatan LEMAH/SEDANG/KUAT, jumlah 'diuji' (berapa kali harga masuk-keluar zona itu "
        "tanpa jebol - makin sering makin teruji), dan 'serap' (absorption - bukti order-flow "
        "ASLI zona itu dimakan transaksi, bukan cuma lot-nya berkurang). PENTING: lot resting "
        "order di zona itu BISA turun drastis TANPA 'serap' naik - itu artinya order dibatalin/ "
        "dipindah (spoofing/reposisi), BUKAN bukti zona itu beneran mau jebol. Jangan pernah "
        "bilang zona 'mulai lemah/mau jebol' hanya karena lotnya berkurang - baru bilang begitu "
        "kalau 'serap' juga ikut naik. Kalau status lagi 'ZONA SEMPIT AKTIF' (supply & demand "
        "ketemu/nempel), JANGAN kasih zona pantau satu arah - bilang terus terang sistem lagi "
        "nunggu salah satu sisi jebol dulu, itu bukan waktunya nentuin arah.\n\n"
        "STATUS BREAK ZONA (S&D Momentum Break Engine, doktrin CLOSE ONLY): kalau ada baris "
        "'Status break zona', itu WAJIB dipakai apa adanya, jangan diperhalus. WICK_BREAK/"
        "CLOSE_BREAK/MOMENTUM_BREAK/FOLLOW_THROUGH artinya BELUM CONFIRMED - jangan pernah "
        "bilang 'sudah breakout' atau 'sudah tembus' kalau statusnya masih salah satu dari itu, "
        "bilang 'lagi mencoba tembus tapi belum confirmed' atau semacamnya. Cuma BREAK_CONFIRMED "
        "yang boleh disebut breakout beneran. FALSE_BREAK/REJECTION artinya percobaan tembusnya "
        "GAGAL, zona-nya justru bertahan - jangan salah baca jadi tanda breakout.\n\n"
        "PENTING - format highlight: SETIAP kali kamu sebutkan angka zona pantau (range harga "
        "atau level spesifik), BUNGKUS angka/range itu SAJA (jangan seluruh kalimatnya) dengan "
        "tanda kurung siku dobel, contoh: 'zona pantau [[4635-4640]]' atau 'kalau tembus "
        "[[di atas 4650]]'. Ini WAJIB dilakukan setiap kali ada angka zona, dipakai web buat "
        "kasih warna visual - jangan pernah pakai tanda [[ ]] ini untuk hal lain selain angka "
        "zona pantau.\n\n"
        "Tugasmu: baca berita gold terbaru + kalender event hari ini + apa yang lagi dibaca "
        "sistem Chain Reaction miliknya sendiri (TERMASUK zona Supply/Demand-nya), lalu kasih "
        "ANALISA SEKALIGUS SARAN ENTRI (dalam bentuk zona pantau, sesuai aturan di atas) dalam "
        "Bahasa Indonesia casual (gak usah formal banget) yang JUJUR dan TEGAS. WAJIB tutup "
        "analisamu dengan satu kalimat singkat: ini bukan saran finansial, keputusan dan "
        "risiko tetap di tangan Commander Dadang. Maksimal 6-7 kalimat total, jangan bertele-tele."
    )
    user = (

...
input bool                ShowBookmapWallMarkers = false;  // v53.42: OFF - Semua data wall sudah tertulis rapi di dalam kotak S&D (chart bersih tanpa garis wall)
...
input bool                InpSC_BigTradesWhale   = true;   // 4. Whale Bubbles / Big Trades (>50 Lot)
...
{
   EventKillTimer(); // v53.30
   ObjectsDeleteAll(0, "SD_"); // v53.32: cleanup stable zone objects
   ObjectsDeleteAll(0, "DD_SW_");
   ObjectsDeleteAll(0, SDZ_PREFIX);
   ObjectsDeleteAll(0, "DD_SND_"); // Bersihkan total semua garis SND tambahan   // v53 - SND zone boxes
...
double CalculateConfluenceScore(string dir, string &gradeOut, string &reasonsOut)
{
   ...
   // 1. Multi-TF CMP Alignment (Max 30%) - selalu tersedia (CMP indicator native, bukan Bookmap)
   maxScore += 30.0;
   ...
   // v53.54: Dadang audit fix - dulu offline dikasih "score += 20.0 baseline"
   // yang MEMALSUKAN 20 dari 100 poin skor (bisa nembus grade A+ padahal
   // Bookmap mati total, gak ada disclosure apa-apa di HUD). Sekarang:
   // kategori ini cuma masuk PEMBILANG *dan* PENYEBUT kalau online - persis
   // pola g_flowScore/g_flowMax yang udah bener di tempat lain di file ini.
   if(g_bookmapOnline)
   {
      maxScore += 25.0;
   ...
   // 3. S&D & Realtime Liquidity Support (Max 25% = 15 struktural selalu ada + 10 Bookmap-only)
   ...
   // 4. Runway Clearance / Barrier Check (Max 20%) - selalu tersedia (CMP/M30 zone, bukan Bookmap)
   maxScore += 20.0;
   string barReason = "";
   ...
   // Disclosure eksplisit di grade sendiri kalau lagi jalan dengan penyebut
   // yang dikecilin (Bookmap offline) - ngalir otomatis ke kedua consumer
   // (panel row + HUD banner tengah-atas) tanpa perlu ubah dua tempat itu.
...
{
   string nameBg  = "DD_HUD_RADAR_BG";
   string nameTxt = "DD_HUD_RADAR_TXT";
   ...
   if(score >= 85.0)
   {
      icon = "🔥";
      borderClr = clrGold;
      textClr   = (tradeDir == "BUY") ? C'0,255,140' : C'255,100,100';
   }
   else if(score >= 70.0)
   {
      icon = "✅";
      borderClr = (tradeDir == "BUY") ? C'0,220,120' : C'245,60,60';
      textClr   = (tradeDir == "BUY") ? C'180,255,200' : C'255,190,190';
   }
   else if(score >= 50.0)
   {
      icon = "⚠️";
      borderClr = C'255,190,0'; // Gold/Yellow
   ...
   int badgeW = 400;
   int badgeH = 34;
   int posX = (chartWidth / 2) - (badgeW / 2);
   if(posX < 220) posX = 220; // Jangan menabrak panel kiri
   int posY = 15;
   
   // 1. Background Glass Badge
   if(EnsureObject(nameBg, OBJ_RECTANGLE_LABEL))
   {
      ObjectSetInteger(0, nameBg, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, nameBg, OBJPROP_XDISTANCE, posX);
      ObjectSetInteger(0, nameBg, OBJPROP_YDISTANCE, posY);
      ObjectSetInteger(0, nameBg, OBJPROP_XSIZE, badgeW);
      ObjectSetInteger(0, nameBg, OBJPROP_YSIZE, badgeH);
      ObjectSetInteger(0, nameBg, OBJPROP_BGCOLOR, C'12,18,26'); // Dark Obsidian
      ObjectSetInteger(0, nameBg, OBJPROP_BORDER_TYPE, BORDER_FLAT);
      ObjectSetInteger(0, nameBg, OBJPROP_COLOR, borderClr);
      ObjectSetInteger(0, nameBg, OBJPROP_WIDTH, (score >= 85.0 ? 2 : 1));
      ObjectSetInteger(0, nameBg, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, nameBg, OBJPROP_HIDDEN, true);
   }
   
   // 2. Text Label
   string hudText = StringFormat("%s RADAR KONFLUENSI: %.0f%% %s [%s]", icon, score, tradeDir, grade);
   if(EnsureObject(nameTxt, OBJ_LABEL))
   {
      ObjectSetInteger(0, nameTxt, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, nameTxt, OBJPROP_XDISTANCE, posX + 16);
      ObjectSetInteger(0, nameTxt, OBJPROP_YDISTANCE, posY + 8);
      ObjectSetString(0, nameTxt, OBJPROP_TEXT, hudText);
      ObjectSetInteger(0, nameTxt, OBJPROP_COLOR, textClr);
      ObjectSetInteger(0, nameTxt, OBJPROP_FONTSIZE, 10);
      ObjectSetString(0, nameTxt, OBJPROP_FONT, "Segoe UI Bold");
      ObjectSetInteger(0, nameTxt, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, nameTxt, OBJPROP_HIDDEN, true);
   }
}
...
void UpdatePanel()
{
   ...
   {
      PnlRule(x0, y, contentW); y += 10;
      PnlTxtB(x0, y, "S&D PRICE MAP (BOOKMAP)", PNL_GOLD, 11); y += 17;

      // Detail Lengkap S1 & D1
      if(g_supplyRoadCount > 0)
      {
         PnlTxt(x0, y, StringFormat("🔴 S1: %.2f–%.2f (%.0fL • %s)", g_supplyRoadLo[0], g_supplyRoadHi[0], g_supplyRoadLot[0], ZoneStrengthID(g_supplyRoadScore[0])), PNL_ROSE, 9);
         y += 14;
      }
      if(g_demandRoadCount > 0)
      {
         PnlTxt(x0, y, StringFormat("🟢 D1: %.2f–%.2f (%.0fL • %s)", g_demandRoadLo[0], g_demandRoadHi[0], g_demandRoadLot[0], ZoneStrengthID(g_demandRoadScore[0])), PNL_EMERALD, 9);
         y += 14;
      }
                  ...
                  string status, int retestCount, int absorptionHits, double score)
{
   ...
   string labelText = StringFormat("%s [%s] %s %.0f | Uji %dx | %s: %.0fL | %s",
                                   tag, ZoneSourceTF(score), ZoneStrengthID(score), score, retestCount,
...
{
   if(!ShowBookmapWallMarkers || !g_toggleWalls)
   {
      // Cleanup individual wall lines if toggled off
      for(int i = 0; i < WALL_SLOTS_PER_SIDE; i++)
      {
   ...
   if(!g_bookmapOnline || g_bookmapPrice <= 0) return; // v53.41: keep existing lines if offline
...
{
   // v53.39: Bersihkan total semua kotak wall lama agar tidak ada kotak dobel/raksasa
...
bool TryOpen(string dir, string tag, bool isScalp, datetime parentFlipTime = 0, datetime childFlipTime = 0, double slOverride = 0.0, double tpOverride = 0.0)
{
   ...
   {
      string alertMsg = StringFormat("🚀 [DD-V3 ENTRY %s]: %s @ %.2f | SL=%.2f | TP=%.2f", dir, tag, px, sl, tp);
      Alert(alertMsg);
...
void UpdateChainSignal()
{
   ...
   // v53.36: DADANG AUTO-ENTRY FOR CHAIN SIGNALS (TESTER REPLAY & LIVE)
   if(InpAllowScalpEntries)
   {
      bool isScalp = (g_scalpEntryDir != g_masterDir);
      string tag = StringFormat("CHAIN_%s_#%d", g_scalpEntryDir, g_chainSigLayer);
      TryOpen(g_scalpEntryDir, tag, isScalp, g_scalpMasterChangeTime, evtTime);
   }
...
bool g_togglePocVa = true;  // POC, VAH, VAL Lines

#define SWPFX "DD_SW_"

void CreateOneToggleButton(string name, int x, int y, int w, int h, string text, bool state, color activeClr)
{
   if(ObjectFind(0, name) < 0)
   {
   ...
   ObjectSetString(0, name, OBJPROP_TEXT, text);
   ObjectSetInteger(0, name, OBJPROP_BGCOLOR, state ? activeClr : C'35,38,45');
   ObjectSetInteger(0, name, OBJPROP_COLOR, state ? clrBlack : C'180,185,195');
   ObjectSetInteger(0, name, OBJPROP_BORDER_COLOR, state ? clrWhite : C'60,65,75');
   ObjectSetInteger(0, name, OBJPROP_STATE, state);
}

void UpdateToggleMiniPanel()
{
   int baseX = 270; // Jarak dari kanan (di sebelah kiri panel countdown)
   int baseY = 10;
   int btnW  = 62;
   int btnH  = 20;
   int gap   = 4;

   // 1. Tombol S&D Bookmap
   CreateOneToggleButton(SWPFX + "SND", baseX, baseY, btnW, btnH,
                         g_toggleSnd ? "S&D: ON" : "S&D: OFF", g_toggleSnd, C'0,230,120');

   // 2. Tombol CMP Lines
   CreateOneToggleButton(SWPFX + "CMP", baseX - (btnW + gap), baseY, btnW, btnH,
                         g_toggleCmp ? "CMP: ON" : "CMP: OFF", g_toggleCmp, C'255,80,80');

   // 3. Tombol Live Walls
   CreateOneToggleButton(SWPFX + "WALL", baseX - (btnW + gap) * 2, baseY, btnW, btnH,
                         g_toggleWalls ? "WALL: ON" : "WALL: OFF", g_toggleWalls, C'0,180,255');

   // 4. Tombol POC / VA
   CreateOneToggleButton(SWPFX + "POC", baseX - (btnW + gap) * 3, baseY, btnW, btnH,
                         g_togglePocVa ? "POC: ON" : "POC: OFF", g_togglePocVa, C'255,200,40');
}
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == SWPFX + "SND")
      {
         g_toggleSnd = !g_toggleSnd;
         if(!g_toggleSnd) ObjectsDeleteAll(0, "DD_BM_SND_");
         else { /* redraw via OnTimer */ }
      }
      else if(sparam == SWPFX + "CMP")
      {
         g_toggleCmp = !g_toggleCmp;
         if(!g_toggleCmp)
         {
            ObjectsDeleteAll(0, "DD_D1_LINE_"); ObjectsDeleteAll(0, "DD_D1_TXT_");
            ObjectsDeleteAll(0, "DD_H4_LINE_"); ObjectsDeleteAll(0, "DD_H4_TXT_");
            ObjectsDeleteAll(0, "DD_H1_LINE_"); ObjectsDeleteAll(0, "DD_H1_TXT_");
            ObjectsDeleteAll(0, "DD_M30_LINE_");ObjectsDeleteAll(0, "DD_M30_TXT_");
            ObjectsDeleteAll(0, "DD_M5Barrier"); ObjectsDeleteAll(0, "DD_M15Barrier");
            ObjectsDeleteAll(0, "DD_M30Barrier");ObjectsDeleteAll(0, "DD_H1Barrier");
         }
         else UpdateAllCandleOpenMarkers();
         // UpdateToggleMiniPanel(); // REMOVED - panel dibuang v53.30
         // ChartRedraw(0); // removed - anti-flicker v53.30
      }
      else if(sparam == SWPFX + "WALL")
      {
         g_toggleWalls = !g_toggleWalls;
         if(!g_toggleWalls)
         {
            ObjectsDeleteAll(0, WALL_PREFIX);
         }

...
LOCAL_BRIDGE_FILE = os.path.join(BASE_DIR, "bookmap_live_signal.csv")

candidates_dir = [
    r"C:\Users\R O V A\AppData\Roaming\MetaQuotes\Terminal\Common\Files",
    r"C:\Users\Administrator\AppData\Roaming\MetaQuotes\Terminal\Common\Files",
    BASE_DIR
]
MT5_COMMON_FILES_DIR = BASE_DIR
for cd in candidates_dir:
    if os.path.exists(cd):
        MT5_COMMON_FILES_DIR = cd
        break
...
FULL_DEPTH_FILE = os.path.join(MT5_COMMON_FILES_DIR, "bookmap_full_depth.csv")

def write_full_depth_file(market_data, wall_tracker=None):
    try:
        now = time.time()
        tmp = FULL_DEPTH_FILE + ".tmp"
        with open(tmp, "w", encoding="ascii", newline="") as f:
            w = csv.writer(f)
            w.writerow(["side", "price", "size", "age_sec"])
            
            # Write bids with age
            for p, s in market_data.bids_depth.items():
                if s > 0:
                    age = 0.0
                    if wall_tracker and ("BID", p) in wall_tracker._wall_memory:
                        age = now - wall_tracker._wall_memory[("BID", p)].get("first_seen", now)
                    w.writerow(["BID", f"{p:.2f}", f"{s:.1f}", f"{age:.0f}"])
                    
            # Write asks with age
            for p, s in market_data.asks_depth.items():
                if s > 0:
                    age = 0.0
                    if wall_tracker and ("ASK", p) in wall_tracker._wall_memory:
                        age = now - wall_tracker._wall_memory[("ASK", p)].get("first_seen", now)
                    w.writerow(["ASK", f"{p:.2f}", f"{s:.1f}", f"{age:.0f}"])
        os.replace(tmp, FULL_DEPTH_FILE)
    except Exception:
        pass
    ...
    try:
        ...
        for _attempt in range(8):
            try:
                ...
                import time as _time; _time.sleep(0.015)
        ...
        while True:
            ...
            if now - last_write >= WRITE_INTERVAL_SEC:
                ...
                try:
                    ...
                    write_full_depth_file(market_data, master_engine.wall_ladder_tracker)

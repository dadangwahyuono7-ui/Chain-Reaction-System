...
function render(data) {
  ...
  // 8. S&D PRICE MAP
  renderSdZoneMap(data.sd_zones || {}, data.price);
}

// v53.12 - S&D Price Map: straight passthrough of the EA's
// g_zone*/g_sdm*/roadmap export (WriteSultanStatus()'s new "sd_zones"
// block) - same "EA computes, web just displays" pattern as everything
// else here. Dadang: "zone di uji berapa kali total lot sedang... dan
// ketika price memasuki area itu harus pulse berkedip atau ada warning
// area sampai suplay atau demand".
const SDM_STRENGTH_CLASS = {
  KUAT: "bg-amber-400/15 text-amber-300",
  SEDANG: "bg-slate-400/15 text-slate-300",
  LEMAH: "bg-slate-700/40 text-slate-500",
};

function sdmZoneRow(z, side, price) {
  const isInside = price !== undefined && price !== null && price >= z.lo && price <= z.hi;
  const dist = side === "SUPPLY" ? (z.lo - price) : (price - z.hi);
  const distTxt = isInside ? "DI DALAM ZONA" : `${dist >= 0 ? "+" : ""}${fmt(dist, 1)} pts`;
  const sideBorder = side === "SUPPLY" ? "border-rose-400/20" : "border-emerald-400/20";
  const insideCls = isInside ? (side === "SUPPLY" ? "zone-inside-supply" : "zone-inside-demand") : "";
  const strengthCls = SDM_STRENGTH_CLASS[z.strength] || SDM_STRENGTH_CLASS.LEMAH;
  const range = z.lo === z.hi ? fmt(z.lo, 2) : `${fmt(z.lo, 2)}-${fmt(z.hi, 2)}`;
  return `<div class="px-2 py-1 rounded-md border ${sideBorder} ${insideCls}">
    <div class="flex justify-between items-baseline">
      <span class="font-mono text-[11.5px] font-bold text-slate-100">${range}</span>
      <span class="text-[9px] font-bold px-1.5 py-0.5 rounded-full ${strengthCls}">${z.strength || "-"}</span>
    </div>
    <div class="flex justify-between text-[10px] text-slate-500 mt-0.5">
      <span>${zoneSourceTfLabel(z.score)} &middot; diuji ${z.retest_count || 0}x</span>
      <span class="${isInside ? "text-amber-300 font-bold" : ""}">${distTxt}</span>
    </div>
  </div>`;
}

// v53.58 EA: zona sekarang dari Barrier CMP (dulu wall-accumulation) -
// total_lot/absorption_hits gak ada padanan buat barrier CMP (selalu 0),
// diganti TF asal barrier-nya (skor deterministik per TF: D1=95/H4=85/H1=70/
// M30=55, sama persis logic ZoneSourceTF() di EA MQL5). retest_count v53.60
// JADI REAL LAGI (Dadang: "kenapa gak bisa, harus bisa ketika disentuh
// price") - TrackBarrierRetests() di EA ngitung tiap kali harga masuk zona
// dari luar, bukan dikarang.
function zoneSourceTfLabel(score) {
  if (score >= 90) return "D1";
  if (score >= 80) return "H4";
  if (score >= 65) return "H1";
  return "M30";
}

function renderSdZoneMap(sd, price) {
  const focusEl = document.getElementById("sdm-focus");
  if (!focusEl) return; // page doesn't have this panel (shouldn't happen, but stay safe)

  const dec = sd.decision || {};
  const zones = sd.zones || [];

  // Public-surface rule: the EA's own FocusText() reads "FOKUS: BUY/SELL"
  // (fine on the private MT5 panel) - this page is reachable via the
  // public tunnel, so it gets reworded to an observation, never a bare
  // command. See feedback_no_explicit_signals_on_public_surfaces memory.
  const rawFocus = dec.focus || "FOKUS: TUNGGU";
  const focusDisplay = rawFocus.replace("FOKUS:", "ZONA PANTAU:");
  const focusClr = rawFocus.includes("BUY") ? "text-emerald-400"
                  : rawFocus.includes("SELL") ? "text-rose-400"
                  : dec.market_state === "ZONE COMPRESSION" ? "text-amber-400"
                  : "text-slate-400";
  set("sdm-focus", focusDisplay, focusClr);
  set("sdm-reason", dec.reason_text || "-", "text-slate-400");

  set("sdm-price", price !== undefined ? fmt(price) : "-", "text-white");
  flashIfChanged("sdm-price", price);
  set("sdm-location", dec.location_text || "-", "text-slate-300");
  const buyerPct = dec.buyer_pct ?? 50, sellerPct = dec.seller_pct ?? 50;
  set("sdm-bs", `${fmt(buyerPct, 0)}% / ${fmt(sellerPct, 0)}%`, buyerPct >= sellerPct ? "text-emerald-400" : "text-rose-400");

  const warnEl = document.getElementById("sdm-warning");
  if (warnEl) {
    const base = "text-center text-[10.5px] font-bold tracking-wider rounded-md py-1 mb-1 zone-warn";
    if (dec.compression) {
      warnEl.textContent = "⚠️ ZONA SEMPIT — TUNGGU SALAH SATU JEBOL";
      warnEl.className = base + " bg-orange-400/15 text-orange-300";
    } else if (dec.location === "NEAR_SUPPLY") {
      warnEl.textContent = "⚠️ MENDEKATI ZONA SUPPLY";
      warnEl.className = base + " bg-rose-400/15 text-rose-300";
    } else if (dec.location === "NEAR_DEMAND") {
      warnEl.textContent = "⚠️ MENDEKATI ZONA DEMAND";
      warnEl.className = base + " bg-emerald-400/15 text-emerald-300";
    } else {
      warnEl.className = "hidden";
    }
  }

  // 2026-08-26: S&D Momentum Break Engine passthrough (MT5 v53.18+, web
  // export v53.25) - straight text/color mirror of the MT5 panel's own
  // "Break Status" row, same source data (sd_zones.break), never
  // recomputed here.
  const brk = sd.break || {};
  const brkTxt = brk.status_text || "NONE";
  let brkClr = "text-slate-400";
  if (brk.direction === "BUY") brkClr = "text-emerald-400";
  else if (brk.direction === "SELL") brkClr = "text-rose-400";
  else if (brk.market_read === "CHOPPY" || brk.market_read === "COMPRESSION") brkClr = "text-amber-400";
  else if (/MOMENTUM_BREAK|FOLLOW_THROUGH|CLOSE_BREAK|WICK_BREAK/.test((brk.supply_state || "") + (brk.demand_state || ""))) brkClr = "text-amber-400";
  else if (/FALSE_BREAK|REJECTION/.test((brk.supply_state || "") + (brk.demand_state || ""))) brkClr = "text-slate-500";
  set("sdm-break", brkTxt, brkClr);

  const supplyEl = document.getElementById("sdm-supply-list");
  const demandEl = document.getElementById("sdm-demand-list");
  if (!supplyEl || !demandEl) return;
  const supply = zones.filter(z => z.side === "SUPPLY" && z.status !== "BROKEN")
    .sort((a, b) => Math.abs(a.lo - price) - Math.abs(b.lo - price)).slice(0, 3);
  const demand = zones.filter(z => z.side === "DEMAND" && z.status !== "BROKEN")
    .sort((a, b) => Math.abs(a.hi - price) - Math.abs(b.hi - price)).slice(0, 3);
  supplyEl.innerHTML = supply.length
    ? supply.map(z => sdmZoneRow(z, "SUPPLY", price)).join("")
    : '<div class="text-[10.5px] text-slate-600 text-center py-1">Belum ada zona</div>';
  demandEl.innerHTML = demand.length
    ? demand.map(z => sdmZoneRow(z, "DEMAND", price)).join("")
    : '<div class="text-[10.5px] text-slate-600 text-center py-1">Belum ada zona</div>';
}

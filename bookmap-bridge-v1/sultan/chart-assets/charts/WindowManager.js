...
import { DrawingEngine } from '../drawings/DrawingEngine.js?v=10';
...
// --- Symbol Search Input (TradingView style) ---
  ...
  // ─────────────────────────────────────────────────────────────────────────
    ...
    if (loc.val > 0 && Math.abs(loc.val - loc.poc) > 1.5) {
      ...
      updateMegaWallPriceLines(liq, curPrice) {
    if (!this.candleSeries || this.symbol !== "XAUUSD" || !liq || !curPrice) return;
    ...
    let askPx = 0, askSz = 0;
    const askLadder = liq.ask_ladder || [];
    ...
    let bidPx = 0, bidSz = 0;
    const bidLadder = liq.bid_ladder || [];
...
export class WindowManager {
  ...
  applyStatusSnapshot(raw, isLiveTick) {
    // Discretionary Box Quant Engine (DrawingEngine.js's "rectangle" case)
    // reads window.lastSultanStatus for LIVE WALL/lot-sizing scanning - this
    // was never actually assigned anywhere, so the scanner always saw
    // `sultan === undefined` and silently no-op'd via its own try/catch.
    window.lastSultanStatus = raw;
